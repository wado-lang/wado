...
- `test`, `test-wado` and `test-gale-o3` refuse to start while another of the same is running (`scripts/exclusive.sh`), naming the holder's pid. Abandoning a job does not stop it, so restarting after an edit means killing that pid first.
