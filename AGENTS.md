...
- Test the language from an e2e fixture: a `.wado` file in `wado-compiler/tests/fixtures/`, expectations in its `__DATA__` section. Nearly everything the language does is stated there, diagnostics included. A fixture states a rejection by naming the message, as `{"compile_error": "…"}`, never the `Code`. Grepping the fixtures for a `Code` name therefore finds nothing and proves nothing. Kiln is the exception: a generator runs against the filesystem, which a fixture cannot set up.
