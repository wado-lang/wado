...
- Answering review findings — a human reviewer, CodeRabbit, any review bot — starts by running `/code-review-response`. Reading a finding and fixing it directly is the mistake it exists to stop.
- Run `/distill` once a piece of work is done, and again after answering review feedback. Running it one more time costs nothing, so run it the moment you wonder whether you should.
