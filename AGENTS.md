...
- `wado-manifest/` — `wado.toml` / `wado.lock` parsing, validation, and dependency resolution. Pure: no I/O.
- `wado-workspace/` — the filesystem side of the above: workspace discovery and offline resolution of `[dependencies]`. Shared by `wado-lsp` and `wado-cli`.
