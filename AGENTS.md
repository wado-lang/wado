...
- Write all documentation and comments in clear, simple English.
  - Comments: write none. That is the default, not a target to approach. The exception is what the code cannot say — why this way, a tradeoff, a constraint, a spec or bug reference — and it gets two lines at most. If the code can say it, rename and decompose until the comment is redundant, then delete it.
