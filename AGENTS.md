...
- Read a branch through `git diff <base> -- $(scripts/changed-sources.sh)`. It drops what `.gitattributes` marks `linguist-generated`, which a generator change otherwise buries the sources under. Regenerate and commit those; do not read them.
