...
- Don't edit sources while a `wado test` run is in flight. The run pins each Kiln generator at its first resolve, so it stays internally consistent, and it fails at the end naming every source that changed under it — its verdict describes neither tree, so re-run rather than reading the result.
