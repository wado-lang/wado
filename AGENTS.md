...
- Test the language from an e2e fixture: a `.wado` file in `wado-compiler/tests/fixtures/`, expectations in its `__DATA__` section. Nearly everything the language does is stated there, diagnostics included. A rejection is stated by its text, as `{"compile_error": "…"}`, matched against the whole report — so writing `":4:13: parse error: …"` pins the position too — and by its `Code`, as `{"compile_error_codes": ["INVALID_SYNTAX"]}`. Kiln is the exception: a generator runs against the filesystem, which a fixture cannot set up.
