...
- Name an item, don't spell out its path: a `crate::` or `super::` path belongs in a `use` item at the top of the module, never inline where the item is read. `mise run check-rust-paths` gates this in CI against `scripts/rust-inline-paths.json`, which holds what each file has left to migrate. Clippy is not the gate: `absolute_paths` reads only the `crate::` half, and only once every dependency is named in an allow-list it has no way to derive.
