...
- Cloud sessions build incrementally, on cargo's default — leave `CARGO_INCREMENTAL` unset rather than pinning it either way. It earns its disk: editing `wado-compiler` and rebuilding costs ~8 s against ~101 s without. Budget for it, though — `target/debug/incremental` reaches several GB over a session, against a fixed disk allowance.
- A cloud session starts warm because `.claude/hooks/cargo-cache-restore.mts` unpacks a prebuilt `target/`; it never rebuilds the dependency graph, so only workspace crates compile. Feature resolution differs per command, though, and each configuration is a separate artifact set: a command the cache was not built for pays for its own dependency configuration once (`cargo check -p wado-compiler` cost ~357 s uncovered). `.github/workflows/cargo-cache.yml` lists the covered set; add to it rather than working around a slow first build.
