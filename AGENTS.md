...
- Name an item, don't spell out its path: a `crate::` or `super::` path belongs in a `use` item at the top of the module, never inline where the item is read. A `pub(in …)` is exempt — it names a scope rather than reading an item, and Rust admits no import there. `mise run check-rust-paths` gates this in CI against `scripts/rust-inline-paths.json`, which is empty: the corpus carries none, and a file that gains one fails the check. The detector is Wado (`package-gale/tools/rust_inline_paths.wado`) and parses with the Gale Rust grammar, so the grammar decides what counts as a path. `scripts/check-rust-paths.sh <file.rs>…` lists what a file carries.
