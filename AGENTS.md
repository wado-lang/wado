...
- Cloud sessions build incrementally (`CARGO_INCREMENTAL` unset). It is worth its disk: editing `wado-compiler` and rebuilding costs ~8 s incrementally against ~101 s without. Budget for it — `target/debug/incremental` reaches several GB over a session, against a fixed disk allowance. Never set `CARGO_INCREMENTAL=1` explicitly: sccache aborts outright on it (`increment compilation is prohibited`), and cargo's own default already gives you incremental.
- A cloud session starts warm because `.claude/hooks/cargo-cache-restore.mts` unpacks a prebuilt `target/` — it never rebuilds the dependency graph. Only workspace crates compile. Feature resolution differs per command, though, and each configuration is a separate artifact set: a command the cache was not built for pays for its own dependency configuration once (`cargo check -p wado-compiler` cost ~357 s when uncovered). `.github/workflows/cargo-cache.yml` lists the covered set; add to it rather than working around a slow first build.
