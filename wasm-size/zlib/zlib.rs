...
fn main() {
    ...
    let (decompressed, rc) =
        zlib_rs::decompress_slice(&mut decompressed, COMPRESSED_DATA, zlib_rs::InflateConfig::default());
    assert!(rc == zlib_rs::ReturnCode::Ok);
    let decompressed_len = decompressed.len();

    // Verify: check first few bytes of pattern
    assert!(decompressed_len == 1024);
