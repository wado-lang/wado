#include <stdio.h>
#include <string.h>
#include "zlib-1.3.1/zlib.h"

// Pre-compressed zlib data (1024 bytes of pattern i%256, compressed with zlib level 6)
static const unsigned char compressed_data[] = {
    
};

int main(void) {
    // Decompress
    unsigned char decompressed[1024];
    uLongf decompressed_len = sizeof(decompressed);
    int rc = uncompress(decompressed, &decompressed_len, compressed_data, sizeof(compressed_data));
    if (rc != Z_OK) {
        printf("uncompress failed: %d\n", rc);
        return 1;
    }

    // Verify: check first few bytes of pattern
    if (decompressed_len != 1024 || decompressed[0] != 0 || decompressed[1] != 1 || decompressed[255] != 255) {
        printf("verification failed\n");
        return 1;
    }

    printf("zlib %s: %lu -> %lu\n",
           zlibVersion(), (unsigned long)sizeof(compressed_data),
           (unsigned long)decompressed_len);
    return 0;
}
