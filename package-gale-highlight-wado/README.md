...
method. A segment spelled with a contextual keyword is still a name, though —
`Instant::from(x)` is the shape every `From` impl is called through — so those
words are captured there. `.method()` is not the same case: there the grammar
