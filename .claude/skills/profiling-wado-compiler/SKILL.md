...
- **A generic hot spot may be dozens of call sites wearing one name.**
  ...
  symbolicator reports whichever name sorts first — so one pass appears
  to own cost that belongs to the whole compiler. A dev build folds 125
  `Body::for_each_child::<closure>` instantiations onto one address,
  which reads as `optimize::drve` at ~10 % self CPU while the `nir/drve`
  span measures 0.8 %. A span that contradicts the profile is the tell;
  confirm with `nm target/debug/wado | grep <symbol>` and count how many
  names share the address before attributing the cost to a pass.
