...
- Keep the code free of compiler warnings and Clippy lints. `mise run clippy` runs `-D warnings`, so any warning or lint fails it. The one standing exception is `clippy::absolute_paths` over the corpus that predates the rule below — leave those alone unless you are migrating the file.
- Import every item you name. A `crate::` or `super::` path goes in a `use` item at the top of the module; where the item is read, write its name. `use crate::elaborator::sem::Ty;` then `Ty`, never `crate::elaborator::sem::Ty` inline. Alias a collision (`use crate::ast::Ty as AstTy;`) rather than spelling the path out. An edit that writes one inline is refused by a hook.
