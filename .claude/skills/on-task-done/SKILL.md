...
- If the changes only includes Wado programs, only `mise run test-wado` is required.
- If the changes only includes documentation, only `mise run format` is required.
