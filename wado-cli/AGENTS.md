...
- `discover.rs`, `test_report.rs` — source file discovery (shared by `test` and `format`) and the progress digest. A path argument goes through `files_in_dir` or `discover_tree`. The subcommand supplies only a `Filters` callback naming its manifest section, so how a directory expands cannot drift between the two.
