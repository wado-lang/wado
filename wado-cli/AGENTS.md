...
- `run_cache.rs` — what one CLI run resolves once and then holds fixed: AOT generator components, generator resolutions, and the watch over the sources the run read. `wado test` shares one across every fixture, so a generator edited mid-run cannot split the run, and the run fails at the end naming any source that moved under it.
