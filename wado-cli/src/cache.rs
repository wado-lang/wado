...
//! logic — and the `root` in [`wado_workspace::cache_root`], so the CLI (which
...
/// [`wado_workspace::cache_root`] resolver (and the embedded LSP server) observe
/// the configured root. Config parsing lives here — the single native host — so
/// the crates below it (`wado-manifest`, `wado-workspace`, `wado-lsp`) never pull
