...
pub struct ResolvedGenerator {
    pub wasm: Vec<u8>,
    /// `None` when the generator has no `pub struct Options`, or a prebuilt
    /// one carries no WIT to read it from. [`typed_encode_options`] then falls
    /// back to the provisional TOML encoding.
