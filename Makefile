...
update-golden-fixtures:
	...
	@files=$$(grep -L '"compile_error"' wado-compiler/tests/fixtures/*.wado); \
	cargo run --bin wado --quiet -- dump --optimize --unparse -O2 $$files 2>/dev/null | awk ' \
		/^\/\/ ========== / { \
			if (filename != "") { \
				close(filename); \
			} \
			name = $$0; \
			sub(/^\/\/ ========== wado-compiler\/tests\/fixtures\//, "", name); \
			sub(/\.wado ==========$$/, "", name); \
			filename = "wado-compiler/tests/fixtures.golden/" name ".lowered.wado"; \
			print "  Generating " filename > "/dev/stderr"; \
			print "// Golden file: Lowered TIR with -O2 optimization" > filename; \
			print "// Source: tests/fixtures/" name ".wado" >> filename; \
			print "// Generated by: make update-golden-fixtures" >> filename; \
			print "" >> filename; \
			in_entry = 0; \
			next; \
		} \
		/^\/\/ --- Module:  ---$$/ { in_entry = 1; next; } \
		/^\/\/ --- Module:/ { in_entry = 0; next; } \
		in_entry && filename != "" { print >> filename; } \
	'
