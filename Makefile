...
update-golden-fixtures:
	...
	@for file in wado-compiler/tests/fixtures/*.wado; do \
		name=$$(basename $$file .wado); \
		output="wado-compiler/tests/fixtures.golden/$$name.lowered.wado"; \
		echo "  Generating $$output"; \
		if cargo run --bin wado --quiet -- dump --optimize --unparse -O2 "$$file" 2>/dev/null | awk ' \
		...
		' > "$$output.tmp"; then \
			if [ -s "$$output.tmp" ]; then \
				{ \
					echo "// Golden file: Lowered TIR with -O2 optimization"; \
					echo "// Source: tests/fixtures/$$name.wado"; \
					echo "// Generated by: make update-golden-fixtures"; \
					echo ""; \
					cat "$$output.tmp"; \
				} > "$$output"; \
				rm -f "$$output.tmp"; \
			else \
				echo "  WARNING: Empty output for $$output (skipping, best effort)"; \
				rm -f "$$output.tmp" "$$output"; \
			fi; \
		else \
			echo "  WARNING: Failed to generate $$output (best effort, continuing...)"; \
			rm -f "$$output.tmp" "$$output"; \
		fi; \
	done
