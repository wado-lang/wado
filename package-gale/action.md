...
The recognizer API is a table, not a chain of special cases. Each entry is a variant of `ApiCall`, and the two questions every entry has to answer — which recognizer carries it, and whether reading it needs the enclosing match — are total matches over that variant, so an entry added without answering them does not compile. The questions used to be asked at the site that emitted each entry, which meant they were asked at some and not others: the parser's token stream reached a lexer body as `lx.la(1)`, and a `@members` method reached a match window that is not in scope there. Both surfaced as the generated module failing to compile rather than as a diagnostic naming the call.
...
Nearly every defect this pipeline has produced had one signature: generation reported nothing and the _generated module_ did not compile. The unit tests cannot see it — they assert on emitted text — and a driver test sees it only for a grammar someone had already written, so the failures were found by reading code rather than by running it.

`tests/driver_surface_matrix_test.wado` exists for that. Its fixtures are not about a behaviour; importing them is the assertion, because a Kiln import compiles what the generator emitted. One grammar per surface that emits differently: the action pipeline end to end (members across blocks, per-receiver member methods, the value channel, prequels), elements named after the locals a parse function binds, and a lexer whose only action content is its `@members` block — the machinery a member reaches is not always the machinery an action turns on.

Adding a surface means adding a fixture here, not only a substring assertion. A fixture pays for itself the first time it is the thing that fails.
