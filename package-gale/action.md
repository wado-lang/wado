...
The analog of the parser's `@members` against `Lexer`. Fields become defaulted fields on `Lexer`, methods an `impl Lexer` block; `language = Wado` members pass through the identity translator, `language = Java` reuses the parser member translator, and an untranslatable block is a loud diagnostic. Bare `@members` is the parser's in a combined grammar and the lexer's in a `lexer grammar`, so the scope predicates partition on the grammar kind. Member state persists for the whole tokenize run, so a counter a winning action bumps is visible to later tokens. Everything is gated on the grammar declaring a lexer-members block; a member-free lexer stays byte-identical.

The difficulty is threading `Lexer` to the two sites that touch members: a lexer action runs post-tournament and a predicate runs mid-match, and neither holds a `Lexer` today.

Landed (members-a1): field/method declarations on `Lexer`, read/written by actions through the threaded lexer handle; effects still flow through the existing effect context.

Deferred:

- members-a2 — predicates reading member state (the match fn must take the `Lexer` too), and member methods reading match position / text (`$text`, column) via a match window on `Lexer`. Until then a member method is limited to member-state logic and members are read by actions, not predicates. `language = Java` lexer bodies (java2wado) are a separate prerequisite.
- members-b — fold the whole lexer effect context onto `Lexer` so a member method can issue effects too, retiring the separate effect struct.

`@lexer::members` and `superClass` are siblings: members inline declarations into `Lexer`, while `superClass` generates an external trait the user implements. The real-world lexers (TypeScriptLexer, RustLexer, ANTLRv4Lexer) keep state in a hand-written base and use `superClass`; `@lexer::members`' consumers are the in-grammar counter / position grammars.
...
- [~] Phase 4 — lexer actions / position-sensitive predicates + SuperClass trait (`tokenVocab` falls out).
  ...
  - [ ] Remaining lexer action surface, each with its own prerequisite:
    ...
    - [~] **`@lexer::members`.** See [Lexer members](#lexer-members-lexermembers). Fields/methods on `Lexer` read/written by actions landed (members-a1); predicate-side reads, the member-method position/text API, and the `language = Java` lexer path remain.
