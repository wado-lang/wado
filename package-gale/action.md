...
4. Runtime layer. An action-context API in the generated parser (current token, matched text, input access) that all translated bodies call into. `superClass = Foo` generates a `Foo` effect interface; the user implements it in Wado and installs it with `with Foo => &mut base do { ... }` — this is how the real-world grammars run (requirement 2 applied to requirement 1's gap). See SuperClass — an effect interface.
...
ANTLR's `superClass` is object inheritance: the generated recognizer `extends Foo`, so `{this.m()}` calls a base method that reaches the recognizer's own runtime (`_input.LA`, `getText`, `getCharPositionInLine`) through the shared `this`, and base state (TypeScriptLexerBase's brace/template stacks) rides on the same object. Wado has no inheritance, no `dyn`, value semantics, and a reference write-back model — so the two things inheritance fuses (base _state_ and recognizer _runtime_) must become two objects, connected without threading a handle through every generated function.

The Wado-native isomorphism is the **effect system**, not a trait. superClass needs two capabilities: (A) an indirect call into user-provided behavior, and (B) ambient reach to that behavior from deep inside prediction/scan without a threaded parameter. `dyn` supplies only (A); an effect supplies both — the per-effect dispatch global _is_ (B). Using a threaded `trait` (generic `<B: Foo>` or `&mut dyn Foo`) would force (B) back into a cross-cutting parameter on every generated fn and monomorphize the whole recognizer per base type; the effect keeps the hot scan path byte-identical to a non-superClass grammar and pays one `call_ref` only where a predicate/action fires. Because base state (the handler's `self`) and the runtime (`&mut Lexer`/`&mut Parser` passed as the operation argument) are separate objects, the `lx.base.m(lx)` self-aliasing that a field/`dyn`-field model would hit never arises.

`options { superClass = Foo }` generates `interface Foo` in the recognizer module. One operation per distinct `this.method` call site, signature inferred from the site:

- `{this.m()}?` (predicate) → `fn m(lx: &Lexer) -> bool` — the runtime is passed `&`.
- `{this.m();}` (action) → `fn m(lx: &mut Lexer)` — the runtime is passed `&mut`.

Each op receives a `LexerView` — a match-window handle over the char stream plus the token start and the live match cursor, exposing the recognizer runtime a base method reaches through `this`: lookahead (`la`, ANTLR `_input.LA`), matched text (`text`, ANTLR `getText`), and the cursor. It is a static type (`src/runtime/superclass.wado`, inlined only under `superClass`), passed by value so the handler reads it with no `stores` clause of its own. The `&self`/`&mut self` split is the user's impl choice. Every corpus and real-world call is zero-argument beyond that view; a call with extra arguments is a loud error pending an explicit signature source (a user-pre-declared `interface`, open). (The lexer match cursor is a match-fn local, not a field of the generated `Lexer`, which is why the runtime arrives as a `LexerView` built at the call site rather than a bare `&Lexer`.)

Emit is ambient — an action/predicate site lowers to `Foo::m(LexerView { chars, start, pos })`, no handler threaded. The op-carrying `try_<rule>` fns declare `with Foo` plus `stores[...]` (the view literal captures the char-stream ref); `tokenize` declares `with Foo` because it calls them, and the parser entries (`parse` / `parse_<rule>` / `_run_parse_entry`) declare it too because they call `tokenize` (a combined grammar's parser must propagate the lexer's base effect). This is how safety falls out for free: a caller that never installs a handler fails to compile with `missing effect 'Foo'` — the "omitting an impl is a compile error, not a silently-predicate-free parser" guarantee, enforced by the effect checker rather than a bespoke check. No `_with<B: Foo>` generic entry is emitted (and a generic-bounded `with Foo => base` handler install is rejected by the effect checker anyway); the user installs a concrete handler at the call site:

```wado
struct TsBase { brace_depth: i32 }        // base state — a plain user struct
impl TypeScriptLexerBase for TsBase {
    fn is_regex_possible(&self, lx: &Lexer) -> bool { resume self.brace_depth == 0 && lx.la(1) != ('/' as i32) }
    fn process_open_brace(&mut self, lx: &mut Lexer) { self.brace_depth += 1; resume () }
}
...
Cost model (verified on a probe): a handler operation with no post-resume code hits the `resume`→`return` optimization, so it lowers to a dispatch wrapper that does a single `call_ref` through the effect's funcref vtable — no Wasm stack switching / JSPI. The dominant per-character scan path emits no effect call and stays byte-identical to a non-superClass grammar; only the (rare) predicate/action sites pay the indirect call.

Gating: a non-superClass grammar emits no `interface`, no `with Foo` on any signature, and no `Foo::` call — byte-identical output.

ATN purity holds: a superClass predicate's `pred_<id>` carries `with Foo` and is pre-evaluated at the decision (in the caller, where the handler is ambient) via the existing exclusion path, so the simulator body stays a pure function of the token stream.

Lifecycle overrides: a base may not only add helpers but _override_ recognizer methods (`nextToken`/`emit`/`reset`, e.g. to track `lastToken`). These map to fixed hook operations the recognizer calls at the right point (`Foo::on_token_emitted(tok, lx)`); the exact hook set is enumerated on real-grammar demand, not designed up front.

`tokenVocab` falls out separately and independently of the effect machinery: another grammar's generated `TokenKind` constants are imported by name.
...
- `Lexer` is the single lexer context (mirroring `Parser`): it carries the input, `@lexer::members` state, and the action-effect latches, so actions (`&mut Lexer`) and predicates (`&Lexer`) reach everything through one `lx` handle.
...
- [~] Phase 4 — lexer actions / position-sensitive predicates + SuperClass effect interface (`tokenVocab` falls out). See SuperClass — an effect interface.
  - [x] Loud safety net for un-executed lexer actions (`warn_unhandled_lexer_actions`). Mirrors the parser-side net: an `Action`-kind lexer item codegen does not run (nested-in-group / mid-element, multi-alt-rule, `mode`-grammar, or fragment action) raises `UnsupportedAction` instead of vanishing. Predicates never warn (they run at every position). Gated on `emit_actions` (`language = Wado`), so a Java grammar warns nothing. Shared emittability helpers (`lexer_rule_executes_actions` / `lexer_rule_executed_action_ids`) keep the warn and the emit in lockstep. Was a silent-drop bug (a `{ ... }` lexer body compiled to a no-op with no diagnostic).
  - [x] Winner-replay execution for single-alt lexer rules (`language = Wado`, modeless), identity translator. A lexer `{ ... }` action runs once, at the tournament commit, for the _winning_ rule only — never a losing candidate. Codegen folds the action-effect surface onto `Lexer` (`lx`), one `apply_<rule>_actions(lx, start, pos)` per executing rule (body identity-translated: `$text` resolves over the match window, `lx.*` passes through), and a `best_act` tracker set in the winner's tournament block and dispatched at commit (after keyword reclassification, before skip/more/channel). Fully gated on an executed-action rule existing (`executed_action_rule_names`), so every actionless / predicate-only / Java grammar stays byte-identical. `lx.set_type(TK_X)` folds into `best_kind`. Fixture `wado_lex_set_type.g4` (`A : 'a' { lx.set_type(TK_B) }` retypes to `B`), driver `driver_action_lex_set_type_test`.
  - [x] Command-style actions: `lx.set_type(TK_X)` / `lx.set_channel(n)` / `lx.skip()` / `lx.more()`. Each latches on the `Lexer` context and folds into the tournament outcome at commit — `act_type` → `best_kind`, `act_channel` → `best_channel`, `act_more` → `best_more`, `act_skip` → `is_skip` (via a `best_act_skip` latch, since there is no `best_skip` tournament var). An action grammar forces the `more_start` / `best_more` / `best_channel` commit machinery on (`has_more_rules`/`has_channel_rules` OR `has_actions`) so the fold targets always exist even without a matching `-> more`/`-> channel` command; still gated overall on `has_actions`, so non-action grammars stay byte-identical. Driver `driver_action_lex_cmd_test` (`wado_lex_skip_action.g4`: `skip`+`set_channel` hide tokens; `wado_lex_more_action.g4`: `more` folds `ab` into one `B`).
  - [x] Mode ops (`lx.push_mode(MODE_X)` / `lx.pop_mode()` / `lx.set_mode(MODE_X)`) + the `mode`-grammar tournament path. `lexer_grammar_executes_actions` now admits `mode` grammars: `best_act` is wired into the per-mode dispatch winner blocks (mirroring `gen_group_calls`), and `gen_mode_constants` emits a `pub global MODE_<NAME>: i32` per mode so an action names a target mode instead of a bare index. The mode-op fold targets the transition vars — `act_push_mode` → `best_push_mode`, `act_pop_mode` → `best_pop_mode`, `act_set_mode` → `best_set_mode` (its machinery forced on by `has_actions && use_modes`); the fields/methods and fold are gated on `use_modes`, so a modeless action grammar is unchanged. Driver `driver_action_lex_mode_test` (`wado_lex_mode_action.g4`: letters lex as `W` in the default mode, digits as `N` only inside `< … >`, proving push/pop moved the stack).
  - [x] Multi-alt rule actions. A multi-alt executing rule additionally emits a match-only `winning_alt_<rule>(chars, start) -> i32` helper (each alt in its own labeled block, `break`-to-next on failure, `return`-index on match — first-match, mirroring `gen_lexer_alts`); the apply fn calls it and dispatches on `_walt` to run only the winning alt's top-level actions. `lexer_rule_executes_actions` now admits any non-fragment rule. Fixture `wado_lex_multi_alt_action.g4` (`T : 'a' { lx.set_type(TK_AA) } | 'b' { lx.set_type(TK_BB) }` retypes per matched alt), driver `driver_action_lex_multi_alt_test`.
  - Known limitations of the lexer-action replay (edge cases, not yet fixed):
    - An action that retypes a token to a _skip_-rule's kind (`lx.set_type(TK_WS)` where `WS -> skip`) skips the token, because `is_skip` is computed from `best_kind` after the fold. ANTLR's `setType` does not skip; a proper fix computes skip from the winning rule's identity, not the post-fold kind.
    - A lexer action referencing a `$`-attribute other than `$text` panics the generator (matching the pre-existing lexer-predicate path) instead of raising a diagnostic; the g4 parser accepts the grammar, so `wado` aborts. Wiring a diagnostic needs the lexer emit chain to carry `GenContext`.
  ...
  - [ ] Remaining lexer action surface, each with its own prerequisite:
    ...
    - [x] **`@lexer::members`.** Fields/methods land on `Lexer`; actions read/write them and predicates read them, all through the single `Lexer` context. Member methods reading match position/text and the `language = Java` lexer path remain, tracked with the wider lexer `$`-attribute surface and java2wado below.
  ...
  - [~] SuperClass effect interface (lexer, predicate bases — RustLexer runs). `src/superclass.wado`: `superclass_base_name` reads `options { superClass = Foo }`; `collect_lexer_super_ops` scans `this.<method>` sites (predicate → `fn m(view: LexerView) -> bool`, action → `fn m(view: LexerView)`, deduped, kind from `AltAction.kind`); `gen_super_interface` emits `pub interface Foo`; `rewrite_superclass_calls` rewrites every lexer action / predicate body `this.<method>(...)` → `Foo::<method>(LexerView { chars, start, pos })` in place (a `generate()` pre-pass, after op collection, so the identity translator then passes the already-Wado body through — no `base_name` threaded through the lexer emit chain). `LexerView` (the match-window handle: `la` / `text` / cursor) is a static runtime fragment (`src/runtime/superclass.wado`), inlined by `gen_runtime` only under `superClass`. `GenContext` carries `superclass_name` / `superclass_ops`; `gen_lexer` emits the interface and threads `sc_name` so `gen_lexer_match_fn` marks a `try_<rule>` `with Foo, stores[chars]` when its body calls `Foo::` (`lexer_rule_calls_super`), `gen_tokenize_fn` marks `tokenize with Foo`, and `gen_parse_entry` / `gen_run_parse_entry` mark `parse` / `parse_<rule>` / `_run_parse_entry with Foo` (they call `tokenize`). `merge_grammars` keeps only the lexer / combined split's `superClass` (a split parser grammar declares its own base; a last-wins merge would clobber the lexer's). The user writes `impl Foo for Base` (base state on `self`, reads `view.la` / `view.text`) and installs it with `with Foo => &mut base do { tokenize(...) }`; an unhandled effect is a compile error (`missing effect 'Foo'`) — the drop-safe replacement for a silently predicate-free lexer. Emission is gated on a **predicate-only** base (`all_super_ops_predicates`): a base with an action op stays carved out (bodies discarded, byte-identical) until the winner-replay path is wired, so a half-wired interface is never emitted. Every emit site gates on a non-empty `sc_name`, so a non-superClass grammar is byte-identical. Fixtures: synthetic `wado_lex_super.g4` (`{this.enabled()}?` decides a KW/ID tie), driver `driver_action_lex_super_test`; real-world `RustLexer.g4` + `RustParser.g4` tokenize + parse through a hand-written `impl RustLexerBase` (`sof` / `float_literal_possible` / `float_dot_possible` reading `view.la`), driver `driver_cst_rust_test`. Unit tests `superclass_test.wado`. Remaining: action ops (`{this.m();}` — TypeScriptLexer / ANTLRv4Lexer, needs the winner-replay path to emit `Foo::m(view) with Foo, stores`); the parser side (parser-rule superClass predicates `this.NextGT()` in the recursive descent / prediction — RustParser / TypeScriptParser, currently discarded); lifecycle-override hooks (`nextToken` / `emit` for TypeScriptLexerBase's `lastToken`); SuperClass method arguments. `tokenVocab` still independent.
...
- Where Wado actions live: in-grammar via `options { language=Wado }` and the SuperClass effect handler (`impl Foo for Base`) are primary. Sidecar ID→snippet mapping is fragile (positional IDs); keep as escape hatch only?
- SuperClass operations with arguments: signature source (sidecar vs user-pre-declared `interface`).
- SuperClass lifecycle-override hook set: which recognizer methods a base may override (`nextToken`/`emit`/`reset`) become fixed effect operations — enumerated on real-grammar demand.
