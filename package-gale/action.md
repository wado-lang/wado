...
- [~] Phase 2 — predicates in prediction (`SemPredEvalParser` / `SemPredEvalLexer` descriptors are the acceptance suite).
  ...
  - [x] `pred_<id>` effect-free standalone fns + the ATN parser predicate path (context-free predicates; a context-dependent ATN predicate still falls to the inline guard).
    - [x] Context-free gated predicates (alt-initial, `language = Wado`, `$`-free) extract to a standalone `fn pred_<id>(p: &Parser) -> bool { return <body>; }` (`gen_predicate_fns`, registered in `mark_alt_gate_predicates`, keyed by `action_id` on `GenContext.predicate_fns`). The dispatch calls `pred_<id>(p)` instead of inlining the condition — the `&Parser` receiver keeps it read-only, so it is callable from every decision path (the token-led / interchangeable-`Ambiguous` dispatch today, the ATN callback next). Context-dependent gates (`$v` → `vals.v`) stay folded into the dispatch, where `vals` is in scope. Fixtures `wado_pred_la.g4` (`{p.la(1) == TK_NUM}?` reading `p`), plus the existing `wado_pred_{select,direct,single}.g4` now routing through `pred_<id>`.
    - [x] The ATN parser predicate path (context-free predicates). Because `pred_<id>` covers only context-free predicates (stack-independent), the ATN respects them by **exclusion** rather than a mid-closure callback: the caller pre-evaluates each gated alt's predicate and passes the false ones to the simulator, which skips seeding them (they can never win). Simpler than threading a `fn(&P, i32)` callback through the generic simulator, and it mirrors the existing precedence-predicate prune.
      ...
      - [x] Codegen wiring: `collect_gated_ambiguous_alts` also marks context-free predicates in an ATN-routed (`AtEndConflict` + `emits_at_end_conflict_via_atn`) group as gated (so the inline copy / unsupported warning is suppressed), and `emit_atn_disabled_seed` emits their `pred_<id>(p)` pre-evaluation (`if !(pred_<id>(p)) { _atn_disabled.push(<alt>); }`) before the `atn_predict_with_stack` call, passing `&_atn_disabled`. Byte-identical for gate-free ATN grammars (the seed is `""` when no gate). Fixture `wado_atn_pred.g4` (`xt/xr : 'a' 'b' | {true/false}? 'a' 'b' 'c'`, the `AtEndConflict` shape): on `abcc` the ATN picks the longer alt, but `{false}?` excludes it so the shorter alt runs (`abc` → `ab`).
      - Remaining: a context-dependent (`$v`/`$arg`) predicate at an ATN decision still falls to the inline guard (it can gate the owning dispatch but not the seed prune).
- [x] Phase 3 — java2wado for the corpus **parser** subset + members translation, wired into codegen ([`java2wado.md`](./java2wado.md)). A non-superClass `language = Java` grammar's parser actions / predicates / `@members` / ctx-cast LR idiom execute during the parse (`driver_java_{action,members,pred}_test`). Descriptor `[output]` corpus is parse-only today; codegen-and-compare of print-`[output]` / `SemPredEval*` descriptors is a follow-up harness step.
