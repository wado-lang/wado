...
The generator fixes only the runtime argument's mutability (predicate vs action); the `&self`/`&mut self` split is the user's impl choice. The runtime argument is the existing `&Lexer`/`&Parser` context (the same one Wado-written actions already use) — no separate view type. Every corpus and real-world call is zero-argument beyond that context; a call with extra arguments is a loud error pending an explicit signature source (a user-pre-declared `interface`, open).

Emit is ambient — an action/predicate site lowers to `Foo::m(lx)`, no handler threaded. The public entries carry the effect in their signature: `pub fn tokenize(input) -> TokenStream with Foo`, `pub fn parse(input) -> ParseResult with Foo` (per-rule entries likewise). This is how safety falls out for free: a caller that never installs a handler fails to compile with `missing effect 'Foo'` — the "omitting an impl is a compile error, not a silently-predicate-free parser" guarantee, enforced by the effect checker rather than a bespoke check. No `_with<B: Foo>` generic entry is emitted (and a generic-bounded `with Foo => base` handler install is rejected by the effect checker anyway); the user installs a concrete handler at the call site:
...
- [~] Phase 4 — lexer actions / position-sensitive predicates + SuperClass effect interface (`tokenVocab` falls out). See SuperClass — an effect interface.
  ...
  - [~] SuperClass effect interface (lexer, base-state predicates). `src/superclass.wado`: `superclass_base_name` reads `options { superClass = Foo }`; `collect_lexer_super_ops` scans `this.<method>` sites (predicate → `fn m() -> bool`, action → `fn m()`, deduped, kind from `AltAction.kind`); `gen_super_interface` emits `pub interface Foo`; `rewrite_superclass_calls` rewrites every lexer action / predicate body `this.<method>(...)` → `Foo::<method>()` in place (a `generate()` pre-pass, after op collection, so the identity translator then passes the already-Wado body through — no `base_name` threaded through the lexer emit chain). `GenContext` carries `superclass_name` / `superclass_ops`; `gen_lexer` emits the interface and threads `sc_name` so `gen_lexer_match_fn` marks a `try_<rule>` `with Foo` when its body calls `Foo::` (`lexer_rule_calls_super`) and `gen_tokenize_fn` marks `tokenize with Foo`. The user writes `impl Foo for Base` (base state on `self`) and installs it with `with Foo => &mut base do { tokenize(...) }`; an unhandled effect is a compile error (`missing effect 'Foo'`) — the drop-safe replacement for a silently predicate-free lexer. All emit sites gate on a non-empty `sc_name`, so a non-superClass grammar is byte-identical. Fixture `wado_lex_super.g4` (`KW : {this.enabled()}? [a-z]+ | ID : [a-z]+` — the handler's `enabled()` decides the KW/ID tie), driver `driver_action_lex_super_test`; unit tests `superclass_test.wado`. Remaining: ops take no recognizer runtime yet (base-state only — a later layer threads a lexer match-window view for mid-cursor `la`/`$text`); action ops (`{this.m();}`) and the parser side (prediction-time `pred_<id> with Foo`, `parse with Foo`) are unwired (an action op would fail loudly — the replay helper is not yet marked `with Foo`); `language = Java` superClass (java2wado `this.m()` → op) still carved out; SuperClass method arguments and lifecycle-override hooks (`nextToken`/`emit`) open. `tokenVocab` still independent.
