...
- **Actions write fields** (post-tournament, in `apply_<rule>_actions` — _landed_). The apply fn takes `lexer: &mut Lexer` in place of its `chars: &List<char>` param and binds a local `chars = &lexer.chars` only where the body reads it (a multi-alt `winning_alt` call or a `$text` slice), so a single-alt member action with no `$text` leaves no unused local. The replay dispatch calls `apply_<rule>_actions(&mut lexer, start, best_end, &mut lx)`. `lx: &mut LexerActions` stays the effect channel unchanged; `Lexer` and `LexerActions` are distinct objects, so `&mut lexer` and `&mut lx` do not alias. For a `language = Wado` action the member is named through the `lexer` handle verbatim (`lexer.count += 1`) — the identity translator only resolves `$`-refs, so no `this.` rewrite is involved; the `this.field` form is the Java-grammar spelling, translated once the java2wado lexer path lands.
- **Predicates read fields** (mid-match, in the match fn — _deferred, see Staging_). A member-referencing predicate needs the `Lexer` where the match fn runs. Match fns are free fns `match_<rule>(chars, start) -> i32`; the plan gives them a trailing `lexer: &Lexer` (read-only — a predicate is effect-free, which the type system enforces), so `{ … lexer.field … }?` reads member state. The `tokenize` loop already has `lexer` in scope at every `match_<rule>(&lexer.chars, start)` call site, so it can forward `&lexer` there. Because fields persist, a counter a _winning_ action bumped would be visible to a _later_ token's predicate — matching ANTLR, where members persist across `nextToken`.

Byte-identical gating: the extra `&mut Lexer` (and, later, `&Lexer`) params and the `impl Lexer` member block are emitted only under `lexer_grammar_has_members`. A member-free action grammar keeps today's `chars/start/pos/lx` apply-fn shape and its predicate-only match fns.

### Member methods and the runtime API

A member method lands as `impl Lexer` / `&mut self`, so `this.` → `self.` (a Wado member `fn` is spliced verbatim with `self` = `Lexer`; the Java arm's `translate_java_members` already binds `this_bind` to the recognizer handle). Its callable surface:

- **member fields / sibling member methods** — direct `self.*` (works now).
- **position / text reads** (_deferred, members-a2_) — a member method has no `start` / `pos` in scope, so `Lexer` will carry the current match window: `tok_start` / `tok_end` fields, set immediately before the replay (`lexer.tok_start = start; lexer.tok_end = best_end;`). `impl Lexer` methods then read them — `self.text()` slices `self.chars[tok_start..tok_end]`; `self.column()` / `self.token_start_column()` compute the 0-based column from the char stream (the same back-scan-to-newline the runtime already does for `ParseError` line/col). A member-method body's `getText()` / `getCharPositionInLine()` / `_tokenStartCharPositionInLine` map onto these — the same fixed renames the standalone predicate/action path uses, now reachable through `self`. Until this lands a member method is limited to member-state logic.

Carve-out: an **effect command** (`setType` / `skip` / `pushMode` …) issued _from inside a member method_ has no `lx` handle (the method receiver is `Lexer`, not `LexerActions`), so it does not resolve — a plain compile error today, and the runtime-API-on-`Lexer` unification below is what makes it legal. Top-level actions still issue effects through `lx` exactly as today — the restriction is only the (rare) method-issues-effect case.
...
- **members-a1 (landed).** `@lexer::members` fields land on the `Lexer` struct (defaulted, so `Lexer::new` omits them) and `fn` items splice verbatim into an `impl Lexer` block — both from `collect_lexer_members` → `translate_members` (the identity `translate_wado_members` for `language = Wado`; the Java arm reuses `translate_java_members`). A member-referencing **action** reaches the field / method through the `lexer: &mut Lexer` the apply fn now receives, `chars` re-derived as a local only where the body reads it. All gated on `lexer_grammar_has_members`, so a member-free grammar is byte-identical. The scope partition (`is_lexer_members` / `is_parser_members` now taking `GrammarKind`) landed with it. Fixture `wado_lex_members.g4` (a `count` field bumped per match, retyping every 2nd token to prove cross-token persistence), driver `driver_action_lex_members_test`; translator + partition unit tests in `wado_members_test`.
- **members-a2 (deferred).** Predicate-side member reads — threading `lexer: &Lexer` into the match fns so a `{ … lexer.field … }?` reads member state mid-match (per "Threading the `Lexer`" above) — and the member-method position / text API (the `tok_start`/`tok_end` window + `self.text()` / `self.column()` / `self.token_start_column()`) a method needs to call `getText()` / `getCharPositionInLine()`. Until then a member method is limited to member-state logic, and members are read by actions, not predicates. The `language = Java` lexer action / predicate path (java2wado over lexer bodies) is a separate prerequisite for Java-grammar members.
- **members-b (deferred).** The runtime-API-on-`Lexer` unification — member methods issue effects, `LexerActions` retired.
...
- [~] Phase 4 — lexer actions / position-sensitive predicates + SuperClass trait (`tokenVocab` falls out).
  ...
  - [x] Winner-replay execution for single-alt lexer rules (`language = Wado`, modeless), identity translator. A lexer `{ ... }` action runs once, at the tournament commit, for the _winning_ rule only — never a losing candidate. Codegen emits a `LexerActions` context (`lx`), one `apply_<rule>_actions(chars, start, pos, lx)` per executing rule (body identity-translated: `$text` resolves over `chars`/`start`/`pos`, `lx.*` passes through), and a `best_act` tracker set in the winner's tournament block and dispatched at commit (after keyword reclassification, before skip/more/channel). Fully gated on an executed-action rule existing (`executed_action_rule_names`), so every actionless / predicate-only / Java grammar stays byte-identical. `lx.set_type(TK_X)` folds into `best_kind`. Fixture `wado_lex_set_type.g4` (`A : 'a' { lx.set_type(TK_B) }` retypes to `B`), driver `driver_action_lex_set_type_test`.
  - [x] Command-style actions: `lx.set_type(TK_X)` / `lx.set_channel(n)` / `lx.skip()` / `lx.more()`. Each latches on the `LexerActions` context and folds into the tournament outcome at commit — `act_type` → `best_kind`, `act_channel` → `best_channel`, `act_more` → `best_more`, `act_skip` → `is_skip` (via a `best_act_skip` latch, since there is no `best_skip` tournament var). An action grammar forces the `more_start` / `best_more` / `best_channel` commit machinery on (`has_more_rules`/`has_channel_rules` OR `has_actions`) so the fold targets always exist even without a matching `-> more`/`-> channel` command; still gated overall on `has_actions`, so non-action grammars stay byte-identical. Driver `driver_action_lex_cmd_test` (`wado_lex_skip_action.g4`: `skip`+`set_channel` hide tokens; `wado_lex_more_action.g4`: `more` folds `ab` into one `B`).
  ...
  - [ ] Remaining lexer action surface, each with its own prerequisite:
    ...
    - [~] **`@lexer::members`.** Designed in [Lexer members](#lexer-members-lexermembers). Landed (members-a1): fields on `Lexer` + a verbatim `impl Lexer` method block, member-referencing **actions** threaded `&mut Lexer`, the `is_lexer_members` / `is_parser_members` scope partition, all gated on `lexer_grammar_has_members`. Deferred (members-a2): predicate-side member reads (match-fn `&Lexer` threading), the member-method `tok_start`/`tok_end` position/text API, and the `language = Java` lexer path. Reuses the parser member machinery (`collect_*_members` / `translate_members`).
