...
- Attribute resolution, value channel, and Wado (identity-translator) actions, including `@init` / `@after`, print-style actions, and the parser runtime-context API — largely done. `$<label>.text` reads a labeled rule call's own token range, and a label resolves per alternative rather than rule-wide. The general `$ctx` typed-child access and the expected-token / vocabulary / rule-stack accessors remain.
- Predicates in prediction (static dispatch, scan tournament, ATN exclusion; parser and single-pass lexer) — largely done at rule level; a group's alternatives get none of the three, and the ATN-class lexer path and the remaining lexer `$`-attribute surface remain.
- Group-scoped bodies — the one parser path that still drops an action or a predicate. Rule alternatives, rule prequels and LR suffixes all run theirs.
