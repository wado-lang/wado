...
- [~] Phase 4 — lexer actions / position-sensitive predicates + SuperClass trait (`tokenVocab` falls out).
  ...
  - [ ] Remaining lexer action surface: `lx.set_channel` / `skip` / `more` / mode ops (fold into `best_channel` / `is_skip` / `best_more` / mode state), `@lexer::members`, print via a lexer output sink, multi-alt / nested-group / mid-element placement (`ActionPlacement` cursor state), the `mode`-grammar tournament path, the LATN path, and the rest of the lexer `$`-attribute surface (`$type` / `getCharPositionInLine`).
