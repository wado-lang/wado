...
- [~] Phase 4 — lexer actions / position-sensitive predicates + SuperClass trait (`tokenVocab` falls out).
  ...
  - [ ] Remaining lexer action surface, each with its own prerequisite:
    - **Multi-alt / nested-group / mid-element placement.** Needs the match fn to record which alt/position won (it returns only the end position today), so the replay can dispatch on `(rule, alt, before_index)` with the right cursor state (`ActionPlacement`). Single-alt top-level is the current cut.
