...
- [~] Phase 4 — lexer actions / position-sensitive predicates + SuperClass trait (`tokenVocab` falls out).
  ...
  - Known limitations of the lexer-action replay (edge cases, not yet fixed):
    - An action that retypes a token to a *skip*-rule's kind (`lx.set_type(TK_WS)` where `WS -> skip`) skips the token, because `is_skip` is computed from `best_kind` after the fold. ANTLR's `setType` does not skip; a proper fix computes skip from the winning rule's identity, not the post-fold kind.
