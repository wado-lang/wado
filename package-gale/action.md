...
A lexer member's receiver follows from its body: a method that writes no field and calls no mutating member takes `&self`, the rest `&mut self`. That split is what lets a Java lexer see its members at all — a predicate runs inside `try_<rule>(lx: &Lexer, ...)`, where the tournament must not mutate through a losing candidate, so a predicate may reach only the `&self` half and an action either. A predicate naming a mutating member is refused by name.
...
- A repeat is what the replay knows most about, not least: the winner's extent is already decided, so the replay runs the strategy's own scan with actions off to find where the loop stopped, then walks the body to that cursor with actions on, once per iteration. The exploratory lookahead a greedy loop does while matching never reaches an action that way, and the emitters that restructure the sequence around a repeat keep their match-time shape untouched.
- Predicates evaluate mid-match, position-sensitively; a false predicate rejects the candidate. The single-pass emitter does this at every position, including inside a repeat. The ATN-class path does not: the lexer ATN has no predicate transition, so a predicate in an ATN-class rule must be refused at generation time until the blob and the simulator carry one — silently matching without it is the one outcome the design does not allow.
...
- Attribute resolution, value channel, and Wado (identity-translator) actions, including `@init` / `@after`, print-style actions, and the parser runtime-context API — largely done. `$<label>.text` reads a labeled rule call's own token range, `$<label>.ctx` renders that call's node, and a label resolves per alternative rather than rule-wide. The rule-invocation stack and the expected-token set (a generation-time constant, at a rule prequel) are in; the general `$ctx` typed-child access beyond a name the alternative already binds remains.
- Predicates in prediction (static dispatch, scan tournament, ATN exclusion; parser and single-pass lexer) — largely done, in a group's alternatives as in a rule's. The ATN-class lexer path refuses a predicate rather than dropping it, and the remaining lexer `$`-attribute surface remains.
