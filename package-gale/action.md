...
- [~] Phase 2 — predicates in prediction (`SemPredEvalParser` / `SemPredEvalLexer` descriptors are the acceptance suite).
  ...
  - [~] `pred_<id>` effect-free standalone fns + the ATN parser predicate-callback path.
    ...
    - [~] The ATN parser predicate path. Because `pred_<id>` covers only context-free predicates (stack-independent), the ATN respects them by **exclusion** rather than a mid-closure callback: the caller pre-evaluates each gated alt's predicate and passes the false ones to the simulator, which skips seeding them (they can never win). Simpler than threading a `fn(&P, i32)` callback through the generic simulator, and it mirrors the existing precedence-predicate prune.
      ...
      - [ ] Codegen wiring: mark context-free predicates in an ATN-routed group as gated, emit their `pred_<id>(p)` pre-evaluation before the `atn_predict_with_stack` call, and pass the resulting `disabled` list. Needs an ATN-class fixture that carries an alt-initial predicate.
