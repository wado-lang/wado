...
- SuperClass effect interface — done for lexer bases, predicate and action ops alike (RustLexer, TypeScriptLexer and ANTLRv4Lexer all run through hand-written handlers). The lexer command surface for a base op (an operation gets a read-only `LexerView`, so a base cannot set type / skip / more / switch mode), parser-rule superClass predicates, and lifecycle-override hooks remain.
...
- What an action op may do to the lexer: `LexerView` is a read-only match window, but a real base sets the token type, skips, `more`s, and switches mode (ANTLRv4's `LexerAdaptor` drives a whole mode from one). An action op runs where `lx` is in scope, so a mutable view is possible; a predicate op must stay read-only, since it is evaluated for losing candidates too.
