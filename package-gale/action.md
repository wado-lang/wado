...
- [~] Phase 1b — attribute resolution + value channel + Wado actions (identity translator) + `@after` / print-style actions via `p.emit`.
  ...
  - [~] 1b-3 — value channel + attribute-substitution engine.
    ...
    - [~] Rule arguments as `_parse_<rule>` params. A single-alt rule with defaultable arg types (`rule_threads_args`) gets one defaulted parameter per arg (appended last, so the entry and plain `a` callers omit them), stored into its `<Rule>Vals` at entry so `$arg` → `vals.<name>`. Call sites split `a[$i, $j]` on top-level commas and translate each expression against the caller's channel. Fixture `wado_rule_arg.g4`. Multi-alt / LR rule args stay a loud error (they need the `_alt_<n>` / precedence-loop forwarding the value channel already defers).
  ...
  - [~] 1b-4 — runtime context API + prequel timing.
    ...
    - [~] `@init` / `@after` for multi-alt (non-LR) rules. `@after` runs at the end of each alt body (once, for the chosen alt) via `rule_after_for`, reading the alt's own `vals` — correct on inlined, singleton, and `_alt_<n>`-helper dispatch. `@init` runs at the multi-alt inner-fn entry, sharing that fn's `vals`; on an overlap-group value-channel rule (dispatch through `_alt_<n>` helpers that own a fresh `vals`) an `@init` seed would not reach the helper, so that shape is a loud `UnsupportedAction`. LR rules stay deferred. Fixture `wado_multi_prequel.g4`.
