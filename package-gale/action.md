...
- [~] Phase 4 — lexer actions / position-sensitive predicates + SuperClass trait (`tokenVocab` falls out).
  ...
  - [ ] Remaining lexer action surface, each with its own prerequisite:
    - **Mode ops** (`lx.push_mode`/`pop_mode`/`set_mode`) + the `mode`-grammar tournament path. Needs (a) emitted `MODE_<NAME>` constants so an action can name a target mode (today modes are bare indices), and (b) `best_act` wired into the per-mode dispatch winner blocks. `lexer_grammar_executes_actions` gates modes off until both land.
