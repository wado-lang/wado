...
- [~] Phase 1b — attribute resolution + value channel + Wado actions (identity translator) + `@after` / print-style actions via `p.emit`.
  ...
  - [x] LR value channel (the corpus LR-binary shape): `rule_returns_vals` now admits LR rules. `gen_lr_fn` seeds `_lr_acc` from the atom's vals; each continuation binds the accumulated left operand under the stripped leading self-ref's name (`prefix_self_ref.name`, so `$l.v` reads it), gives the level a fresh `vals` the suffix action writes (`$v`), and folds it back (`_lr_acc = vals`) — the trailing self-ref's own `vals` (`$r.v`) comes from its recursive call. Works through both the static and ATN LR loops. LR `@init`/`@after` remain deferred. Fixture `wado_lr_vals.g4` (`l=e op r=e {$v = $l.v op $r.v}`, precedence-correct evaluation).
...
- [~] Phase 2 — predicates in prediction (`SemPredEvalParser` / `SemPredEvalLexer` descriptors are the acceptance suite).
  ...
  - [~] Prediction-time gating: alt-initial predicates choosing which alt to take (static dispatch / scan tournament / ATN), the `SemPredEvalParser` acceptance suite.
    ...
    - [~] Context-dependent predicates ($local/$ret) — an alt-initial predicate referencing the value channel is gateable: `gate_condition_for` translates its body (`$n` → `vals.n`) and folds the condition into the parse-side dispatch (token-led / singleton / interchangeable-`Ambiguous`), where `vals` is in scope. Marking and emission share `gate_condition_for`, so a body the translator cannot emit is simply not gated (it falls to the inline guard + warning). Fixture `wado_ctx_pred.g4` (`@init`-seeded `$n`, token-led). Remaining: `$arg` (blocked on rule-arg threading, Increment B); scan-tournament cross-rule exclusion (a context-dependent predicate cannot gate a scan — the scan side has no `vals` — so a non-interchangeable overlap group with a dependent predicate keeps the tournament); and the ATN predicate-callback path.
  - [~] Lexer semantic predicates (`SemPredEvalLexer`): a single-alt lexer rule's alt-initial (`{cond}? ...`) and trailing (`... {cond}?`) predicates emit as `if !(cond) { return -1; }` guards in the rule's match fn (`gen_lexer_rule_predicates`, gated on `emit_actions`), so a false predicate rejects the rule and the tokenizer tries the next — deciding a longest-match tie (`KW` vs `ID`) or disabling a rule (`{false}?`). The condition is Wado with the match fn's `chars` / `start` / `pos` in scope. Fixture `wado_lex_pred.g4`. Remaining: mid-alt / multi-alt lexer predicates, fragment-inlined predicates, the LATN (ATN-class) lexer path, and lexer `$`-attribute (`$text` / `getCharPositionInLine`) resolution.
  - [ ] `pred_<id>` effect-free standalone fns (currently the condition is inlined at the dispatch); the ATN parser predicate-callback path.
