...
The per-rule emit _decisions_ below the route — plain vs lookahead-aware repeat, first-match vs arm scoring, maximal munch, suffix cutting, fragment inlining — are `lexer_rule_plan`: one tree, taken once per rule, that `gen_lexer` emits from and the dump renders. Neither walks the grammar deciding for itself, so neither can reach a construct the other does not, and tail position is a property of the plan rather than a parameter each function re-derives. A new strategy is a new plan node with two consumers; adding a branch to only one of them does not compile.

The one thing the plan does not hold is a second copy of anything: a scored alternation only peeks what follows it, so its suffix stays a step of the enclosing sequence and `gen_lexer_alt_seq` re-emits those same steps from a `from` index. Planning that suffix separately is what made the dump report it twice — and would let the peek and the commit pick different strategies for the same elements. `lexer_dump_test.wado` still counts the strategies the dump reports against the locals the emitter mints for them (`alts_best_`, `la_win_`, `accept_`, `ng_saved_`), over shapes that force each one; the grammars are action-free on purpose, since an action-carrying rule emits its body twice.
