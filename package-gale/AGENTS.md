...
`tools/rust_inline_paths.wado` is the parser's second consumer. It reports every `crate::` / `super::` path a Rust file writes outside a `use` item or a `pub(in …)`, which is this repository's own Rust rule (`../AGENTS.md` > General Rules), and `../scripts/check-rust-paths.sh` drives it. It needs the same clean corpus, since a file it cannot parse is reported rather than counted. Both tools share `tools/rust_corpus.wado`, which holds the recognizer with its base ports installed.
