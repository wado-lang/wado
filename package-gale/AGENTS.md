...
A `superClass` grammar has no behaviour without its hand-written base class, so `antlr4-oracle.sh` refuses to guess one. Pass `--super tests/grammars/java/<Base>.java` — each of those is the Java twin of a Wado `impl` in the matching driver test, so ANTLR4 and Gale run the same specification and keeping the pair in sync is what makes the comparison mean anything. That is the only way to oracle such a grammar: `--probe-super` synthesizes a base from the grammar's own `this.<name>(...)` call sites and reports which of them the input reached, but it writes to stderr only and always exits 3, because a base class also reaches the recognizer through overrides of inherited methods the grammar never names (`ANTLRv4Lexer` declares `TOKEN_REF` in `tokens {}` with no rule producing it — `LexerAdaptor` assigns it from an `emit()` override, and the stub answers `ID` for an input that reaches no `LexerAdaptor` member at all). Use the probe to size the work of writing the base, never to pin. `scripts/antlr4-oracle-selftest.sh` pins both paths (needs java; run it after touching the oracle).
