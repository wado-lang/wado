...
Each corpus file carries up to `DESCRIPTORS_PER_FILE` descriptors, each importing its grammar as `t_<Name>`. Grouping is what the corpus's compile time is bound by: every entry module is a whole-program `-O3` build, so the shared Gale runtime is paid for once per file rather than once per descriptor.
