...
Compatibility is a **capability** contract, not byte-for-byte output equality: parse trees, tokens, and semantics must match; incidental rendering differences that carry no structural meaning may diverge. Canonical case — the `<EOF>` marker in `toStringTree()` output; see `antlr4-compatibility.md` ("EOF in parse trees").
