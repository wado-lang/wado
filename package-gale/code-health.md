...
- [ ] Magic numbers, remaining: `TK_LIT_` prefix length `7` and the stream chunk sizes 8192/4096/64 (read-buffer sizes in scripts / `codegen.wado`). Named so far: `MAX_LOOKAHEAD_DEPTH` (5), `MAX_CONFIG_SET` (200), `BROAD_FIRST_SET_THRESHOLD` (20), `KEYWORD_LEN_SENTINEL` (9999), `ATN_MERGE_CACHE_STRIDE` (1000000, now documented against `ATN_ARENA_CAP`), and the pre-existing `MAX_SHAPE_OPTIONALS` (8).
...
- [ ] Performance papercuts (non-blocking), remaining: `preceded_by_prequel_keyword` re-collecting the whole output per `{` (`src/g4/action_strip.wado` — needs a caller-side suffix tracker in `strip_action_bodies`); redundant `clone_dispatch` under value semantics (`src/lower.wado`) and the manual deep-copy loops in `prediction.wado` (both semantics-sensitive — verify they are truly redundant before touching). Done: `line_col_at` O(n²) rescan replaced by a precomputed `line_starts` + binary search (`line_col_from_starts`) on the token-dump path; `compute_alt_gc_starts`/`case_names_for_rule` hoisted to per-rule via `lower_alt_body` params; `break` added to the membership-check found-flag loops in `lexer_gen.wado`.
