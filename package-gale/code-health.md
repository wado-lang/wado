...
- [ ] Names that lie, remaining: `compute_first_chars` builds a full `TryCall`; `skip_rule_prequel` returns harvested options; `rep_recovery_call` calls nothing; `main_test.wado` tests the g4 parser, not `main.wado`. Done: `take_diagnostics` → `copy_diagnostics`, `contains_upper` → `contains_substr`, `iter_follow_unused` → `empty_iter_follow`.
- [ ] Retired vocabulary still load-bearing: `_bt_` infix on helper names (backtracking is gone; the alt's twin helpers are `{fn}_bt_{i}` vs `{fn}_scan_{i}`); `k_prefix_match` vs `k_match_scan` for the same runtime gate, with comments still describing the retired K-prefix mask mechanism. `src/parser_gen.wado:457`, `:1443-1452`, `:3938-3941` — these are *emitted* helper names, so renaming requires a full regen.
