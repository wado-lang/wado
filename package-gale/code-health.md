...
- [ ] Performance papercuts (non-blocking), remaining: O(n²) `line_col_at` rescans (`src/runtime/tools.wado:143-156` — runtime fragment, batched into the regen pass); `preceded_by_prequel_keyword` re-collecting the whole output per `{` (`src/g4/action_strip.wado` — needs a caller-side suffix tracker in `strip_action_bodies`); redundant `clone_dispatch` under value semantics (`src/lower.wado`) and the manual deep-copy loops in `prediction.wado` (both semantics-sensitive — verify they are truly redundant before touching). Done: `compute_alt_gc_starts`/`case_names_for_rule` hoisted to per-rule via `lower_alt_body` params; `break` added to the membership-check found-flag loops in `lexer_gen.wado`.
