...
Status: **Phase 3 landed.** The translator (`src/java2wado.wado` + `src/java2wado/{jlexer,jparser,jemit,jmembers}.wado`) covers the corpus parser subset — expressions, statements, method mapping, predicates, the ctx-cast LR idiom, and `@members` — and it is wired into codegen: a non-superClass `language = Java` grammar's parser actions / predicates / members now execute during the parse. End-to-end driver fixtures (`driver_java_{action,members,pred}_test`) prove print actions, `@members` field+method, and member-field predicate gating. Remaining: **lexer** actions / position-sensitive lexer predicates + superClass grammars are **Phase 4**; wiring the ANTLR descriptor `[output]` corpus to codegen-and-run (rather than parse-only) is a **follow-up** acceptance step.
...
- [x] **3.v — ctx-cast LR-binary.** `JCast` + cast parsing (`(Type) operand`, discarded — Gale's CST is untyped); the ctx-child accessor `((Ctx)$ctx).<rule>(i)` maps to the LR operand vals — child `0` → the accumulator `LR_ACC_LOCAL` (`_lr_acc`, the left operand at action time), child `i>=1` → the i-th suffix self-ref's post-dedup binding (`rule_binding_for`). Guarded to a left-recursive rule's own children; else a loud error. `((BinaryContext)$ctx).e(0).v + ((BinaryContext)$ctx).e(1).v` → `_lr_acc.v + e.v`. Deferred (same `MultipleAlternativesWithCommonLabel` grammar): the token accessor `((UnaryContext)$ctx).INC()` and the `assert ... != null` it feeds (no i32 null in Wado; token always present → tautology).
- [x] **3.vi — members.** `translate_members` (facade) → `src/java2wado/jmembers.wado`: parses the `@members` class body — field decls → `MemberField {name, ty, init}` (struct fields), method decls → ready-to-splice `fn ... { ... }` sources. Method bodies reuse the statement translator with `this_bind = "self"` (the receiver, so no alias line), over a synthetic empty rule (member bodies carry no `$`-refs). Added the `return` statement (jparser/jemit) and exposed `emit_java_stmts` / `emit_java_expr` / `map_java_type` for the members path. `boolean pred(boolean v) { System.out.println("eval=" + v); return v; }` → `fn pred(&mut self, v: bool) -> bool { self.emit(`eval={v}\n`); return v; }`. Field names stay verbatim (`enumKeyword`) to match `this.enumKeyword`. Deferred: `ContextListGetters`' `SContext s = null; s.a();` (context-typed local + `null`). Codegen wiring into the struct/impl is 3.vii.
- [x] **3.vii — codegen wiring + flip.** (a) `GenContext.action_language` + the three parser_gen splice sites routed through `translate_predicate` / `translate_action`; the ref-free passthrough short-circuits only for Wado. (c) `action_language_is_emittable` accepts non-superClass Java (moved to `java2wado.wado`, shared); the ctx-tree `CTX_TOKEN_NAMES` table is gated on emittability, not Wado-only. (d) `collect_parser_members` → `@members` fields become defaulted `Parser` fields (`FieldSpec` gained a default), methods a separate `impl Parser` block. The recognizer handle is `p` (the parse-function parameter; `this` was dropped — a global rename to `this` was too pervasive for a cosmetic gain). Driver fixtures: `driver_java_{action,members,pred}_test`. **Not done here (Phase 4):** lexer actions / position-sensitive lexer predicates + superClass. **Follow-up:** the descriptor corpus is parse-only today; making print-`[output]` / `SemPredEval*` descriptors codegen-and-compare is a separate harness step.
...
- **Recognizer handle = `p`** at the codegen splice (the parse-function parameter, always in scope, warning-free, consistent with the surrounding generated code). The translator is handle-parameterized (attribute engine + `this_bind`), so the choice is codegen's; a literal `this` would have needed a pervasive `p`→`this` rename of the whole generated parser for a cosmetic gain, so it was dropped. `@members` method bodies use `self` (the method receiver).
