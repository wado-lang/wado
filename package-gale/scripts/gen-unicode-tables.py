...
def main():
    ...
    out.append("//!")
    out.append(f"//! Do not edit by hand — generated from the Unicode Character Database")
    out.append(f"//! version {version} by `scripts/gen-unicode-tables.py`.")
    out.append("//!")
    out.append("//! Each entry is a category's ranges as `START[-END]` in hex, comma")
    out.append("//! separated. `\\P` complements what `\\p` selects, so an approximate table")
    out.append("//! would admit real members of the property, not merely miss some.")
    out.append("")
