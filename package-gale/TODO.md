...
- [ ] The `sqlite` Stage B′ `#[TODO]`s are independent divergences, each worth its own fixture: `column_def` skips `type_name?` where ANTLR4 enters it (`a NOT NULL`, `a CONSTRAINT c NOT NULL`); `type_name`'s non-greedy `name+?` stops after one name, rejecting `a UNSIGNED BIG INT`; the `BETWEEN` operands climb from too high a precedence, so `a NOT BETWEEN 1 AND 2 AND b` brackets the wrong way (a silent mis-parse, no diagnostic); `IN`'s bare `table_name` branch (`a IN t`) is rejected; and so are the `sql_stmt_list` `';'` separators.
