...
1. **Soundness and compatibility divergence** — these mis-parse valid input, so they outrank every feature below. One entry, blocked on ICU.
2. **A descriptor re-extract** whenever a JDK and the `vendor/antlr4` submodule are at hand — it is what proves an entry is still blocked rather than merely old. Last run 2026-08-24, all three phases, 98/98 oracle invocations clean: the corpus regenerated byte-identically.
3. Everything else, in whatever order a live case surfaces it.
...
- **Host-side output (Stage C).** Every composite descriptor's expected output is a host-side artefact — action prints, token dumps, or empty — so none survive the Stage B output normalizer. Re-evaluate once Stage C lands.
=======
- **Importer multi-input plumbing.** A grammar import (`import S;`) must resolve against the sibling slave-grammar files. Kiln already supports multi-input; lift the short-circuit once resolution lands. Actionable on its own.
