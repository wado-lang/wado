...
The 33 are not a gap: each carries neither `[output]` nor `[errors]`, so the descriptor holds nothing further to compare. Of the 48, 17 are composite (below), 10 are `<DumpDFA()>` ATN traces and 3 more are StringTemplate directives — all out of scope for the reasons above. The rest is real, and each entry below is a rule the extractor applies, not a per-descriptor accident:

- [ ] **An empty `[input]` is read as "nothing to feed the parser".** Ten descriptors lose their test to that one conjunct — three Lexer ones whose `[output]` is a real token dump for the empty string (`LexerExec/EOFByItself`, `EOFSuffixInFirstRule_1`, `EscapeTargetStringLiteral`), five `ParserExec` ones with a non-empty expected print (`AStar_1`, `AorAStar_1`, `AorBStar_1`, `LL1OptionalBlock_1`, `ReferenceToATN_1`), and two `ParserErrors` ones. The empty string is an input; the guard belongs on `[output]` alone. Two sites: the `!parsed.input.is_empty()` conjunct in each Stage A eligibility rule, and the shared `continue` gating Stage B / B′ / C.

- [ ] **An error descriptor's `[output]` is never compared** — 15 of the 37 `ParserErrors` / `LexerErrors` descriptors carry one alongside `[errors]` (`ParserErrors/ExtraneousInput`, `LexerErrors/DFAToATNThatFailsBackToDFA`, …), and claim (c) asserts only that the parse fails. The output is what error _recovery_ produced, which is where Gale and ANTLR4 are known to differ: Gale emits `TK_ERROR` tokens where ANTLR4 skips, and its trees carry the `<skip …>` markers `strip_skip_markers` already normalises for Stage B / C. **Design judgement:** decide what the comparison means before writing it — either the recovery output is part of the compatibility contract (then these are 15 pins and some become `[stage_a_todo]`), or it is not (then say so here and the claim stays as it is). Do not pin it to whatever Gale currently prints.
