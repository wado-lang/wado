...
- The SuperClass effect interface for the real-world grammars below. Landed for **predicate-only** lexer bases, including `language = Java`: `RustLexer` tokenizes and parses end to end through a hand-written `impl RustLexerBase` (`options { superClass = Foo }` → `pub interface Foo`, `this.m()` → ambient `Foo::m(LexerView { .. })`, `LexerView` a static runtime fragment exposing `la` / `text` / cursor, `try_<rule>` / `tokenize` / `parse` carry `with Foo`, user installs `impl Foo for Base` via `with Foo => &mut base do`). See `action.md` (Phase 4, "SuperClass effect interface") and driver `driver_cst_rust_test`. Remaining before TypeScript / ANTLRv4 run: action ops (`{this.m();}` — the winner-replay path), the parser side (parser-rule superClass predicates like `this.NextGT()`, currently discarded), and lifecycle hooks (`nextToken` for `lastToken`). Action-op bases stay carved out (byte-identical) until the replay path lands.
