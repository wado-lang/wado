...
2. **Exact prediction context (return-state stack), not rule indices.** The decision hinges on the _exact_ enclosing return state ("this `expr` is `between`'s first operand, after which the caller expects literal `'and'`"). The simulator's during-prediction `AtnConfig.stack` is already exact return states; the gap is the **caller** stack `p.atn_stack`, which the rule-entry wrapper fills with rule **indices** (`atn_call_return_states` then over-approximates by matching _every_ call site of the callee). That can't distinguish between's-first-operand from a loop operand. Upgrade: the generated parser pushes the **ATN return-state id** at each call site (not the rule index at rule entry), so the simulator pops to the precise caller continuation and sees between's pending `'and'`.
   ...
   - **P4.2b-ii — runtime rewire (next).** Build the ATN before parser emit and stash `call_site_returns` on `GenContext`; add a `Parser.atn_ret_pending` field a caller sets before each `_parse_X` call (`p.atn_ret_pending = call_site_returns[rc.atn_call_site]`), which `gen_rule_entry_wrapper` pushes onto `p.atn_stack` (now a stack of return states, not rule indices) and pops on exit — error-safe because the push/pop stays in the wrapper. Upgrade `atn_closure` to pop the caller stack to the *exact* return state (`rstk[rs]`), retiring `atn_call_return_states`'s over-approximation; `-1` entries (un-stamped sites) degrade to SLL-accept. Pass the current rule to `atn_ng_optional_enter` as a compile-time constant (the stack top is no longer the current rule index). Validate end-to-end with the non-greedy driver tests (the IfIfElse outer-binding trees); migrate the `atn_sim_test.wado` full-context unit stacks from rule indices to return states. Co-designed with P4.3's precedence eval (both touch `atn_closure` and the caller-stack representation).
...
**Phasing.** P4.1 LR ATN build _(done)_ → P4.2a call-site↔return-state correspondence _(done)_ → P4.2b runtime rewire (push/pop exact return states + simulator pops to them; migrate non-greedy stacks, keep green) → P4.3 precedence eval + loop-entry wiring, flip a small `tests/grammars/lr_between.g4` fixture (`between X1 and X2 and X3`) → P4.4 run the real descriptor, measure, add GSS iff the cap is hit.
