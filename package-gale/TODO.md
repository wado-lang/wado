...
- A left-recursive rule's `@init` / `@after` cannot read `$x.text` / `$x.ctx`: the call is in the atom or a suffix function, the prequel runs in the entry, and the local does not cross. Reported and the body dropped (`prequel_spans_reachable`). Carrying the span on the LR value channel is what a fix needs; no corpus or real-world grammar asks for one.
- The scannable `+` whose body is a group still admits an iteration silently: its first (mandatory) iteration shares one emit with the guarded later ones, so it is emitted as not-required and a token no alternative admits appends nothing. Every other required-group position now reports (`gen_group_no_viable_else`). Splitting that emit in two is what a fix needs.
