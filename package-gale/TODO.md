...
- ~~**Set list-label `val+=(INT | FLOAT)*` panics inside Gale** — `ParserExec/ListLabelsOnSet`.~~ Fixed: `gen_op_inner_for_storable_repeat` now strips the `Label(Group)` wrapper before forwarding the surface element to `gen_op_storable_group_inner`, mirroring the unwrap `lower_repeat_op`'s list-label branch already performs on the body op. The generated parser allocates `Array<Token>` and consumes each iteration via `p.match_set([TK_INT, TK_FLOAT])` (the existing storable-Group dispatcher's normal output once it actually sees a Group). Regression test in `src/codegen_test.wado`.
- ~~**Non-greedy optional `??` routed through `*`-style codegen** — `ParserExec/IfIfElseNonGreedyBinding1`.~~ Partially fixed: emit shape is now `Option<T>` plus a single `if` check (matching the lower-side `FieldKind::Option` decl), so the test compiles. `lower_repeat_op` dispatches non-greedy `Optional` through the greedy strategy pipeline; `gen_op_repeat_with_follow_override` routes the same case to the greedy Optional emitter. Regression test in `src/codegen_test.wado` (`non-greedy optional (??) emits an Option<T> field, not Array<T>`). Remaining (`[stage_a_todo]`): the greedy emitter applies first-set prediction, not the prefer-skip semantics ANTLR4's `??` requires — the descriptor's expected parse tree binds the dangling `else` to the _outer_ if, but Gale still binds it to the _inner_ if. Closing this needs either a follow-guarded Optional dispatcher or runtime ATN simulation.
