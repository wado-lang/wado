...
- [ ] **`import Foo = Bar;` is a loud error and should not stay one.** ANTLR4 accepts the aliased form, so rejecting it breaks Stage A claim (a) for a grammar upstream compiles, which is why claim (a) now carves it out. Composition itself needs nothing new, since the alias binds the same delegate `import Bar;` would. What is missing is the surface the alias exists for, ANTLR's `gFoo.rule` qualified references inside action bodies, and an oracle answer for the cases the corpus does not cover: two aliases of one grammar, and an alias shadowing another supplied grammar's declared name. Landing the binding without the qualified references would accept the form and silently drop what it means.
- [ ] **An embedded delegate cannot also be used on its own.** Nesting a language needs the delegate's lexer rules in a mode (`example/MiniCss.g4`), and which mode a rule sits in is decided by the file that declares it. So a delegate written to be embedded starts standalone in an empty `DEFAULT_MODE` and lexes nothing, and one written to stand alone gets swallowed by the host's catch-all rule when composed. Closing it means the host saying where a delegate's rules land (`import MiniCss in mode CSS;` or equivalent), after which the delegate stays `DEFAULT_MODE` and works both ways. `rebase_lexer_rules` already remaps a delegate's modes, so the composition side is one extra argument. The cost is `.g4` syntax ANTLR4 has no counterpart for, which makes it a compatibility decision rather than an implementation one.
- [ ] **A composite holds one action language; a delegate cannot bring its own.** `options { language = X }` is read per grammar into `Grammar.action_language`, but a composite is one `Grammar`, so the master's choice governs every body and a delegate written in another language is fed to the wrong translator. That fails loudly rather than silently, since java2wado refuses a Wado body, but it stops a Java grammar importing a Wado one, which is the natural way to extend a vendored grammar. Closing it means the language travelling with each retained action, predicate and rule signature rather than sitting grammar-wide, and the translator being chosen per body. `action.md` ("IR retention") already records a language tag per site; only the selection is grammar-wide.
...
`wado-compiler`: **327 files, 303 with diagnostics** at `e925ee58bf`, **33** now. Closed so far: the caller's continuation stopping at the enclosing group (`useTree`), a greedy loop eating its alternative's mandatory suffix (`matchArms`), a nullable alternative at a lookahead nothing selects (`structExprFields`), a recursive non-greedy fragment (`RAW_STRING_LITERAL`), a predicate-carrying alternation scored by first match rather than length (`FLOAT_LITERAL`), a semantic predicate that could not gate a scan tournament's alternative, a parser `superClass` that carved actions out of its own grammar, and the grammar being several Rust versions behind (let-else, let chains, async closures, `union`, `&&T`, and a string catch-all admitting the backslash).

Split the 33 into top-level items and those into methods and 41 methods fail. **34 of the 41 are the one gap below**, in its two faces (`macroItem` where recovery lands at an item boundary, `else` where it lands inside the block). The other 7 are singletons: `&raw const t` (Rust 1.82's raw borrow — `raw` lexes as an ordinary identifier, so the parser grammar has no literal to match and it needs a lexer rule or a base predicate), an attribute on a block's trailing expression (`{ #[allow(…)] f() }` — adding `outerAttribute*` to `statements` costs more than it buys, 33 files to 35, because it gives `statement` and the tail the same head), and five that have not been minimised.

Rust's "no struct literal in a condition" is now in the grammar, as the flag an ANTLR4 grammar has to use (`@parser::members`, mid-rule assignments — `++` is refused by java2wado — and `{noStruct == 0}?` on the two brace-carrying alternatives of `expression`). One gale gap is left, and it is what the rest of the corpus waits on.

- [ ] **A scan cannot evaluate a semantic predicate, so a gate the parse honours the scan still ignores.** With the gate above in place, `y = if v { a } else { b };` does not parse: `statements` chooses between `statement+ expression?` and a lone `expression` by scan length, `scan_statement` measures the `if` head's condition as the struct literal it is no longer allowed to be, fails, and the lone-expression alternative wins on `y` alone. `let y = if v { a } else { b };` parses, because no scan decides there.

  A `scan_*` fn is `(tokens, pos, follow) -> i32` and pure by design, which is what makes it usable speculatively. Threading the recognizer into every scan so the gated alternative can ask does not close it, and was tried: a scan fn carrying `p: &Parser`, the gate hoisted into the alternative's own helper, `ScanAltPlan` carrying the projection from plan-local to rule-local alt numbering so the lookup lands on the right alternative. It emits correctly — `scan_expression_atom_alt_14` opens with `if !((p.no_struct == 0)) { return -1; }` — and changes nothing, because the flag is **parse-time state and a scan is lookahead**. When `statements` scans `statement+`, the parser has not entered the `if` yet, so the flag reads 0 and the scan takes the struct reading anyway.

  So the scan would have to _simulate_ the flag as it walks — apply the rule's mid-rule actions to a scan-local value and thread that through its own callees. That is ALL(\*) predicate simulation, which is the ATN, not a parameter. Route the decision there rather than growing the scan signature.

  A caller-FOLLOW tier on the tournament does not substitute, even scoped to a dispatch whose alternatives span the whole rule (where the alternative's scan end really is the rule's end, so the question is the right one — `statements`' lone `expression` stops on `y` and the `}` its caller owes cannot start at `=`). It emits, and changes nothing: at that decision _neither_ alternative is viable, because the other one's scan failed on this same gate. Every route back to the corpus runs through the scan seeing the predicate.

  The flag it reads is deliberately a flag rather than a depth: clearing on the way into a bracket never restores, so the ban does not resume after one inside the same head. That accepts more than Rust does and never less, which is the safe direction; a depth would need a stack per frame, which a members block has no room for.

The rest of the corpus is a handful of singletons behind the same scan gap — reach them by splitting a failing file into its top-level items and those into methods, which is what turns a recovery landing into the construct that actually failed.
