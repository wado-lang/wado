...
- **P3** — wire prediction into codegen at `Ambiguous` sites and feed the simulator the **real** rule-return stack. **Refinement discovered in P2:** IfIfElse is NOT SLL-decidable with an empty stack. With an empty (SLL) stack the exit-config reaches rule-stop and matches any follow (wildcard), so non-greedy would skip the `else` at the OUTER `ifStatement` too and the parse fails. ANTLR4-correct binding (`else` → outer) needs the real follow: at the outermost decision the real FOLLOW excludes `else`, killing the exit alt so enter wins; at the inner decision the real follow (through the outer `??`) includes `else`, so exit wins on the min-alt tie-break. Therefore P3 must:
  1. Build the ATN at codegen, serialize it to a `global` blob, decode once into a `Parser.atn: AtnSim` field, and add `Parser.atn_stack: List<i32>` (the rule-return ATN states), gated like `emit_follow` so gate-free grammars carry nothing.
  2. At each `RuleRef` call site on a path to an ATN-class decision, push the call's follow ATN state before the call and pop after (maintains the real context cheaply — one i32 per frame, ATN-class grammars only).
  3. Replace the `Ambiguous` longest-match tournament with `atn_predict_with_stack(&p.atn, DECISION_<id>, &p.atn_stack, &p.tokens, p.pos, TK_EOF)` and dispatch to the returned alt.
  4. Map each `Ambiguous` GIR decision to its ATN decision number (lower/parser_gen) so the right blob decision is queried.
     Flips `[stage_b_oracle_todo] IfIfElseNonGreedyBinding1/2`. Add the per-`Parser` DFA cache here once correct (keyed by decision + token-prefix).
