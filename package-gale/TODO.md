...
- [ ] **A struct literal wins the head of an `if` / `while` / `for`.** `if any_error { None }` reads `any_error { None }` as a struct expression, and `for id in b { … }` reads `b { … }` as one, because the alternative tournament takes the longest and nothing tells it the caller still owes a block. This is now essentially the whole remainder: splitting the 24 files that report `externBlock` into their top-level items leaves 33 failing items, and **32 of the 33 are this**.
  ...
  Rust's rule is the depth counter an ANTLR4 grammar has to use, and gale takes each piece on its own: `@parser::members { int noStruct = 0; }`, the mid-rule `{noStruct = noStruct + 1;}` (not `++`, which java2wado refuses), and `{noStruct == 0}?` on an alternative. Two gale gaps sit between that and a working grammar, and they must be closed in order.

- [ ] **A parser `superClass` and parser actions cannot coexist.** Adding any `@parser::members` / action body to `RustParser.g4` makes gale report `superClass 'RustParserBase' is called from a body carrying host code the grammar's action language has no translator for, so the base is not wired`; the generated module then stops exporting `RustParserBase`, and `shl` / `shr` lose the predicates they are built on. It is not the `this.` form — a bare member reference reports the same. Reproduce by adding an empty `@parser::members` block to any `superClass` grammar.

  ANTLR4 compiles members and a `superClass` together, so this is the compatibility bug the guide's first rule names. The fix is measured and small: `action_language_is_emittable` carves out `Java` + `superClass` for the _lexer_, whose base owns the action surface too — the parser's base is predicate-only, and the rewrite leaves `Base::m(p)` calls java2wado passes through. Splitting the question (a `parser_action_language_is_emittable` read by `collect_members`'s non-lexer half and by `parser_bodies_execute`) keeps `RustParserBase` wired with no java2wado warnings. Dropping the carve-out for _both_ recognizers does not work: it breaks `driver_action_lex_super_action_java`, `driver_cst_antlr4`'s mode constants, and TypeScript's regex-vs-division base predicate. Two tests pin the current contract deliberately and would move with it — `parser_super_test`'s "a base call sharing its body with untranslatable host code is refused" and "a wired parser base does not un-carve java members". The first is already what `action.md` calls warn-and-emit: report the one body, keep the base.

- [ ] **A semantic predicate on an LR atom alternative does not gate it.** With the above worked around, `{noStruct == 0}? structExpression` inside `expression` is ignored: the alternative tournament picks by scan length, and a scan is a pure `(tokens, pos, follow)` function that cannot evaluate a parser predicate. Putting the predicate on `structExprStruct` instead is worse — the scan still picks the struct reading and the parse then dies on `failed semantic predicate` rather than falling back. Gale's predicate-gated dispatch (`_atn_disabled`) covers a direct alternation, not an LR atom reached through a scan tournament; the same limitation `rule_body_decision` states as "AtEndConflict routing is non-LR only". This is what actually has to be built, and it is the same machinery the ATN route would need.
