...
- **User token name collides with Gale's internal sentinel** — `Performance/ExpressionGrammar_{1,2}`. A user `ERROR : .;` lexer rule emits `global TK_ERROR: i32 = …;` a second time, on top of Gale's built-in `TK_ERROR` sentinel. `wado compile` rejects with `duplicate definition 'TK_ERROR'`. Renaming the user rule eliminates the bug — the "all-binary-op LR rewrite" path the old triage blamed is in fact clean. Min repro: [`tests/grammars/lexer_user_error_collides_with_sentinel.g4`](./tests/grammars/lexer_user_error_collides_with_sentinel.g4). Fix in `codegen.wado`: prefix or otherwise namespace the internal sentinel kinds (`TK_ERROR`, `TK_EOF`) so they cannot clash with user-named tokens.
- **Parser rule references an implicit (lexer-undefined) token** — `LeftRecursion/WhitespaceInfluence_{1,2}`. The grammars reference `K_IN` / `K_AND` / `K_OR` / `K_TRUE` / `K_FALSE` / `K_NULL` from parser rules without defining them as lexer rules. ANTLR4 synthesizes implicit lexer rules for that shape (see `vendor/antlr4/doc/lexer-rules.md`); Gale's post-merge `check_references` rejects with `reference to undefined token "K_IN"`. Stripping the K_* refs (or supplying explicit lexer rules) confirms the all-binary-op LR rewrite path is otherwise clean. Min repro: [`tests/grammars/parser_rule_refs_implicit_token.g4`](./tests/grammars/parser_rule_refs_implicit_token.g4). Fix in `ir.wado` (`check_references` / `merge_grammars`): synthesize an implicit lexer rule for every upper-case parser-side reference with no defining rule.
- **List-label `b2+=b*` shadows the container with the per-iteration value** — `ParserExec/Labels`. Generated body is `let mut b2: Array<BNode> = []; … let b2 = _parse_b(p)?; b2.push(b2);` — the `let b2 = _parse_b(…)?` shadows the array and the push lands on a scalar (`no method 'push' found on type 'BNode'`). Literal labels like `b3+=';'` work because Gale derives a distinct temp name (`semicolon`) from the literal; only RuleRef labels collide. Fixture: existing `tests/antlr4-compat/grammars/ParserExec/Labels.g4` (6 lines, already minimal). Fix in `parser_gen.wado`: rename the per-iteration temp inside list-label loops so it never matches the label name.
...
- **Non-greedy optional `??` routed through `*`-style codegen** — `ParserExec/IfIfElseNonGreedyBinding1`. For `('else' statement)??` Gale emits `let mut lit_else_statement: Array<…> = []; while p.peek_kind() == TK_LIT_ELSE { … let lit_else_statement = …{…}; lit_else_statement.push(lit_else_statement); }`. Two bugs overlap: (a) `??` is routed through the `*` (zero-or-more) emitter instead of the `?` (optional) one, so the field's type is `Array<T>` instead of `Option<T>`, and (b) the per-iteration value shadows the container (same root as `Labels`). Fixture: existing `tests/antlr4-compat/grammars/ParserExec/IfIfElseNonGreedyBinding1.g4` (8 lines). Fix in `parser_gen.wado`: dispatch `??` through the optional emitter (returning `Option<T>` rather than `Array<T>`), and apply the temp-rename from the `Labels` fix.
- **Wado-reserved-word rule names + list-label collision** — `ParserExec/ReservedWordsEscaping`. Inside `if kind == TK_IF { let mut if_: Array<Token> = []; let if_ = p.expect(TK_IF)?; if_.push(if_); … }` — the `let if_ = …` shadows the list-label array (same Labels bug) and the rule rename `if → if_` collides with that new binding too. Fixture: existing `tests/antlr4-compat/grammars/ParserExec/ReservedWordsEscaping.g4` (22 lines). Fix: lands automatically once the `Labels` per-iteration temp-rename is in place; verify with this fixture as the broader regression case.
- **`type(N)` lexer command leaves `TK_<N>` undeclared** — `LexerExec/TokenType0xFFFF`. Grammar is `T_FFFF: 'FFFF' -> type(65535);`. Gale emits `best_kind = TK_65535;` but never declares `global TK_65535: i32 = …;`, so `wado compile` rejects with `unknown identifier 'TK_65535'`. Fixture: existing `tests/antlr4-compat/grammars/LexerExec/TokenType0xFFFF.g4` (2 lines). Fix in `codegen.wado`: when a lexer command is `type(N)`, register `TK_<N>` in the token-id table alongside the rule's own kind.
