...
- [ ] A non-greedy loop at a rule's tail still takes the minimum when the caller's continuation is nullable: `type_name`'s `name+?` stops one name early on `a UNSIGNED BIG INT` and `a CONSTRAINT c NOT NULL DEFAULT 1` (two `sqlite` oracle cases, `#[TODO]`).
  ...
  The ambiguity itself is decided: a greedy `rule?` enters when entering stays viable, a non-greedy `*?` / `+?` iterates when exiting would strand the continuation, both by scanning the continuation rather than predicting. What is left is the case with nothing local to scan — at a rule's tail the probe falls back to the runtime FOLLOW mask, and the mask cannot answer this. `column_def`'s suffix after `type_name?` is the nullable `column_constraint*`, so the local continuation and the caller's both start at depth 0 while `FollowArg::Combined` stacks them by depth; the `,` ending the column then reads as "the caller cannot continue" and the loop takes another name. Measured: threading the mask that way fixes `a UNSIGNED BIG INT` and breaks `a VARCHAR(10), b DECIMAL(10, 2)`.

  The two uses want different sets. A yield gate must be conservative — hence `compute_call_site_follow` dropping a non-first-exact deep-nullable suffix (soundness invariant 2). A viability probe must be complete, and an under-approximation makes the loop over-consume. Closing this needs a viability set carried separately, with the union-at-depth-0 a nullable local suffix implies.
...
- [x] Scan/parse EOF asymmetry — **not a bug; the asymmetry is what matches ANTLR4.** The parse side matches EOF without advancing while the scan side advances, so a scan counts one more whenever EOF is the last matched element. That extra token is what lets an EOF-reaching alternative win a tournament tie, and winning is the ANTLR4 answer: on `s : u | t ; u : A ; t : A EOF ;` the jar parses `a` as `(s (t a <EOF>))`, not as the lower-numbered `u`. Making the scan stop at EOF was implemented and measured — it produces `(s (u a))` and diverges. Fixture `ll_eof_tie.g4` pins the jar's answer so the "fix" cannot be re-applied silently. The two sides do not have to agree on the integer, only on which alternative wins.
- [x] Tournament / scan-gate call sites now forward the runtime caller-FOLLOW argument, so a scan gate answers on the same continuation its parse call does. Four sites took the callee's defaulted `&EMPTY_FOLLOW` — the LR branch gate, both LR suffix twins (the parse call one line below already forwarded), and the alt dispatch's full and partial scans.
