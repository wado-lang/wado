...
- A rule prequel reading `$x.text` / `$x.ctx` for a call inside a group nested two deep under a shape-lookahead optional finds the local declared inside the shape branch, not outside it. The three top-level shape emitters declare ahead of their branch chain; the inner-group arm declares in its branch, which is where its own alternative's actions read it. Declaring both places would shadow, so a fix wants the surface group threaded to the emitter rather than a second hoist.
- A group whose dispatch takes no alternative appends nothing and reports nothing, where ANTLR reports a no-viable-alternative. Long true for a token outside every alternative's first set; a false alt-initial predicate now reaches it too (`r : ( {false}? A | B ) ;` on `a`). A required group and an optional one share `gen_general_group_store_optional_op`, so telling those apart is what a fix needs first — the repeat-body emit (`gen_general_group_store_op`) already closes its gated chain with `no_viable`.
