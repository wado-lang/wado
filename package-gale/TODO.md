...
4. **Scaling — graph-structured contexts. _(done — exponential killed; O(n) optimization still open)_** The exact per-config `List<i32>` stacks blew up exponentially under deep left recursion (measured ×7 per +2 depth; past depth ~10 the `ATN_MAX_CONFIGS` cap truncated and **mispredicted**). Replaced by a hash-consed `PredictionContext` DAG (`CtxArena`): each node is a set of `[return_state, restore_pr, parent]` edges, merged so the closure keeps **one config per `(state, alt, rs, pr)`** (`CfgSet`). Now polynomial (measured `lr_between.g4` depth 8/16/32 = 1.3s / 7s / 48s — no blow-up, no cap misprediction). The during-prediction recursion is input-driven (an LR operand is reached only after its operator token), so context depth is input-bounded — no cyclic-context machinery needed; generous `ATN_CLOSURE_GUARD`/`ATN_ARENA_CAP` are pathological-only. All 16 ATN unit + driver tests green.
   ...
   **Still open — O(n²) → O(n).** Each LR-loop iteration calls `predict`, whose closure explores the exit alt's **O(depth)** context-pop chain even when the decision resolves in one token; over the parse that is O(n²) (deep descriptor ≈ 48s, over the 5s test budget). Measured dead-ends: a per-`Parser` closure memo (full-context contexts encode the whole nesting depth, so config sets never recur across positions → memo never hits, net slowdown) and a persistent arena (grows the intern buckets). The clean fix is the **"drop loop entry branch"** optimization the descriptor is named for: in the closure, when the exit branch returns through the caller back to *this same* loop-entry decision, that re-entry is redundant with the direct enter branch and can be pruned — collapsing the O(depth) exit exploration to O(1). Clean-room from the construct's meaning (do **not** read `ParserATNSimulator.java`). Flip `Performance/DropLoopEntryBranchInLRRule_4` once the deep input runs in time. (`lr_between.g4` already exercises the correctness; a depth-scaling perf assertion pins the win.)
...
**Remaining for `DropLoopEntryBranchInLRRule_4` (P4.4 — perf).** The correctness gap is closed, but the descriptor's own 15-line deeply-nested input still **times out** (>5s). Measured on `lr_between.g4` with `N×("between X1 and X2 or ")`: depth 2/4/6/8 = 91ms / 136ms / 770ms / 5.85s (**≈×7 per +2 depth — exponential**), and by depth 12 the config set hits `ATN_MAX_CONFIGS` (6000) and the min-alt bail **mispredicts** (parse returns `Err`). So this is config blow-up _within_ a single prediction: the exact per-config `List<i32>` stacks fork without merging under deep left recursion (the #1398 perf bomb). A per-`Parser` DFA cache does **not** fix this (the growth is intra-prediction, not repeated decisions). The real fix is **graph-structured stacks** — represent the during-prediction stack as a shared `PredictionContext` DAG (hash-consed nodes, multi-parent merge) so configs converging on the same return state collapse instead of multiplying; this also removes the cap-induced misprediction. Sizeable: it reworks `AtnConfig.stack`/`pr_stack` and the closure push/pop. Flip the descriptor from `[stage_a_todo]` once it runs in time.

**Phasing.** P4.1 LR ATN build _(done)_ → P4.2a call-site↔return-state correspondence _(done)_ → P4.2b runtime rewire _(done)_ → P4.3 _(done: part 1 precedence eval, part 2 loop-entry wiring; `lr_between.g4` flipped green)_ → **P4.4 (next)** the real descriptor times out — measure, add graph-structured stacks / DFA cache, then flip `DropLoopEntryBranchInLRRule_4`.
