...
- [ ] A repeat's loop guard scans the whole body to decide the iteration and then drops which alternative it found; the iteration's dispatch picks again by lookahead, so the alternative the guard validated and the one the loop parses are two answers. An optional's guard no longer works this way. Measured: 88 loop guards in the corpus stand over a multi-alt group, none of which emits a second tournament, so today the cost is the re-decision rather than a second scan. A `+`'s mandatory first iteration is emitted without the guard — the op-only walker emits that iteration separately, the surface walker shares one body behind `_plus_first` — so that position has no winner to read.
- [ ] A group scanned inline inside a longer scan measures its own longest alternative — `gen_scan_op_group` argmaxes over the alternatives whose first sets overlap, first-matches the rest, and visits wildcard alternatives last — which is not the partition the parse-side dispatch picks with. So the enclosing decision can be measured on an alternative the parse will not take. Not currently observed in the corpus; `ScanGroupElement.dispatch` is lowered for these groups and read by nobody.
