...
- [ ] **A scan cannot evaluate a semantic predicate, so a gate the parse honours the scan still ignores.** With the gate above in place, `y = if v { a } else { b };` does not parse: `statements` chooses between `statement+ expression?` and a lone `expression` by scan length, `scan_statement` measures the `if` head's condition as the struct literal it is no longer allowed to be, fails, and the lone-expression alternative wins on `y` alone. `let y = if v { a } else { b };` parses, because no scan decides there.
  ...
  A `scan_*` fn is `(tokens, pos, follow) -> i32` and pure by design, which is what makes it usable speculatively; the flag it would have to read is parser state. ANTLR4 has no asymmetry here because its simulator holds the parser and evaluates predicates as it goes. Closing it means the scan side seeing the gate — the flag threaded beside `follow`, or the decision going to the ATN. This is the last thing between the corpus and zero: the 20 files reporting `macroItem` are items whose bodies fail this way.
