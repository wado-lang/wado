...
- [ ] Two operator alternatives opening on the same token (`expr NOT? IN …` and `expr NOT? BETWEEN …` on `NOT`) are decided by alternative order rather than lookahead: `atn_lr_loop_decision` takes the first precedence-allowed enter edge whose suffix admits the token. Latent — it needs a grammar whose LR loop runs on the simulator, and none committed does. (This entry used to say SQLite acquires the shape as soon as the mid-operand item lands; that landed without routing the rule through the simulator, and SQLite did not.)
