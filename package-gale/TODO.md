...
- The scannable `+` whose body is a group still admits an iteration silently: its first (mandatory) iteration shares one emit with the guarded later ones, so it is entered as `Guaranteed` and a token no alternative admits appends nothing. Every other required-group position reports (`gen_group_dispatch`), and a gated alternative is handled there by the loop's own gate break — only the ungated first iteration is left. Splitting that emit in two is what a fix needs.
