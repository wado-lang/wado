...
1. **LR ATN construction (`atn.wado`).** `build_atn` today builds the _pre-LR-rewrite_ surface alts (atn.wado:177), i.e. naive left-recursive `expr → RuleRef(expr) …` cycles the simulator cannot walk (infinite left-recursion). P4 must build the rule in the ANTLR4 rewritten form (`vendor/antlr4/doc/left-recursion.md`, license-OK):
