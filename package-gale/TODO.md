...
`gale dump` renders the overlap-group prediction tree (the same `build_prediction` tree `gen_multi_alt_body_bt` emits), so ATN-class edges surface as `Ambiguous([alt N, alt M]) — <reason>` leaves under the relevant rule, where `<reason>` (the `AmbiguityReason` enum in `prediction.wado`) names why the static path halted — e.g. `DropLoopEntryBranchInLRRule_4`'s `stat` resolves to `Ambiguous([alt 0, alt 1]) — opaque rule-ref prefix` at depth 0 (`expr ';'` and `expr '.'` share their entire `expr` prefix). The dump also carries the per-site LR loop-entry dispatch (`loop-entry:` with per-alt `conflict-min` and suffix-first overlap groups) and the per-call-site K-prefix mask on multi-token follow-variants (`k-prefix=[@0=… @1=…]`), so the LR precedence and multi-token-Repeat decisions are visible alongside the halt reason. The dump diagnostics are now complete; a concrete ATN-class fix can read all of them.
