...
**Remaining for `DropLoopEntryBranchInLRRule_4` (P4.4 — perf).** The correctness gap is closed, but the descriptor's own 15-line deeply-nested input still **times out** (>5s): the simulator runs at every loop-entry and the exact per-config `List<i32>` stacks blow up under deep left recursion (the #1398 perf bomb). Next: measure, then add a shared-stack representation (graph-structured stacks: a shared `PredictionContext` DAG + merge) and/or a per-`Parser` DFA cache so repeated loop-entry decisions hit a cache instead of re-simulating. Flipping the descriptor from `[stage_a_todo]` happens when it runs in time.
