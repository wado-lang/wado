...
- [ ] **A scan cannot evaluate a semantic predicate, so a gate the parse honours the scan still ignores.** With the gate above in place, `y = if v { a } else { b };` does not parse: `statements` chooses between `statement+ expression?` and a lone `expression` by scan length, `scan_statement` measures the `if` head's condition as the struct literal it is no longer allowed to be, fails, and the lone-expression alternative wins on `y` alone. `let y = if v { a } else { b };` parses, because no scan decides there.
  ...
  So the scan would have to _simulate_ the flag as it walks — apply the rule's mid-rule actions to a scan-local value and thread that through its own callees. That is ALL(\*) predicate simulation, which is the ATN, not a parameter. Route the decision there rather than growing the scan signature. This is the last thing between the corpus and zero: the 20 files reporting `macroItem` are items whose bodies fail this way, and `y = if v { a } else { b };` is the whole of it in one line.
