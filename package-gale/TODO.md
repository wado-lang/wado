...
**Codegen wiring (P4.3 part 2 — the remaining piece; flips the descriptor).** The simulator now has everything it needs (rewritten LR ATN, exact return-state stack, precedence eval). What remains is to route the LR loop-entry decision through it. When `ctx.needs_atn()` and the rule is LR with a loop-entry that is ATN-class (atoms whose own body recursively references the rule with a trailing literal that overlaps an LR suffix-first — i.e. the `between … 'and'` shape in `DropLoopEntryBranchInLRRule_4`), the generated LR loop dispatch (`gen_scan_lr_functions` / `gen_scan_lr_suffix_dispatch` and the parse-side loop) calls `atn_predict_with_stack(decision, &p.atn_stack, &p.tokens, p.pos, TK_EOF, min_prec)` to choose enter-loop-alt-i vs exit, instead of the static suffix-first dispatch. Open sub-tasks, in order:
  - **Detect ATN-class LR rules** (the heuristic above) and `mark_needs_atn` for them (today only non-greedy `??` sets it). Keep the static `scan_X_lr_*` fast path for every LR rule that is NOT ATN-class (the common case stays byte-identical).
  - **Decision identity.** Stamp the loop-entry decision number (from `build_atn`'s `STAR_LOOP_ENTRY`) onto the `LeftRecursiveRule` so the emitter and the ATN agree. (`build_atn` already builds the loop-entry; thread its `decision` back, the same single-source stamping used for call sites.)
  - **Emit the simulator call** at the loop dispatch; map the returned alt index (enter-alt-i vs the trailing exit edge) onto the existing suffix-helper dispatch.
  - **Fixture + flip.** Add `tests/grammars/lr_between.g4` (`between X1 and X2 and X3`) + a driver test; flip `Performance/DropLoopEntryBranchInLRRule_4` from `[stage_a_todo]`. Validate the whole `LeftRecursion/*` + LR corpus stays green (the static path must be untouched for non-ATN-class LR rules).
