...
- [ ] `column_def`'s `type_name?` is not entered on `a UNSIGNED BIG INT` (rejected outright) or on `a CONSTRAINT c NOT NULL DEFAULT 1` (`CONSTRAINT c` lands under `column_constraint` where ANTLR4 reads it as the type name) — two `sqlite` oracle cases, `#[TODO]`. `_parse_type_name` is called with `&EMPTY_FOLLOW`, so its `name+?` probe reads "the caller cannot continue" everywhere and always takes one name; the entry probe then measures viability against that one name.
  ...
  Three measurements, none shippable. (1) Asking the probe whether the token appears at any depth of the mask (rather than `follow_yields`'s lockstep read) and giving the `e?` body a mask of its own resolves both cases, and leaves `a VARCHAR(10), b DECIMAL(10, 2)` broken — `column_def` inside `(',' column_def)*` gets a mask with no `,` in it. (2) Also handing a repeat body's tail the enclosing continuation repairs that and breaks `htmlContent` nested tags, CSS `selector`'s child combinator and `FullContextParsing/ExprAmbiguity_2`: one mask feeds both the probe and every `follow_yields` gate, so widening it for the probe widens the gates — the over-broad guard `AGENTS.md` records. (3) Conjoining the rule's classical FOLLOW (`rule_follow_kinds`, moved ahead of lowering) narrows nothing: that fixed point is built for `at_end_conflict_routes`, where over-approximating is the safe direction, and for `type_name` — two call sites, real continuations `FIRST(column_constraint)`, `,`, `)` — it yields every keyword plus `IDENTIFIER`.

  What is left is the precise per-call-site continuation, which only the call site knows and only a second runtime argument can carry. It has to be separate from `follow`, since (2) is what sharing costs.
