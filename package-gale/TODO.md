...
2. **A descriptor re-extract** whenever a JDK and the `vendor/antlr4` submodule are at hand. Last run 2026-08-24 (all three phases, 98/98 oracle invocations clean): the corpus regenerated byte-identically, so no entry below is merely stale. The skip buckets are small; the standing value is that a re-extract is what proves an entry is still blocked rather than merely old.
