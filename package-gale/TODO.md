...
- [ ] A non-greedy loop at a rule's tail still takes the minimum when the caller's continuation is nullable: `type_name`'s `name+?` stops one name early on `a UNSIGNED BIG INT` and `a CONSTRAINT c NOT NULL DEFAULT 1` (two `sqlite` oracle cases, `#[TODO]`).
  ...
  Elsewhere the ambiguity is decided by scanning the continuation, but at a rule's tail there is nothing local to scan and the probe falls back to the runtime FOLLOW mask. Measured: handing that mask `column_def`'s local suffix as a `FollowArg::Combined` prefix fixes `a UNSIGNED BIG INT` and breaks `a VARCHAR(10), b DECIMAL(10, 2)`.
