...
- [ ] A lexer rule that cannot match at its first char never reaches a catch-all (`ANY : .`): the catch-all is excluded from every specific dispatch branch on the assumption that a rule reaching that branch matches at least one char, which is false for `A : [a-f] '1' ; C : . ;` on the input `e`. The exclusion applies to single-char branches as much as range ones.
