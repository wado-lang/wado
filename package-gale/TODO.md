...
**Status:** the **parser** half has landed (java2wado Phase 3; design in [`action.md`](./action.md)). A non-superClass `language = Java` grammar's parser actions, predicates, `@members`, and the ctx-cast LR value idiom are translated to Wado and execute during the parse (`driver_java_{action,members,pred}_test`). The **lexer** half is under way (Phase 4): un-executed lexer `{ ... }` actions now warn loudly instead of vanishing (`warn_unhandled_lexer_actions`), and a single-alt `language = Wado` lexer action runs at the tournament commit via winner-replay. The command-style family — `lx.set_type` / `set_channel` / `skip` / `more` — and the mode ops — `lx.push_mode` / `pop_mode` / `set_mode` (with emitted `MODE_<NAME>` constants and the mode-grammar tournament wired) — execute end to end (`driver_action_lex_set_type_test`, `driver_action_lex_cmd_test`, `driver_action_lex_mode_test`). Remaining lexer surface (`@lexer::members`, print, multi-alt / nested placement, the LATN path) and the **SuperClass trait** for the real-world grammars below are still open; each remaining item's specific prerequisite is listed in [`action.md`](./action.md) (Phase 4). A follow-up harness step will make the ANTLR descriptor `[output]` corpus codegen-and-compare (it is parse-only today), unblocking the `[output]` acceptance.
