...
- **Pin the `superClass` lexers as their own Stage B′ key.** `antlr4-oracle.sh --super` now answers for `RustLexer` against the same base class `driver_cst_rust_test` models, so its token stream can be oracle-pinned the way `sqlite` and `json` pin trees. `regen-oracle.sh` pins `to_string_tree()` output only; a token-stream key needs a normalizer from TestRig's `[@i,a:b='text',<Type>,l:c]` dump to Gale's `token_kind_name` + text, and a Gale-side dump that installs the base handler.
- **`TypeScriptLexer` and `ANTLRv4Lexer` have no oracle at all** until each has a base class on both sides — a Wado `impl` (blocked on Stage C action ops, above) and its `tests/grammars/java/` twin. `--probe-super` deliberately does not substitute: `ANTLRv4Lexer` declares `TOKEN_REF` / `RULE_REF` in `tokens {}` with no rule producing them, so `LexerAdaptor` assigns them from an `emit()` override that no stub derived from the grammar can have. Writing `LexerAdaptor` clean-room from the observable token stream is its own task; until then those grammars are pinned only by parse-success.
