...
- [ ] The `sqlite` Stage B′ `#[TODO]`s are independent divergences, each worth its own fixture. Two produce a wrong tree with no diagnostic, so they rank with the rest of this section: `sql_stmt_list`'s trailing `';'*` loses to the `( ';'+ sql_stmt )*` iteration, so `SELECT 1;` is accepted with a phantom empty `sql_stmt` appended; and the `BETWEEN` operands climb from too high a precedence, so `a NOT BETWEEN 1 AND 2 AND b` brackets as `(a BETWEEN 1 AND 2) AND b`. The rest surface as a tree difference or an outright rejection: `column_def` skips `type_name?` where ANTLR4 enters it (`a NOT NULL`, `a CONSTRAINT c NOT NULL`); `type_name`'s non-greedy `name+?` stops after one name, rejecting `a UNSIGNED BIG INT`; and `IN`'s bare `table_name` branch (`a IN t`) is rejected.
