...
- [ ] `a NOT BETWEEN 1 AND 2 AND b` brackets as `(a BETWEEN 1 AND 2) AND b` — a wrong tree with no diagnostic. Fixtures `lr_mid_operand.g4` and `lr_between.g4`, both `#[TODO]`, plus the `sqlite` oracle case. The mid operand is emitted as `_parse_e(p, <alt's own precedence>)`, which cannot climb the shared delimiter; ANTLR4 emits `e[0]` and stops it by prediction. Against the jar, `a between b and c` keeps its operand at `b`, `a between b and c and d` climbs to `b and c`, and `a between b and c and d and e` to `b and c and d` — climb while a spare delimiter is left, which no k-token lookahead sees (depth 0 and 1 are `and`, ID either way).

  Measured: routing the operand through the ATN simulator puts it on every operator of a hot expression rule ([`perf.md`](./perf.md) has the numbers). A cheaper route exists and is half-built — stamp the operand `e[0]`, and gate each LR loop iteration on the caller's continuation still matching after the operator's suffix scan, which is the viability probe the `?` / `*?` sites already use. The parse side of that works (`_parse_e(p, follow, 0, &FOLLOW_MASK_n)` with the gate reading a dedicated `lr_cont` argument, kept separate from `follow` because every caller sets that one and the loop must not confuse the two). It was reverted because the scan twins have no `lr_cont`: their mid operand climbs greedily, `scan_e_lr_2` then cannot find its `and`, and `a between b and c` stops parsing at all. Finishing it means threading `lr_cont` through the scan functions in lockstep.
