...
- `@lexer::members` for a `language = Java` grammar. A Java member method takes `&mut self`, but a lexer predicate runs inside `try_<rule>(lx: &Lexer, ...)` — the tournament must not mutate through a losing candidate — so a body reaching one would emit a call the compiler rejects. Java lexer bodies therefore see no members and a reference is reported. Wiring them needs a split between members a predicate may read and members only an action may touch.
- Two same-named rule labels bound to _different_ rules in different alternatives (`x=a | x=b`). Per-alternative resolution disambiguates token-vs-rule, but the binding it consults records only that much, so a `.field` read on the wrong rule's value channel still resolves against the first-declared one. `$<label>.text` is unaffected (it reads the call's own span).
