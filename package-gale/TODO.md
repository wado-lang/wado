...
- [ ] A `\p{...}` escape inside a char-set _range_ is accepted and given an invented meaning. `[\p{L}-z]`, `[a-\p{L}]` and `[\p{L}\p{Nl}-\p{Pattern_Syntax}]` all parse; ANTLR4 rejects each with `error(182): unicode property escapes not allowed in lexer charset range`. There is no range from one property to another, so this is the "reject loudly" case, not the superset case. The third form appears (commented out) in `RustLexer.g4`, where it is Rust-spec set-difference notation that ANTLR4 does not have.
- [ ] `\p{...}` reaches the UCD and the POSIX surface, but not ANTLR4's enumerated properties beyond `General_Category=` / `Script=` / `Block=`. Measured against the published jar over 42 candidates: the remaining gap is exactly twelve `Property=Value` names — `Bidi_Class`, `Line_Break`, `Word_Break`, `Sentence_Break`, `Grapheme_Cluster_Break`, `Numeric_Type`, `East_Asian_Width`, `Hangul_Syllable_Type`, `Joining_Type`, `Indic_Syllabic_Category`, `Decomposition_Type`, `Vertical_Orientation`. Each needs a UCD input `scripts/regen-unicode-tables.sh` does not fetch (`LineBreak.txt`, `auxiliary/*.txt`, `EastAsianWidth.txt`, …) and a new table; they partition the whole code-point space, so the 159 KB of tables grows materially. The same measurement settles what is _not_ a gap, so it needs no re-investigation: ANTLR4 itself rejects `\p{LC}`, `\p{Cased_Letter}`, `\p{Punct}`, `\p{Word}`, `\p{Script_Extensions=}`, `\p{Age=}` and `\p{Canonical_Combining_Class=}`. No corpus grammar needs any of it — the two real-world lexers use general categories only, and `RustLexer.g4`'s ICU-looking lines are comments.
