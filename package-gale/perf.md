...
(2) the runtime fragments are `#include_str`'d and may import only sibling
`runtime/*.wado` files, so `impl Deserialize for AtnSim` can't live with the
struct; codegen would
