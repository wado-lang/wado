...
| Open-ended (`.` / `~X`-led, empty static `suffix_first`) LR suffix in an ATN-class rule                 | `valid_lr_alts` is built over the **same** real-suffix set as the loop-entry enter edges, so the simulator's edge index lines up with the dispatch branches; the rule is forced ATN-class. This is the superset case (ANTLR4 rejects it). Fixtures `tests/grammars/lr_complement_op.g4`, `lr_wildcard_postfix.g4` |
| Self-reference nested inside a Group/Repeat LR-suffix element (wrong precedence floor)                  | `build_lr_suffix_element` recurses through the container, deciding each self-ref's precedence floor from its **own** follow — `conflict_min` when an LR operator can follow, else `0` (a primary reference like a bracketed arg). Test: `build_atn handles a nested self-reference in an LR suffix`               |
| Non-greedy `??` inside a left-recursive rule (prediction at `min_prec 0` ignored the raised precedence) | `atn_ng_optional_enter` takes a `min_prec` parameter and forwards it to `atn_predict_with_stack`, so the `??` decision honors the loop's precedence floor                                                                                                                                                         |
