...
Higher stages depend on lower stages already being green.
...
Two test sources cover Stage A:
...
### Stage A2 — generated-parser-accepts-input (no tree comparison)

> **Claim:** For every descriptor whose upstream `[output]` describes a
> *successful* parse, the parser generated by Gale accepts the
> descriptor's `[input]` without returning an error.

Stage A2 sits between Stage A (the `.g4` source parses) and Stage B
(parse-tree shape equivalence). It is strictly weaker than Stage B but
covers the descriptors Stage B's normaliser auto-skips — `$text` echoes,
sub-tree prints, action-block output, token-name streams — because none
of those reflect the parse tree, yet the generated parser must still
accept the canonical input.

Stage A2 is emitted automatically for every descriptor that satisfies:

- `[type]` is `Parser` (lexer-only descriptors are Stage L territory;
  composite types await multi-input plumbing),
- non-empty `[input]`,
- empty `[errors]` (descriptors that expect parse errors are Stage E),
- single grammar (no `[slaveGrammar]` blocks).

Triage is independent of Stage A and Stage B via `[stage_a2_todo]` /
`[stage_a2_skip]` in `status.toml`.
...
tests/antlr4-compat/<category>_test.wado            (Stage A: one test per
                  │                                  descriptor, `#![generated]`)
tests/antlr4-compat/stage_a2/<Category>/<Name>_test.wado
                  │                                 (Stage A2: parse-accepts-input,
                  │                                  one file per eligible descriptor)
tests/antlr4-compat/stage_b/<Category>/<Name>_test.wado
                  │              │                  (Stage B: tree-shape equivalence,
                  │              │                   one file per eligible descriptor)
                  │              └── all three consult tests/antlr4-compat/status.toml
...
| `tests/antlr4-compat/<category>_test.wado` | Generated Stage A tests (committed; each carries `#![generated(by=…)]`) |
| `tests/antlr4-compat/stage_a2/`            | Per-descriptor Stage A2 drivers (committed)                      |
...
[stage_a2_todo]
"<Category>/<Name>" = "one-line reason Stage A2 parse-accepts-input fails"

[stage_a2_skip]
"<Category>/<Name>" = "one-line reason Stage A2 emission must be skipped"
...
| `[todo]`          | Stage A test marked `#[TODO]` so failure is expected and recorded | Descriptor exposes a real Gale gap (parser, lexer, or IR feature). Add a corresponding entry to `TODO.md`                                   |
| `[skip]`          | Stage A test not emitted at all                                   | Descriptor relies on the upstream test runner's StringTemplate substitution / target-language template, so it is not a self-contained `.g4` |
| `[stage_a2_todo]` | Stage A2 test emitted with `#[TODO]` so failure is expected       | Generated parser rejects the canonical `[input]` (typically LL prediction, codegen, or `[start]` rule mismatch)                             |
| `[stage_a2_skip]` | Stage A2 test not emitted                                         | Gale codegen produces invalid Wado, or `[start]` differs from `parse()`'s entry by design                                                   |
