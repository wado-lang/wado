...
language source is currently *skipped* during code generation. That
...
Today the g4 frontend *recognises* these constructs and preserves
...
behaviour can still pass *Stage A* (the grammar parses) and may pass
*Stage B* if the action body is `Inspect`/`writeln`-style chatter
