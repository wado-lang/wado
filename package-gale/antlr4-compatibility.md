...
### Stage A — syntactic compatibility plus generated-parser-accepts-input

> **Claim:** (a) Every well-formed `.g4` file accepted by upstream
> `antlr4` is accepted by Gale's `g4::parse` (modulo the action-body
> exception documented in [`AGENTS.md`](./AGENTS.md)); and (b) for
> every descriptor whose upstream `[output]` describes a _successful_
> parse, the parser generated by Gale accepts the descriptor's
> `[input]` without returning an error.
...
2. **Upstream descriptor corpus** at
   ...
   for claim (a).
3. **Per-descriptor parse-accepts-input drivers** under
   `tests/antlr4-compat/stage_a/<Category>/<Name>_test.wado`, also
   emitted by the descriptor extractor, for claim (b). Strictly
   weaker than Stage B, but covers the descriptors Stage B's
   normaliser auto-skips — `$text` echoes, sub-tree prints,
   action-block output, token-name streams — because none of those
   reflect the parse tree, yet the generated parser must still
   accept the canonical input.

The per-descriptor parse-accepts-input drivers (3) are emitted
automatically for every descriptor that satisfies:
...
- `[start]` rule matches the grammar's first parser rule (Gale's
  `parse()` entry).
...
tests/antlr4-compat/<category>_test.wado            (Stage A claim (a): one
                  │                                  test per descriptor,
                  │                                  `#![generated]`)
tests/antlr4-compat/stage_a/<Category>/<Name>_test.wado
                  │                                 (Stage A claim (b):
                  │                                  parse-accepts-input,
...
[stage_a_todo]
"<Category>/<Name>" = "one-line reason the generated parser rejects [input]"

[stage_a_skip]
"<Category>/<Name>" = "one-line reason the parse-accepts-input driver must be skipped"
