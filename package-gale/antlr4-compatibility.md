...
### Token identity — a parser literal aliases its lexer rule

A parser string literal whose text is the **entire body of a default-mode
lexer rule** denotes that rule's token type. The two spellings are one
token, so `s : 'kw' | K 'x' ;` with `K : 'kw' ;` puts both alternatives on
`K` and decides between them by lookahead. `alias_parser_literals`
(`src/ir.wado`) rewrites such a literal to a `TokenRef` before
`collect_literal_tokens` runs, so no anonymous `TK_LIT_*` is minted for it
and the rule keeps its own matcher, its commands, and its `caseInsensitive`
setting. Fixture `tests/grammars/lit_alias_ref.g4`.
