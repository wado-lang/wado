...
fn test_format_keeps_resource_extends() {
    ...
    assert!(
        formatted.contains("linearity = \"unrestricted\""),
        "the linearity must survive formatting, got:\n{formatted}"
    );
...
fn test_format_field_visibility_modifiers_roundtrip() {
    let source = "struct S {\n    pub a: i32,\n    internal b: i32,\n    c: i32,\n}\n";
    let formatted = wado_compiler::format(source).expect("format failed");
    assert!(formatted.contains("pub a: i32"), "got:\n{formatted}");
    assert!(formatted.contains("internal b: i32"), "got:\n{formatted}");
    assert!(formatted.contains("\n    c: i32"), "got:\n{formatted}");
    assert_format_preserves_ast(source);
    let formatted2 = wado_compiler::format(&formatted).expect("reformat failed");
    assert_eq!(formatted, formatted2, "format should be idempotent");
...
///
/// `format.fixtures/all_dirty.wado` exercises all the syntax constructs whose
...
///
/// `format.fixtures/mess_dirty.wado` has many deliberately weird patterns.
...
///
/// `format.fixtures/ops.all_dirty.wado` has redundant parens and inconsistent
...
///
/// `format.fixtures/ops.mess_dirty.wado` has messy formatting around operators.
