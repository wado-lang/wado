...
- `build_tir` emits TIR from `&Semantics`; used only by batch compilation.
...
- Use→def edges are recorded by the real elaborator as it performs name resolution (`resolve_ident`, `resolve_call`, …). `Semantics::referenced_symbol` is the single source of truth; there is no separate lexical re-scan. `semantics_of` always drives `build_tir` so the edges exist for both LSP and batch compilation.
