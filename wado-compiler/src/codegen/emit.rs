...
/// Number of scratch i64 locals reserved per function for the
/// `-f no-wide-arithmetic` software lowering (see `declare_wide_arith_scratch`).
/// Sized for the widest expansion, the 64×64→128 unsigned multiply.
...
impl<'a> WirEmitter<'a> {
    ...
    /// Pool of scratch i64 locals for the `-f no-wide-arithmetic` software
    /// lowering of the 128-bit ops. A single pool is shared across every
    /// wide-arith site in a function: each expansion evaluates its operands
    /// into these slots and consumes them before control leaves the op, so the
    /// slots are never simultaneously live across sites (the ops never nest —
    /// a multi-value 128-bit result is never an operand of another). Declared
    /// once per function, deduped on the first slot name.
    fn declare_wide_arith_scratch(&mut self, locals: &mut Vec<(String, ValType)>) {
    ...
    /// Software 64×64→128 multiply for `-f no-wide-arithmetic`. Pushes
    /// `[low, high]` (high on top), matching native `i64.mul_wide_u/s`. Splits
    /// each operand into 32-bit limbs and sums the partial products; for the
    /// signed form the unsigned product's high word is corrected by subtracting
    /// `b` when `a < 0` and `a` when `b < 0`.
    fn emit_mul_wide_soft(&mut self, f: &mut Function, a: &WirInstr, b: &WirInstr, signed: bool) {
    ...
    /// Software 128-bit add/sub for `-f no-wide-arithmetic`. Pushes
    /// `[low, high]`, matching native `i64.add128` / `i64.sub128`. The
    /// carry (add) or borrow (sub) out of the low word is recovered with an
    /// unsigned compare.
    #[allow(clippy::too_many_arguments)]
    fn emit_add_sub_128_soft(
        &mut self,
        f: &mut Function,
        a_lo: &WirInstr,
        a_hi: &WirInstr,
