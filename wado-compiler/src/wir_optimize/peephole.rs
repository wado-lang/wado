...
/// and by `MultiValueLiteral` lowering / `emit_stream_or_future_new`:
///
/// ```text
...
/// sequence. If a later instruction reads `T` (e.g., `LocalGet(T)` from a
/// second destructure of the same tuple, or any other field access), the
/// pass refuses to fire — otherwise that later read would observe an
/// uninitialised local.
fn elide_multi_value_structs(instrs: &mut Vec<WirInstr>, types: &[WirTypeDef]) {
    ...
    while i < instrs.len() {
        ...
        // Step 5b: Reject elision if `temp_name` is read anywhere after the
        // consecutive StructGet sequence (any descendant `LocalGet(temp)` or
        // `LocalSet(temp, …)` invalidates the rewrite, since the LocalSet
        // we are about to drop is the only definition of `temp`).
        if any_local_use_in_body(&instrs[j..], &temp_name) {
            // Keep the instructions as-is and skip past the LocalSet so we
            // don't loop on the same anchor.
            result.push(std::mem::replace(&mut instrs[i], WirInstr::Nop));
            if has_declare {
                result.push(std::mem::replace(&mut instrs[set_idx], WirInstr::Nop));
            }
...
fn any_local_use_in_body(body: &[WirInstr], name: &str) -> bool {
    ...
    impl WirRefVisitor for UseFinder<'_> {
        fn visit_instr(&mut self, instr: &WirInstr) {
            ...
            match instr {
                WirInstr::LocalGet { name, .. } if name == self.name => self.found = true,
                WirInstr::LocalSet { name, .. } if name == self.name => self.found = true,
                WirInstr::LocalTee { name, .. } if name == self.name => self.found = true,
