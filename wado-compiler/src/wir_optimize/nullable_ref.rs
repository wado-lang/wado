...
use crate::wir::{
    WirAbstractHeapType, WirInstr, WirModule, WirType, WirTypeDef, WirVariantRepr,
};
...
fn is_nullable_ref_eligible(
    cases: &[crate::wir::WirVariantCase],
) -> Option<(u32, WirType)> {
    if cases.len() != 2 {
        return None;
    }
...
fn collect_nullable_variants(types: &[WirTypeDef]) -> IndexMap<u32, (u32, WirType)> {
    ...
    for (idx, typedef) in types.iter().enumerate() {
        if let WirTypeDef::Variant(vt) = typedef {
            if let Some((payload_case, nullable_ty)) = is_nullable_ref_eligible(&vt.cases) {
        ...
        let to_remove: Vec<u32> = candidates
            ...
            .filter(|&variant_idx| {
                if let WirTypeDef::Variant(vt) = &types[variant_idx as usize] {
                    ...
                    if let WirType::Ref { type_id, .. } =
                        &vt.cases[payload_case_pos].payload[0]
                    {
...
fn substitute_type(ty: &mut WirType, nullable_map: &IndexMap<u32, (u32, WirType)>) {
    if let WirType::Ref { type_id, .. } = ty {
        if let Some((_, nullable_payload)) = nullable_map.get(&type_id.index()) {
        ...
        match &module.types[i] {
            WirTypeDef::Struct(_) => {
                ...
                for field_idx in 0..field_count {
                    ...
                    if needs_update {
                        if let WirTypeDef::Struct(s) = &mut module.types[i] {
            ...
            WirTypeDef::Func(_) => {
                ...
                for j in 0..param_count {
                    ...
                    if needs_update {
                        if let WirTypeDef::Func(ft) = &mut module.types[i] {
            ...
            WirTypeDef::Array(_) => {
                ...
                if needs_update {
                    if let WirTypeDef::Array(a) = &mut module.types[i] {
...
fn transform_instr(
    ...
    match instr {
        ...
        WirInstr::DeclareLocal { ty, .. } => {
            ...
            if let WirType::Ref { type_id, .. } = ty {
                if let Some(&(variant_idx, case_idx)) = vci.get(&type_id.index()) {
                    if let Some(&(payload_case, ref nullable_payload)) =
                        nullable_map.get(&variant_idx)
        ...
        WirInstr::StructNew { type_id, fields } => {
            ...
            if let Some(&(variant_idx, case_idx)) = vci.get(&type_id.index()) {
                if let Some(&(payload_case, _)) = nullable_map.get(&variant_idx) {
            ...
            if let Some(&(payload_case, _)) = nullable_map.get(&type_id.index()) {
                if let Some(WirInstr::I32Const(disc)) = fields.first() {
            ...
            for (i, f) in fields.iter_mut().enumerate() {
                ...
                if nullable_field_indices.contains(&i) {
                    if let WirInstr::RefAsNonNull(inner) = f {
        ...
        WirInstr::StructSet {
            ...
            let field_is_nullable =
                if let Some(WirTypeDef::Struct(st)) = types.get(type_id.index() as usize) {
                    st.fields.iter().find(|f| f.name == *field_name).is_some_and(|f| {
                        matches!(f.ty, WirType::Ref { nullable: true, .. })
                    })
                } else {
                    false
                };
            if field_is_nullable {
                if let WirInstr::RefAsNonNull(inner) = value.as_mut() {
                    let inner_val = std::mem::replace(inner.as_mut(), WirInstr::Nop);
