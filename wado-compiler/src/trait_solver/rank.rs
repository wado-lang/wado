...
mod tests {
    ...
    /// The prelude turns on this: `RangeExclusive<T>` implements `Iterator`, so
    /// `impl<I: Iterator> IntoIterator for I` applies to every range beside the
    /// `impl<T: Step + Ord> IntoIterator for RangeExclusive<T>` written for it,
    /// and without this rank every `for x of 0..n` is ambiguous.
    #[test]
