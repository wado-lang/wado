...
/// The integer a float → int cast produces: truncated toward zero, and
/// saturated at `target`'s bounds with NaN as 0, which is what both the
/// runtime's clamp-and-`trunc_sat` and Rust's own `as` compute.
///
...
pub(crate) fn float_to_int(value: f64, prim: PrimitiveType, target: PrimitiveType) -> Value {
    ...
    let bits = match target {
        ...
        | PrimitiveType::Bool
        | PrimitiveType::Char
        | PrimitiveType::V128 => panic!("float_to_int: non-integer target {target:?}"),
    };
    Value::Int {
        value: bits,
        prim: target,
