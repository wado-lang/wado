...
fn desugar_stmt(stmt: &Stmt, ctx: &mut DesugarContext) -> Stmt {
    match stmt {
        ...
        // ForOf is NOT desugared here - it's handled by the resolver which has
        // special fast-path for Array<T> iteration. Desugaring here would lose
        // the type information needed for the optimized path.
    ...
    fn test_desugar_compound_assign() {
        ...
        let ca = CompoundAssignExpr {
            ...
            value: Expr::Literal(LiteralExpr {
                value: crate::ast::Literal::Number(crate::ast::NumberLiteral {
                    repr: "1".to_string(),
                }),
                span: dummy_span(),
            }),
