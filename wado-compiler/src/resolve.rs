...
impl std::fmt::Display for ResolveError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ResolveError::UnknownFunction { name, span } => {
                write!(
                    f,
                    "{}:{}: unknown function '{}'",
            ...
            ResolveError::UnknownVariable { name, span } => {
                write!(
                    f,
                    "{}:{}: unknown variable '{}'",
                    span.line, span.column, name
...
impl<'a> Resolver<'a> {
    ...
    ) -> Result<TirModule, Vec<ResolveError>> {
    ...
    pub fn resolve_all_modules(
        ...
        entry_path: &[String],
        entry_source: &'a str,
    ) -> Result<HashMap<Vec<String>, TirModule>, Vec<ResolveError>> {
