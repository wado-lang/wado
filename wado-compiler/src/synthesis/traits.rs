...
pub(crate) struct TraitsStdlibNames {
    ...
    pub display_fq: crate::name::FqTraitName,
    pub display_alt_fq: crate::name::FqTraitName,
    pub inspect_fq: crate::name::FqTraitName,
    pub inspect_alt_fq: crate::name::FqTraitName,
    pub lower_hex_fq: crate::name::FqTraitName,
...
impl TraitsStdlibNames {
    pub(crate) fn from_type_table(type_table: &crate::tir::TypeTable) -> Self {
        ...
        Self {
            ...
            inspect_alt_fq: items.trait_fq(CompilerItem::InspectAlt),
...
struct TraitPair {
    ...
    delegate_trait: crate::name::FqTraitName,
...
impl TraitPair {
    fn display_alt(names: &TraitsStdlibNames) -> Self {
        ...
        Self {
            target_trait: names.display_alt_fq.clone(),
            target_method: names.display_alt_method.clone(),
            delegate_trait: names.display_fq.clone(),
...
fn generate_enum_trait_impls(module: &mut TirModule, ctx: &mut SynthesisCtx<'_, '_, '_>) {
    ...
    let (eq_trait_name, ord_trait_name) = {
        ...
        let items = tt.compiler_items();
        (
            items.trait_fq(crate::compiler_item::CompilerItem::Eq),
            items.trait_fq(crate::compiler_item::CompilerItem::Ord),
        )
    };
...
fn generate_variant_eq_impls(module: &mut TirModule, ctx: &mut SynthesisCtx<'_, '_, '_>) {
    ...
    let eq_trait_name = module
        .type_table
        .borrow()
        .compiler_trait_fq(crate::compiler_item::CompilerItem::Eq);
    let mut tt = module.type_table.borrow_mut();
...
fn generate_enum_display_fn(
    ...
    span: Span,
    display_trait: &crate::name::FqTraitName,
    display_method: &str,
...
fn generate_fn_inspect_alt_fn(
    ...
    fmt_type: TypeId,
    span: Span,
    inspect_alt_trait: &crate::name::FqTraitName,
...
fn generate_inspect_alt_impls(module: &mut TirModule, ctx: &mut SynthesisCtx<'_, '_, '_>) {
    ...
    let inspect_fq = ctx.names.inspect_fq.clone();
    let inspect_alt_fq = ctx.names.inspect_alt_fq.clone();
    ...
    for (type_id, _base_name, type_arg_names) in collect_parameterized_types(&tt) {
        ...
        let resolved = tt.get(type_id).clone();
        if matches!(resolved, ResolvedType::GenericInstance { def, .. } if TypeTable::is_tuple_type(tt.def_name(def)))
        {
...
fn generate_struct_inspect_alt_fn(
    ...
    let method_info = trait_method_info(receiver, inspect_alt_trait, inspect_alt_method);
    let qualified_name = method_info.to_mangled_name();
...
fn generate_fallback_impls(
    ...
    let module_source = module.module_source.clone();
    let is_display_pair = pair.target_trait.base_name() == ctx.names.display;
    let all_fn_names: IndexSet<String> = module
        .functions
        .iter()
        .filter_map(|f| f.try_borrow().ok().map(|func| func.name.clone()))
    ...
    let needs_fallback = |receiver: &FqTypeName, ctx: &SynthesisCtx<'_, '_, '_>| -> bool {
        ...
        let delegate_key =
            MethodName::format_local(receiver, Some(&pair.delegate_trait), &pair.delegate_method);
    ...
    for (type_id, base_name, type_arg_names) in collect_parameterized_types(&tt) {
        ...
        let ref_type = tt.make_ref(type_id);
        let base_head = tt.fq_base_type_name(type_id);
        let target_info = trait_method_info(&base_head, &pair.target_trait, &pair.target_method)
            .with_struct_type_args(&type_arg_names);
        let delegate_info =
            trait_method_info(&base_head, &pair.delegate_trait, &pair.delegate_method)
                .with_struct_type_args(&type_arg_names);
        generated.push(Rc::new(RefCell::new(generate_display_fallback(
            target_info,
            delegate_info,
...
fn generate_enum_ord_fn(
    ...
    ref_enum_type: TypeId,
    ordering_type: TypeId,
    ord_trait_name: &crate::name::FqTraitName,
    span: Span,
...
fn cmp_call_expr(
    ...
    let ref_type = tt.make_ref(field_type);
    let arg = ref_expr(other_field, ref_type, span);
    let ord_trait_name = tt.compiler_trait_fq(crate::compiler_item::CompilerItem::Ord);
    trait_call_on_type(
        self_field,
        field_type,
        &ord_trait_name,
