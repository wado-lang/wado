...
/// Every compiler trait item names a declaration, so its canonical key is
...
/// types that don't already have user-provided implementations.
pub fn synthesize_traits(project: Package) -> Package {
    ...
    let (eq_trait_key, ord_trait_key) = {
        ...
        (
            ...
            items.trait_fq(CompilerItem::Ord).canonical().expect(KEYED),
        )
    };
    ...
    for module in project.tir_modules.values_mut() {
        ...
        // (`peel_transparent_newtype`), not here.
...
fn generate_struct_reflect_impls(
    ...
    for target in &targets {
        ...
        let methods =
            generate_struct_reflect_methods(&module.type_table, &env, reflect_trait_name, target);
        generated.extend(methods.into_iter().map(|f| Rc::new(RefCell::new(f))));
        ctx.record_impl(
            &target.receiver,
            &reflect_trait_name.canonical().expect(KEYED),
        );
    }
...
fn collect_reflect_targets(module: &TirModule) -> Vec<ReflectTarget> {
    ...
    module
        ...
        .filter(|s| match s.def {
            StructDef::Anon(shape) => tt.template_shape(shape).is_none(),
            StructDef::Decl(_) => true,
        })
        .map(|s| ReflectTarget {
            name: s.name.clone(),
            def: s.def,
...
fn resolve_member_struct(tt: &TypeTable, item: CompilerItem) -> (String, DefId) {
...
fn generate_struct_members_fn(
    ...
    let method_info = trait_method_info(receiver, reflect_trait_name, &env.members_method);
    let qualified_name = method_info.to_mangled_name();
        ...
        .map(|(f, member_type)| {
            let wire_override = {
                ...
                match &f.wire_name_override {
                    ...
                    None => common::option_none(option_string_type, items),
                }
            };
...
fn generate_struct_empty_slots_fn(
    ...
    let elements = slot_types
        .iter()
        .map(|&slot_type| common::option_none(slot_type, &items))
        .collect();
...
fn generate_struct_from_fields_fn(
    ...
    let method_info = trait_method_info(receiver, reflect_trait_name, &env.from_fields_method);
    let qualified_name = method_info.to_mangled_name();
...
pub(super) fn generate_read_bridge_helpers(
    ...
    let mut by_value_type: hashmap::IndexMap<String, Vec<&ReadBridgeMember>> =
...
fn generate_type_name_fn(
    ...
    let method_info = trait_method_info(receiver, reflect_trait_name, type_name_method);
    let qualified_name = method_info.to_mangled_name();
...
fn generate_template_reflect_impls(
    module: &mut TirModule,
    ctx: &mut SynthesisCtx<'_, '_, '_>,
    template_trait_name: &FqTraitName,
) {
    ...
    for target in &targets {
        ...
        let mut functions = vec![
            ...
            string_fn(&tail_method, &unescape_template_segment(tail_raw)),
...
fn generate_variant_reflect_impls(
    module: &mut TirModule,
    ctx: &mut SynthesisCtx<'_, '_, '_>,
    variant_trait_name: &FqTraitName,
) {
    ...
    for target in &targets {
        let methods =
            generate_variant_reflect_methods(&module.type_table, &env, variant_trait_name, target);
        generated.extend(methods.into_iter().map(|f| Rc::new(RefCell::new(f))));
        ctx.record_impl(
            &target.receiver,
            &variant_trait_name.canonical().expect(KEYED),
        );
    }
...
fn generate_variant_cases_fn(
    ...
    let method_info = trait_method_info(&target.receiver, variant_trait_name, &env.cases_method);
    let qualified_name = method_info.to_mangled_name();
...
pub(super) fn generate_case_bridge_helpers(
    ...
    let mut by_payload: hashmap::IndexMap<String, (TypeId, Vec<(String, u32)>)> =
        hashmap::IndexMap::default();
    ...
    for (mangled_payload, (payload_type, cases)) in &by_payload {
        helpers.push(generate_case_extract_helper(
            case_extract_helper_name(&mangled_variant, mangled_payload),
            variant_type,
            ref_variant_type,
            *payload_type,
            cases,
        ...
        helpers.push(generate_case_construct_helper(
            case_construct_helper_name(&mangled_variant, mangled_payload),
            variant_type,
            *payload_type,
            cases,
            span,
    ...
    if !by_payload.contains_key(&unit_payload) {
        helpers.push(generate_case_construct_helper(
            case_construct_helper_name(&mangled_variant, &unit_payload),
            variant_type,
            TypeTable::UNIT,
            &[],
            span,
...
fn generate_variant_discriminant_fn(
    ...
    let method_info = trait_method_info(receiver, variant_trait_name, discriminant_method);
    let qualified_name = method_info.to_mangled_name();
    let mut function = make_synthetic_method(
        qualified_name,
        method_info,
...
fn generate_enum_reflect_impls(
    ...
    for target in &targets {
        let methods =
            generate_enum_reflect_methods(&module.type_table, &env, enum_trait_name, target);
        generated.extend(methods.into_iter().map(|f| Rc::new(RefCell::new(f))));
...
fn generate_enum_members_fn(
    ...
    let method_info = trait_method_info(&target.receiver, enum_trait_name, &env.members_method);
...
fn generate_newtype_reflect_impls(
    module: &mut TirModule,
    ctx: &mut SynthesisCtx<'_, '_, '_>,
    newtype_trait_name: &FqTraitName,
) {
...
fn generate_flags_reflect_impls(
    ...
    for target in &targets {
        ...
        ctx.record_impl(
            &target.receiver,
            &flags_trait_name.canonical().expect(KEYED),
        );
    }
...
fn generate_flags_members_fn(
    ...
    let method_info = trait_method_info(&target.receiver, flags_trait_name, &env.members_method);
...
fn generate_flags_from_bits_fn(
    ...
    let none = common::option_none(option_flags_type, type_table.borrow().compiler_items());
    ...
    let as_u32 = common::cast(local_expr(0, "raw", TypeTable::U64, span), TypeTable::U32);
    let as_flags = common::cast(as_u32, target.flags_type);
    let some = option_some(
        as_flags,
        option_flags_type,
        type_table.borrow().compiler_items(),
...
fn make_type_param_ids(type_params: &[TirTypeParam], tt: &mut TypeTable) -> Vec<TypeId> {
    type_params
        ...
        .map(|tp| {
            if tp.is_pack {
                tt.make_type_pack(tp.name.clone(), tp.index)
            } else {
                tt.make_type_param(tp.name.clone(), tp.index)
            }
        })
        .collect()
}
...
fn collect_struct_fields(module: &TirModule) -> Vec<(String, Vec<FieldInfo>, Span, StructDef)> {
    module
        .structs
        .iter()
...
fn collect_generic_struct_fields(
    module: &TirModule,
) -> Vec<(String, Vec<TirTypeParam>, Vec<FieldInfo>, Span, DefId)> {
    module
        .structs
        .iter()
        .filter(|s| !s.type_params.is_empty() && s.monomorph_info.is_none())
...
fn collect_variant_cases(module: &TirModule) -> Vec<(String, Vec<VariantCaseInfo>, Span, DefId)> {
    module
        .variants
        .iter()
...
fn collect_generic_variant_cases(
    module: &TirModule,
) -> Vec<(String, Vec<TirTypeParam>, Vec<VariantCaseInfo>, Span, DefId)> {
    module
        .variants
        .iter()
        .filter(|v| !v.type_params.is_empty())
...
fn generate_struct_default_impls(module: &mut TirModule, ctx: &mut SynthesisCtx<'_, '_, '_>) {
    ...
    let infos: Vec<(String, Vec<(String, TypeId, u32, TirExpr)>, Span, StructDef)> = module
        .structs
        .iter()
        .filter(|s| s.type_params.is_empty() && s.monomorph_info.is_none())
        .filter_map(|s| {
    ...
    for (_name, fields, span, def) in &infos {
        ...
        let struct_type = tt.make_struct(*def);
        let func =
            generate_struct_default_fn(receiver, fields, struct_type, &default_trait_name, *span);
        generated.push(Rc::new(RefCell::new(func)));
...
fn generate_struct_default_fn(
    ...
    struct_type: TypeId,
    default_trait_name: &FqTraitName,
    span: Span,
) -> TirFunction {
    ...
    let struct_fields = fields
        .iter()
        .map(|(name, _type, index, value)| TirStructField {
            name: name.clone(),
            value: value.clone(),
            field_index: *index,
        })
...
fn generate_inspect_impls(module: &mut TirModule, ctx: &mut SynthesisCtx<'_, '_, '_>) {
    ...
    let resource_infos: Vec<(String, Span, DefId)> = module
        .resources
        .iter()
