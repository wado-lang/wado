...
fn lower_to_flat_inner(
    ...
    match resolved {
            ...
            if ctx.type_table.borrow().def_name(*def) == names.array && type_args.len() == 1 =>
        {
            ...
            let elem_size = cm_size_with_registry(&elem_ast_type, ctx.cm_interface_registry);
            let elem_align = cm_align_with_registry(&elem_ast_type, ctx.cm_interface_registry);
...
fn push_sync_return_epilogue(
    ...
    if is_unit_return {
        ...
        let size = cm_size_with_registry(ty, env.cm_interface_registry);
        let align = cm_align_with_registry(ty, env.cm_interface_registry);
        let ptr_local = alloc_local(next_local, locals, TypeTable::I32);
        body_stmts.push(let_stmt(
            "$ret_ptr",
            ptr_local,
