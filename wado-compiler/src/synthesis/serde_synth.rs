...
pub fn synthesize_serde(project: &mut Project) {
    ...
    // Generate Deserialize impls for tuple types.
    // Serialize is handled by the variadic generic impl in core:serde (serde.wado),
    // but Deserialize still requires synthesis because the TypePackExpansion +
    // cross-module instantiation pipeline has unresolved issues.
    synthesize_tuple_deserialize(project);
}
...
fn synthesize_tuple_deserialize(project: &mut Project) {
