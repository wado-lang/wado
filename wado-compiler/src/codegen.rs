...
use crate::name::{
    ...
    mangle_array_type, mangle_generic_name,
};
...
struct BuildMainModuleParams<'a> {
    ...
    available_wasi_funcs: &'a IndexSet<String>,
}
...
pub struct Codegen<'a> {
    ...
    struct_types: IndexMap<StructName, StructTypeInfo>,
    /// Registry of tuple types (keyed by element `TypeIds`, maps to GC struct type index)
    tuple_types: IndexMap<Vec<TypeId>, u32>,
    ...
    #[allow(dead_code)]
    closure_struct_types: IndexMap<(u32, u32), u32>,
    ...
    canonical_closure_types: IndexMap<(Vec<TypeId>, TypeId), (u32, String, u32)>,
    ...
    pending_type_indices: IndexMap<String, u32>,
    /// Set of internal Box<T> struct names (generated by boxing lowering pass).
    ...
    internal_box_names: IndexSet<String>,
    ...
    scalarized_funcs: IndexMap<u32, Vec<ValType>>,
}
...
struct FunctionContext {
    ...
    local_type_map: IndexMap<String, ValType>,
    ...
    indirect_call_counters: IndexMap<u32, u32>,
    ...
    scalarized_locals: IndexMap<u32, Vec<u32>>,
...
impl Codegen<'_> {
    ...
    fn build_main_module(&mut self, params: BuildMainModuleParams<'_>) -> Vec<u8> {
        ...
        let mut _import_lookup: IndexMap<String, String> = IndexMap::new();
        ...
        let imported_canonical_names: IndexSet<&str> = entry_tir
            .imports
            .iter()
        ...
        if self.project.has_http_handler_export {
            ...
            builder.define_func_type("http-response-new", &response_new_params, &[]);
            // Also define under WASI alias name so user code can call Response::new()
            builder.define_func_type("wasi:http/Response::new", &response_new_params, &[]);
        }
        ...
        // PHASE 0: Register internal Box<T> structs before anything else.
        // Box<T> structs are generated by the boxing lowering pass for Option<primitive>
        // and &primitive types. They must be registered first because:
        // - Struct fields with Option<primitive> need Box<T> to determine their Wasm type
        // - Closure parameter types may include Box<T>
        // Internal Box<T> structs have a single primitive field (no dependencies).
        let is_internal_box = |s: &crate::tir::TirStruct, tt: &TypeTable| -> bool {
            s.monomorph_info
                .as_ref()
                .is_some_and(|info| info.generic_name == "Box")
                && s.fields.len() == 1
                && matches!(tt.get(s.fields[0].type_id), ResolvedType::Primitive(_))
        };
        for (module_source, tir_mod) in all_tir_modules {
            if module_source == entry_module_source {
                continue;
            }
            let lib_tt = tir_mod.type_table.borrow();
            for tir_struct in &tir_mod.structs {
                if is_internal_box(tir_struct, &lib_tt) {
                    self.internal_box_names.insert(tir_struct.name.clone());
                    let struct_name =
                        StructName::new(entry_module_source.clone(), tir_struct.name.clone());
                    self.register_struct_type(struct_name, tir_struct, &lib_tt, &mut builder);
                }
            }
        }
        // Also check entry module for internal Box structs (single-module tests)
        for tir_struct in &entry_tir.structs {
            if is_internal_box(tir_struct, type_table) {
                self.internal_box_names.insert(tir_struct.name.clone());
                let struct_name =
                    StructName::new(entry_module_source.clone(), tir_struct.name.clone());
        ...
        let mono_struct_names: IndexSet<String> = entry_tir
            .structs
            .iter()
        ...
        for (module_source, tir_mod) in all_tir_modules {
            ...
            // Collect monomorphized structs (skip internal Box<T> already registered in PHASE 0)
            let mono_lib_structs: Vec<_> = tir_mod
                .structs
                .iter()
                .filter(|s| s.monomorph_info.is_some() && !is_internal_box(s, &lib_type_table))
                .cloned()
                .collect();
        ...
        let mono_structs: Vec<_> = entry_tir
            ...
            .filter(|s| {
                s.type_params.is_empty()
                    ...
                    // Skip internal Box<T> - already registered in PHASE 0
                    && !is_internal_box(s, type_table)
            })
            .cloned()
            .collect();
        ...
        // Build a global map of all variant declarations (name -> declaration) across all modules
        // so that GenericInstance types can look up variant declarations regardless of which
        // module defines the variant and which module uses it.
        let all_variant_decls: IndexMap<String, &crate::tir::TirVariantDecl> = {
            let mut map = IndexMap::new();
            for variant in &entry_tir.variants {
                map.insert(variant.name.clone(), variant);
            }
            for (_, tir_mod) in all_tir_modules {
                for variant in &tir_mod.variants {
                    map.entry(variant.name.clone()).or_insert(variant);
                }
            }
            map
        };
        ...
        if self.project.has_http_handler_export {
            ...
            // Also import under WASI alias name for user code (Response::new())
            builder.import_func("wasi", "wasi:http/Response::new");
        }
    ...
    fn generate_component(&mut self) -> Vec<u8> {
        ...
        if self.project.has_http_handler_export && ctx.has_core_func("http-fields-constructor") {
            ...
            // Also export under WASI alias name for user code (Response::new())
            wasi_exports.push((
    ...
    fn generate_wasi_imports(
        ...
        for interface_info in self.project.wasi_registry.interfaces() {
            ...
            // Skip all wasi:http interfaces - they contain resource types (Fields,
            // Request, Response) that the generic loop can't handle. For Service world,
            // import_http_types_for_service handles them specially. For Command world,
            // they aren't needed.
    ...
    fn import_http_types_for_service(
        ...
        let http_types_instance_type = ctx.register_type("http-types-instance-type");
        {
            ...
            // ================================================================
            // Fields and Response resource methods (metadata-driven)
            // ================================================================
            ...
            // Register pre-existing own<response> at type 22
            ...
            // Get Fields and Response instance methods from wasi_registry (excluding
            // constructor and static methods which are handled separately)
            let resource_methods: Vec<WasiFunctionInfo> = self
                .project
                .wasi_registry
                .interfaces()
            ...
            for func in &resource_methods {
                ...
                // Build CM parameter types (names must be kebab-case for CM)
        ...
        // Register the constructor's local alias name so lower_wasi_functions can find it.
        // The constructor alias was already created above as "http-fields-constructor",
        ...
        // Alias Fields and Response resource methods from wasi_registry metadata
        {
            let resource_funcs: Vec<(String, String)> = self
                .project
                .wasi_registry
                .interfaces()
                .find(|i| i.package == "http" && i.interface == "types")
        ...
        // (for resource methods registered in wasi_registry) and
        // the explicit lowering below (for constructor/response.new).
    ...
    fn import_interface_with_resource(
        ...
        // Skip all wasi:http interfaces - they contain multiple resource types
        // (Fields, Request, Response) and are handled by import_http_types_for_service.
        // This generic handler only supports single-resource interfaces (e.g., terminal-stdin).
        if interface_info.package == "http" {
            return;
        }
    ...
    ///
    /// `all_variant_decls` is a global map of all variant declarations across all modules,
    /// enabling cross-module variant lookup (e.g., `Result` defined in prelude but used in entry).
    fn register_monomorphized_variants_from_table(
        &mut self,
        all_variant_decls: &IndexMap<String, &crate::tir::TirVariantDecl>,
        type_table: &TypeTable,
        builder: &mut CoreModuleBuilder,
    ) {
        // Collect all GenericInstance types that are variants
        let mut generic_variants: Vec<(String, ModuleSource, Vec<TypeId>)> = Vec::new();
        ...
        for (base_name, variant_module_source, type_args) in generic_variants {
            // Find the base variant declaration from the global map
            let base_variant = all_variant_decls
                .get(base_name.as_str())
                .expect("variant should exist");

            // Build type parameter substitution map
            let mut type_subst: IndexMap<String, TypeId> = IndexMap::new();
            ...
            let qualified_mangled_name = variant_module_source.qualify_name(&mangled_name);
    ...
    /// Look up the Box<T> struct type index for a primitive type.
    /// Box<T> structs are created by the lower phase and registered as normal structs.
    /// Returns None if no Box struct is registered for this primitive.
    fn get_box_struct_type_idx(
        &self,
        prim: &PrimitiveType,
        _type_table: &TypeTable,
    ) -> Option<u32> {
        let inner_name = match prim {
            ...
            PrimitiveType::I128 | PrimitiveType::U128 => return None,
        };
        let box_name = mangle_generic_name("Box", &[inner_name.to_string()]);
        // Box types are registered under entry_module_source (same as other mono structs)
        let struct_name = StructName::new(self.project.entry_module_source.clone(), box_name);
        self.struct_types
            .get(&struct_name)
    ...
    /// Box<T> types don't need copying - they are GC reference cells used to
    /// implement primitive boxing, and should share the heap object on assignment.
    fn needs_value_copy(&self, type_id: TypeId, type_table: &TypeTable) -> bool {
        match type_table.get(type_id) {
            ResolvedType::Struct { name, .. } => {
                ...
                // Assignment should copy the GC ref (share heap object), not deep-copy.
                // User-defined Box<T> (e.g., Box<Point>) ARE normal structs with value copy.
                !self.internal_box_names.contains(name)
            }
    ...
    fn type_id_to_valtype(&self, type_table: &TypeTable, type_id: TypeId) -> ValType {
        match type_table.get(type_id) {
            ...
            ResolvedType::Option(inner) => {
                // For primitive types, use nullable Box<T> struct reference.
                // The lower phase creates Box<T> structs and transforms expressions,
                // but keeps Option(primitive) in the type table for pattern matching.
                ...
                if let ResolvedType::Primitive(prim) = type_table.get(ultimate_inner)
                    && let Some(box_type_idx) =
                        self.get_box_struct_type_idx(prim, type_table)
                {
                    return ValType::Ref(RefType {
                        nullable: true,
            ...
            // Reference types - after the lower phase, Ref/MutRef of primitives
            // are rewritten to Box<T> struct types. So this branch only handles
            // non-primitive references (structs, arrays, etc.)
            ResolvedType::Ref(inner) | ResolvedType::MutRef(inner) => {
    ...
    fn generate_expr(
        ...
        match &expr.kind {
            ...
            TirExprKind::OptionSome { value } => {
                // Option::Some(value) - the lower phase has already wrapped primitive
                // values in Box<T> struct literals. Just generate the value expression.
            ...
            TirExprKind::VariantConstruct {
                ...
                if variant_name == "Option" {
                    if case_name == "Some" && payload.is_some() {
                        ...
                        // Option::Some(value) - the lower phase has already wrapped
                        // primitive values in Box<T> struct literals. Just generate.
            ...
            TirExprKind::UnwrapOption {
                ...
                // The lower phase wraps this in FieldAccess(.value) for primitive
                // types, so codegen just needs ref.as_non_null.
                self.generate_expr(func, inner, type_table, ctx, builder);
                func.instruction(&Instruction::RefAsNonNull);
            }
            ...
            TirExprKind::Unary { op, expr: inner } => {
                // After the lower phase, all primitive boxing/unboxing is handled
                // as StructLiteral/FieldAccess. Ref/MutRef for address-taken locals
                // are transformed to just Local access. So just generate normally.
                self.generate_expr(func, inner, type_table, ctx, builder);
            ...
            TirExprKind::Assign { target, value } => {
                match &target.kind {
                    TirExprKind::Local { index, .. } => {
                        ...
                        // (After the lower phase, address-taken local assignments are
                        // transformed to FieldAccess(.value) assignments)
                        let adjusted_index = *index + ctx.local_index_offset;
                        self.generate_expr(func, value, type_table, ctx, builder);
                ...
                match base_receiver_type {
                    ...
                    ResolvedType::Struct {
                        ...
                        let final_func_idx = func_idx
                            ...
                            .or_else(|| {
                                // Fallback: after boxing lowering, the receiver type may
                                // be Box<T> but the method is registered under the original
                                // type. Use the FunctionRef's own method_info to find it.
            ...
            TirExprKind::StaticCall {
                ...
                // Use the FunctionRef's module_source - this is set correctly by the resolver
                ...
                // Methods are registered with MethodName format: {module_source}/{struct_name}^{trait}::{method_name}
                let func_idx = if let Some(info) = &method_info {
                    ...
                    let full_name = if module_source.is_entry_point() {
                        ...
                        format!("{}/{}", module_source.to_path_string(), func_name)
                    };
    ...
    fn generate_assignment_as_stmt(
        ...
        match &target.kind {
            TirExprKind::Local { index, .. } => {
                // After the lower phase, address-taken local assignments are transformed
                // to FieldAccess(.value) assignments. This is the simple local.set path.
                let adjusted_index = *index + ctx.local_index_offset;
                self.generate_expr(func, value, type_table, ctx, builder);
    ...
    fn generate_unary_op(
        ...
        match op {
            ...
            TirUnaryOp::Ref | TirUnaryOp::MutRef => {
                // After the lower phase, primitive boxing is handled as StructLiteral(Box<T>).
                // For GC types (structs, arrays, tuples), references are transparent.
                // No operation needed in codegen.
            }
            TirUnaryOp::Deref => {
                // After the lower phase, primitive dereferences are handled as
                // FieldAccess(.value) on Box<T>. For non-primitive references,
                // dereferencing is transparent. No operation needed in codegen.
            }
        }
    }
    ...
    fn allocate_copy_source_locals(
        &self,
        copy_source_types: &IndexSet<TypeId>,
        type_table: &TypeTable,
        ctx: &mut FunctionContext,
    ) {
    ...
    fn wasi_func_to_core_params(&self, func: &WasiFunctionInfo) -> Vec<ValType> {
        ...
        for (_, ty) in &func.params {
            // Resolve newtypes (e.g., FieldName -> String) before converting to ValType
            let resolved_ty = self.project.wasi_registry.resolve_type(ty);
            flatten_wasi_param_type(&resolved_ty, &mut params);
        }
    ...
    fn build_memory_module(&self, strip_names: bool) -> Vec<u8> {
        ...
        // Global section: bump pointer for realloc (starts at 1024 to avoid offset 0)
        ...
        // Code section: bump-allocator realloc
        // Signature: (old_ptr: i32, old_size: i32, align: i32, new_size: i32) -> i32
        // Ignores old_ptr/old_size (no deallocation). Aligns the bump pointer up to
        // `align`, advances it by `new_size`, and returns the aligned pointer.
        let mut code = CodeSection::new();
        let mut realloc_func = Function::new([(1, ValType::I32)]); // local 4: aligned ptr
        // ptr = (bump + align - 1) & ~(align - 1)
        realloc_func.instruction(&Instruction::GlobalGet(0)); // bump
        realloc_func.instruction(&Instruction::LocalGet(2)); // align
        realloc_func.instruction(&Instruction::I32Add);
        realloc_func.instruction(&Instruction::I32Const(1));
        realloc_func.instruction(&Instruction::I32Sub); // bump + align - 1
        realloc_func.instruction(&Instruction::I32Const(0)); // 0
        realloc_func.instruction(&Instruction::LocalGet(2)); // align
        realloc_func.instruction(&Instruction::I32Sub); // 0 - align = ~(align - 1) for power-of-2
        realloc_func.instruction(&Instruction::I32And); // aligned ptr
        realloc_func.instruction(&Instruction::LocalSet(4)); // save aligned ptr
        // bump = ptr + new_size
        realloc_func.instruction(&Instruction::LocalGet(4));
        realloc_func.instruction(&Instruction::LocalGet(3)); // new_size
        realloc_func.instruction(&Instruction::I32Add);
        realloc_func.instruction(&Instruction::GlobalSet(0)); // update bump
        // return aligned ptr
        realloc_func.instruction(&Instruction::LocalGet(4));
