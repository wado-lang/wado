...
impl Codegen<'_> {
    ...
    fn build_main_module(&mut self, params: BuildMainModuleParams<'_>) -> Vec<u8> {
        ...
        // PHASE 2.7: Register internal Box<T> structs before closure types.
        // Box<T> structs are generated by the boxing lowering pass and may appear as
        // closure parameter types (e.g., fn(Box<i32>) -> i32). They must be registered
        // before canonical closure types, which call type_id_to_valtype for all params.
        // Internal Box<T> structs have a single primitive field (no dependencies).
        // We distinguish internal Box<T> from user-defined Box<T> by checking that the
        // single field type is a primitive (internal) vs. a complex type (user-defined).
        ...
        for (module_source, tir_mod) in all_tir_modules {
            ...
            // Collect monomorphized structs (skip internal Box<T> already registered in PHASE 2.7)
        ...
        let mono_structs: Vec<_> = entry_tir
            ...
            .filter(|s| {
                s.type_params.is_empty()
                    ...
                    // Skip internal Box<T> - already registered in PHASE 2.7
