...
fn capture_field_value(cap: &TirCapture, enclosing_self: EnclosingSelf, span: Span) -> TirExpr {
    match cap.source {
        CaptureSource::Local(index) => TirExpr::new(
            TirExprKind::Local {
                index,
                name: cap.name.clone(),
            },
...
/// Build the `$capture_<i>` struct-field list for a functor literal from the
/// closure's captures.
fn build_capture_fields(
    captures: &[TirCapture],
...
impl ClosureLowerer {
    ...
    fn generate_specialized_function(
        ...
        let specialized_func = TirFunction {
            ...
            immediates: callee.immediates.clone(),
            trap: callee.trap.clone(),
            linear_memory: callee.linear_memory,
            body: new_body,
            span: callee.span,
        ...
        for arg_idx in &raw_indices {
                ...
                && let Some(functor) = self.functor_infos.get(*closure_id as usize)
            {
                arg.expr.kind = TirExprKind::StructLiteral {
                    ...
                    fields: build_capture_fields(captures, self.enclosing_self, arg.expr.span),
                };
