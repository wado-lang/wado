...
impl<H: CompilerHost> Elaborator<'_, H> {
    ...
    fn case_of_callee(
        ...
        let owner = match callee_kind {
            ...
            CalleeIdentKind::AbstractTypeParam { .. } => return None,
        };
    ...
    pub(super) fn resolve_call_with_args(
        ...
        {
            if let Some(sig) = self.tysys.as_fn_signature(value_ty) {
        ...
        if !matches!(&call.callee, Expr::Ident(_)) {
            ...
            if let Some(sig) = self.tysys.as_fn_signature(callee_type) {
            ...
            effective_name.find("::")
        {
            ...
            // registered with mangled names "Type::method".
                ...
                for (i, arg) in args.iter().enumerate() {
                    ...
                    if let Some(&expected) = substituted.get(i)
                        && !self.tysys.is_unbound_type_param(expected)
                    {
            ...
            if self.record_reference_to_decl(ident.id, op.op, ident.span) {
                return TypeTable::ERROR;
            }
            (
                Some(self.effect_operation_callee(op)),
            ...
            .filter(|def| self.tysys.resolutions.defs().kind(*def) == DefKind::Function)
        {
            ...
            (
                Some(self.tysys.callee_of(callee)),
                effective_name.to_string(),
            )
        }
        ...
        let reached = self.tysys.packs_callee_args_reach(&callee, args.len());
    ...
    pub(super) fn infer_fn_type_args(
        ...
        let (type_param_list, resolved_param_types, decl_return_type) = if let Some(v) = cached {
            ...
            let Some(info) = self.tysys.lookup_generic_func_for_inference(callee) else {
                return vec![];
            };
            info
        };
...
impl TypeSystem {
    ...
    fn lookup_generic_func_for_inference(
        ...
        let sig = self.signatures.function_sig(callee.def()?)?;
    ...
    fn expected_variant_args(
        ...
        (*def == table.defs().def_at(variant_info.defined_at)
    ...
    fn variant_slot_hints(
        ...
        (0..variant_info.type_param_type_ids.len())
            .map(|i| match written.get(i) {
                ...
                _ => from_expected
                    .as_ref()
                    .map_or(TypeTable::UNKNOWN, |args| args[i]),
            })
            .collect()
    }
    ...
    pub(super) fn infer_variant_type_args(
        ...
        if let Some(expected_args) = self.expected_variant_args(variant_info, expected_type) {
            ...
            }
        {
                ...
                && !method_info_result.method_type_param_ids.is_empty()
            {
                ...
                for (i, (param_type, arg)) in method_info_result
                    ...
                    .enumerate()
                {
                    if answers_last(call.args.get(i)) {
                        infer.add_deferred(*param_type, *arg);
                    } else {
                        infer.add(*param_type, *arg);
                    }
