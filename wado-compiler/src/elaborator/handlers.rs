...
impl<H: CompilerHost> Elaborator<'_, H> {
    ...
    pub(super) fn resolve_with_handler(
        ...
        // block's value (the shared block-result rule handles every tail
        ...
        // the body's tail is `Unit`, so this is unchanged. Read from
        // `expression_types` (AST level) so it does not depend on the body TIR.
