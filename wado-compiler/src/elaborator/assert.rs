...
impl<H: CompilerHost> Elaborator<'_, H> {
    pub(super) fn desugar_assert(
        ...
        // The synthetic locals scope normally inside reify's expansion; here
        // we only need a matching enter/exit pair so the assert-capture hook's
        // `add_local` calls don't leak into the surrounding scope.
        ctx.enter_scope();
        ...
        // which is the only signal reify needs (the recorded slot table tells
        // it which `__vK` bindings to materialise).
        let _ = self.resolve_expr(&assert_stmt.condition, ctx, None);
