...
impl Body {
    ...
    /// Visit every operand slot of a single expression node.
    ///
    /// The read-only counterpart of [`Self::map_expr_operands`]. `for_each_child`
    /// reaches only the operands that are still skeleton expressions; a promoted
    /// `Operand::Value` is invisible to it, so a walk that needs to see promoted
    /// values — rather than rewrite them — comes here.
    pub fn for_each_expr_operand(&self, id: ExprId, f: &mut impl FnMut(Operand)) {
