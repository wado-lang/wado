        ...
        let (callee_module_source, func_name, is_known) = match &call.callee {
            Expr::Ident(ident) => {
                ...
                if let Some(pos) = ident.name.find("::") {
                    ...
                    {
                        // suffix may contain :: for ns::Type::method or ns::Variant::Case
                        if let Some((type_name, case_or_method)) = suffix.split_once("::") {
                            // Check for variant construction: ns::Shape::Circle(5.0)
                                ...
                                && let Some(variant_info) = module_variants.get(type_name)
                            {
                                ...
                                if let Some((case_index, case_data)) = variant_info
                                    ...
                                    .iter()
                                    .enumerate()
                                    .find(|(_, c)| c.name == case_or_method)
                                    .map(|(i, c)| (i, c.clone()))
                                {
                                    let payload_is_unit = matches!(
                                        self.type_table.borrow().get(case_data.payload),
                            ...
                            // Static method call: ns::Type::method(...)
                            // Construct a StaticMethodCallExpr and delegate to existing resolution.
                            let static_call = ast::StaticMethodCallExpr {
                                target_type: ast::Type::Named(ast::NamedType {
                                    name: format!("{prefix}::{type_name}"),
                                    span: call.span,
                                }),
                                method: case_or_method.to_string(),
                                args: call.args.clone(),
                                has_trailing_comma: false,
                                span: call.span,
                            };
                            return self.resolve_static_method_call(&static_call, ctx)
                        } else {
                            // Simple ns::function
