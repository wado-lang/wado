...
/// If `body` is a single `Call` to a named function whose arguments are
/// the closure's parameters in declaration order (and no captures
/// participate), the closure is the synthetic forwarder produced by
/// `FuncRefToClosureRewriter` for a bare `&fn_name` expression. Return
/// the user-readable form (`&fn_name`); otherwise return `None`.
fn render_func_ref_closure(
    ...
    let TirExprKind::Call { func, args, .. } = &body.kind else {
        return None;
    };
    ...
    for (i, arg) in args.iter().enumerate() {
        let TirExprKind::Local { index, .. } = &arg.expr.kind else {
            return None;
        };
