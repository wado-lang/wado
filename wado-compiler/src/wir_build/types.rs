...
pub fn register_types(ctx: &mut WirContext<'_>) {
    ...
    // Phase 2.1: WASI structs from registry that weren't loaded as TIR modules
    // (e.g., Instant from wasi:clocks used inside DescriptorStat from wasi:filesystem)
    register_wasi_registry_structs(ctx);
...
/// Register WASI structs from the registry that appear in the type table but don't
/// have corresponding TIR struct declarations (e.g., Instant from wasi:clocks when
/// only wasi:filesystem is loaded but `DescriptorStat` references Instant).
fn register_wasi_registry_structs(ctx: &mut WirContext<'_>) {
    let registry = ctx.project.wasi_registry;
    let tt = ctx.project.entry_module().type_table.clone();
    let type_table = tt.borrow();

    // Collect WASI struct types that need registration
    let mut needed: Vec<(String, ModuleSource)> = Vec::new();
    for type_id in type_table.iter_type_ids() {
        if let ResolvedType::Struct {
            name,
            module_source: module_source @ ModuleSource::Wasi { .. },
            ..
        } = type_table.get(type_id)
        {
            let struct_name = StructName::new(module_source.clone(), name.clone());
            if !ctx.struct_type_map.contains_key(&struct_name)
                && ctx.lookup_struct_by_name(name).is_none()
                && registry.is_struct(name)
            {
                needed.push((name.clone(), module_source.clone()));
            }
        }
    }

    for (name, module_source) in &needed {
        if let Some(wado_fields) = registry.get_struct_fields_with_wado_names(name) {
            let fields: Vec<WirField> = wado_fields
                .iter()
                .filter_map(|(wado_name, _, field_ty)| {
                    let ty = wasi_ast_type_to_wir_type(field_ty, ctx, &type_table, registry);
                    if matches!(ty, WirType::Unit) {
                        return None;
                    }
                    Some(WirField {
                        name: wado_name.clone(),
                        ty: ty.as_nonnull(),
                        mutable: true,
                    })
                })
...
/// Convert a WASI AST type to a WIR type for struct field registration.
fn wasi_ast_type_to_wir_type(
    ty: &crate::ast::Type,
    ctx: &WirContext<'_>,
    type_table: &TypeTable,
    ...
    match ty {
        crate::ast::Type::Named(named) => match named.name.as_str() {
            ...
            "char" => WirType::Char,
            "String" => {
                let sn = StructName::new(ModuleSource::string(), "String".to_string());
                if let Some(tid) = ctx.struct_type_map.get(&sn) {
                    WirType::Ref {
                        type_id: tid.clone(),
                        nullable: false,
                    }
            ...
            other => {
                // Check if it's a resolved type in the type table (enum, newtype, etc.)
                if let Some(type_id) = type_table
                    .find_enum_type_by_name(other)
                    .or_else(|| type_table.find_resource_type_by_name(other))
                {
                    ctx.type_id_to_wir_type(type_table, type_id)
                } else if registry.is_struct(other) {
                    // Another WASI struct — look it up
                    if let Some(tid) = ctx.lookup_struct_by_name(other) {
                        WirType::Ref {
                            type_id: tid.clone(),
                            nullable: false,
                        }
                ...
                } else if let Some(resolved) = registry.get_newtype(other) {
                    wasi_ast_type_to_wir_type(resolved, ctx, type_table, registry)
                } else {
                    WirType::I32
                }
            }
        },
        crate::ast::Type::Generic(g) => match g.name.as_str() {
