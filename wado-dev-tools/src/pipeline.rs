...
async fn compile_one(
    ...
    .await
    .map_err(|_| diagnostics_summary(&host, "compilation failed"))?;
    ...
    match phase {
        Phase::Wir => {
            ...
            let unparsed = wado_compiler::wir_unparse::unparse_wir(wir_package, Some(input_path));
