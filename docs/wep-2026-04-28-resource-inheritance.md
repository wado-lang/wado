...
### Linearity: two disciplines, pilot the unrestricted one through the web bindings
...
| `unrestricted` | copyable, owning nothing and dropping nothing | WebIDL-derived resources (`wado-lang:web`) |
...
This makes the web bindings the right pilot for unrestricted resources. The blast radius is contained: if the discipline proves wrong, WIT/WASI is untouched.
...
**v1 is invariant in both**, and the rest of this section is the direction, not the state. Invariance only rejects programs subtyping would accept, and no consumer writes a resource-typed function parameter yet: the web bindings' callbacks reach the host through a key, not a Wado function type ([Web § Callbacks](./wep-2026-04-01-web.md#callbacks)).
...
- Every real use site destructures the result immediately. A method returning
  `Option<T>` builds an aggregate only to take it apart on the next token, and the
  examples in [The Web Interface for Wado](./wep-2026-04-01-web.md) are all that
  shape.
...
This WEP is the full feature. The order it lands in is [Web § Minimum Implementation Roadmap](./wep-2026-04-01-web.md#minimum-implementation-roadmap), the one place milestones are sequenced.
...
- The registry reads that newtype two ways. `resolve_type` peels the handle to its `f64`, the view the boundary decides with: the flat ABI, the canonical options, the emitted WIT. `value_type` keeps the resource's own type, the view the guest holds. A binding's signature takes the second, because `Option<Element>` and `Option<f64>` are distinct GC types (`tests/integration/web_dom.rs`). A module outside the stdlib declares such a resource as the stdlib would: it is not a local newtype, so no instance exports it as a named type.
...
Nothing inside the component target reaches it. An `externref`-typed host import belongs to a core module, which is how `wasm-bindgen` reaches the same APIs. Targeting a core module would give up the CM machinery the web target rides on, including the jco transpile path [the web bindings](./wep-2026-04-01-web.md) assume.
...
- [The Web Interface for Wado](./wep-2026-04-01-web.md) — the primary consumer of this feature
