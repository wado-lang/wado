...
draws no conclusions about Wado. [WEP: Loam](./wep-2026-09-20-loam.md) draws
them.
...
The field is usually discussed as one category. It holds at least seven kinds,
which differ in _when_ the computation graph is fixed and _who_ writes the
kernels.
...
The two kinds with the weakest compiler stories are the runtime graph and the
load-time optimizer. They have the largest deployments. Section 3 covers why.

---

## 3. llama.cpp and ggml

llama.cpp is a runtime graph interpreter driving hand-written kernels. It sits at
the opposite end of the space from the compiler approach.
...
The technical center is elsewhere. It is the quantization formats (k-quants,
i-quants, importance-matrix-driven quantization) and the deployment story, where
GGUF gives a single mmap-able file, no dependencies, and one binary.

### Why the Weaker Compiler Story Won

Batch-1 decode is memory-bandwidth bound and has low arithmetic intensity. In
that regime hand-written kernels land near the achievable ceiling, so fusion for
FLOPs and bandwidth, which is what compilers optimize hardest, has little left to
take. TVM and IREE are better engineered as compilers and were adopted far less.

Section 5 complicates this. On WebGPU the batch-1 ceiling is set by dispatch
overhead instead, and dispatch count _is_ something only a compiler can collapse.
...
support many quantization formats from one source. That last one is the
"quantization format as a kernel parameter" idea, written out as hand-authored
templates.

Portability no longer costs an order of magnitude. The common assumption that
WebGPU is for reach and native is for speed is too pessimistic as of 2026.
...
(2026-08) shows that naive measurements conflate dispatch cost with
synchronization cost, then introduces a sequential-dispatch measurement method
...
dispatch amortization, both in inference engines and in the WebGPU specification
itself. Part of the ceiling is therefore not under an engine author's control.
...
Operators chain through shared memory and registers instead of round-tripping
through HBM.
...
### WebGPU Cannot Host a True Megakernel
...
expressed inside one dispatch, so a persistent kernel spanning the whole model is
not implementable.

The reachable target on WebGPU is fewer and larger dispatches, collapsing a layer
or a fused block into one, rather than one dispatch per token. The CUDA speedups
above should not be assumed to transfer.
...
(Suwa and Igarashi, **ECOOP 2026**) starts from why existing approaches have not
been adopted. Dependent types (`Mat m n`) and refinement types both require
_proofs_ to decide type equality or subtyping, which is unwieldy under continuous
development. Automated proving does not fix that, because its running time is
unpredictable.

The alternative rests on an observation: the conditions for shape consistency can
usually be extracted before the tensor computation runs. So the formalism uses
staging, and discharges shape consistency through **assertions evaluated as
...
`[b]` says the dot product runs over `b`. The axes `a` and `c` are merely
vectorized over. Unlike `einsum`, axis names are self-documenting and the style
is declarative rather than imperative.

The practical state of the art is therefore a string DSL, with static checking
added from outside. `pyrefly-einops-stubs` provides shape-aware annotations for
`rearrange`, `reduce`, `repeat`, and `einsum` through a type-level shape DSL.

Named axes as a first-class language feature have not stuck. PyTorch named
tensors shipped and went quiet, `xarray` thrives in a different niche, and
...
The pattern worth recording is that these formats are not a fixed list of names.
They are a moving target parameterized by _block size_, _scale representation_,
and _whether a rotation is applied_.
...
than shipping a separate frontend, and compiles kernels just in time, generating
only the variants actually launched. Its stated reason is that ahead-of-time
compilation must cover every combination of shape, hardware target, and
...
[HetGPU](https://arxiv.org/html/2506.15993v1) takes the other route, with a
compiler, runtime, and abstraction layer that let a single GPU binary run on
NVIDIA, AMD, Intel, and Tenstorrent.

CubeCL's reason for choosing JIT is a caution that outlives its choice of
abstraction. Fully ahead-of-time specialization across shapes and targets does
not scale, so generating only the variants actually used is a requirement rather
than an optimization. That agrees with the staging approach in §7.

### Effects Are Unexplored Here

No example was found of using algebraic effects or effect handlers to abstract
the compute device. Effect-handler research in 2025-2026 is active, covering
tracing JIT compilation for effects and handlers, control-flow analysis for
handlers, and staging effect handlers for modular search.
[Continuing WebAssembly with Effect Handlers](https://arxiv.org/pdf/2308.08347)
covers the Wasm substrate. The intersection with device abstraction appears
untouched.
...
whole model to the host rather than describing how kernels are produced, which
makes it an alternative delivery path rather than a competitor to a compiler
stack.
...
1. **WebGPU's dispatch ceiling is partly a specification problem.** The paper in
   §5 recommends amortization in the spec itself. Engine-side work has a limit
   that no implementation can pass.
