...
> The typed-CST + Visitor design sketched in this section was the original
> plan. The shipped generator instead emits an error-resilient, untyped
> `CstNode` tree (see [Progress](#progress) and `package-gale/README.md`); the
> example below is kept as the historical design record.
...
`CstNode` / `CstChild` tree (`ParseResult { root, tokens, diagnostics }`), left
...
- [x] Error-resilient parse tree: untyped `CstNode` / `CstChild` +
      `ParseResult { root, tokens, diagnostics }`, with local recovery
...
- [x] `gale dump [--atn]` prediction/automaton inspector and opt-in `trace`

### Remaining

- [ ] Recognize-and-skip target-language elements (action blocks, semantic
      predicates, `@header`/`@members`)

Further open work is tracked in `package-gale/TODO.md`.
