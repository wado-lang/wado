...
5. **Lexer function** — `fn tokenize(input: &String) -> Array<Token>`
...
The runtime is a single `runtime.wado` source file inside the gale project. At code generation time, gale reads its own runtime source and emits it verbatim into the generated file using `#include_str`:

```wado
// src/codegen.wado (inside gale's own source)

let RUNTIME_SRC: String = #include_str("./runtime.wado");

fn emit_header(out: &mut String, grammar_name: &String, gale_version: &String) {
    out.append(`// Generated by gale v{gale_version} from {grammar_name}.g4 — do not edit\n`);
    out.append("// === Inlined gale runtime ===\n");
    out.append(RUNTIME_SRC);
    out.append("// === End inlined runtime ===\n\n");
}
```

This requires the `#include_str` language feature (see [WEP: Compile-Time File Inclusion](./wep-2026-03-02-include-str.md)).
...
The runtime itself (`src/runtime.wado`) is plain Wado and can be tested independently with `wado test`, since it has no dependencies on gale's generator code.
...
package-gale/
  ...
  src/
    ...
    codegen.wado         ← GrammarIR → .wado source string (inlines runtime)  [planned]
    runtime.wado         ← Span, Token, ParseError (standalone, unit-testable)
...
ParseError      { message: String, span: Span, expected: Array<String> }
```
