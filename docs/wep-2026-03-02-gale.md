...
1. **Inlined runtime** — `Span`, `TokenStream`, `Diagnostic`, the CST store, etc.
2. **Token kinds** — one `TK_<name>` constant per lexer rule plus the synthetic `TK_EOF` and `TK_ERROR`
