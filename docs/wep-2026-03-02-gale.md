...
The generator emits an error-resilient, untyped flat CST (`CstStore`), not a
typed node per rule and a visitor over it. That typed shape was the original
plan and was not shipped. For everything a generated parser exports, see
...
3. **Node kinds** — one `RK_<RULE>` constant per parser rule, and a `<Rule>Alt` enum for a rule with labeled alternatives
4. **Lexer function** — `tokenize(input) -> TokenStream`
5. **Parser functions** — `parse` and one `parse_<rule>` per parser rule, each returning a `ParseResult`
