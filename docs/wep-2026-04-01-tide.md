...
Host re-entry is forbidden only on the current task's chain
([Research: Callbacks across the CM Boundary](./research-cm-boundary-callbacks.md)).
An event-loop callback is legal, even while `run` is suspended. One invoked
_during_ an import call, such as a sort comparator, traps, so v1 binds deferred
callbacks only.

Known gaps:

- `dispatch_event` in a browser runs the listeners during the import call, so a
  program dispatching an event to its own listener traps there. `SurfaceDom`
  runs the listener.
...
| Handle release                         | any loop that receives handles         | v1 leaks one table slot per handle the program ever receives, not per handle it keeps        |
