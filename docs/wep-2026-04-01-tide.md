...
- Host runtime (jco or similar) must provide `instanceof` checks for narrowing.
...
- [ ] Glue generator over the same IR: per-interface method shims over a handle table that interns each object and tags it with its nearest class in the slice
- [ ] jco transpile with the web imports mapped to the glue (`scripts/jco`, `mise run jco-*`)
- [ ] CI gate: Node run against a minimal DOM stub, which checks the glue alone; manual browser check for the same program
