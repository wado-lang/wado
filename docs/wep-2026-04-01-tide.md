...
One handler serving several event types is the shape the pattern form is for — a
`match`, flat, with the `_` an open hierarchy requires:
...
- [ ] Pattern ascription (`p: T`) in the grammar, absorbing the `let` annotation slot so one construct serves both
- [ ] Refutability from the subtype relation: irrefutable where `S <: T`, refutable where `T` is a strict subtype, an error otherwise; generic `T` rejected
- [ ] `match` over type patterns: `_` required, and a dead arm shadowed by an earlier ancestor reported
- [ ] Per-target `is-T` import synthesis; a refutable pattern lowers to that call and a branch
