...
snapshot changes together, so it lives together: the bindings, the fake DOM a
server-side program runs against, and the browser glue. The glue tags each
...
└── src/
    ...
    ├── fake_dom.wado       # FakeDom — a DOM held in memory, answering the Dom effect
...
- [x] A fake DOM in `package-web` (`FakeDom`), enough for the MVP slice: a tree, text, lookup by id, an input's value

### M7 — Server-side rendering under `wado run` / `wado serve`

- [ ] The fake DOM serializes to HTML
