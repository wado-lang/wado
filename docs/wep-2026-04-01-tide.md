...
- [x] Per-target `is-T` import synthesis; a refutable pattern lowers to that call and a branch
- [x] `Eq` auto-derived through `is-same`
...
- [x] IR and codegen: `extends`, `#[cm(..., linearity = "unrestricted")]`, `#[cm_params]`
...
- [ ] `is-T` and `is-same` are declared in `web:dom`, so a handler answers narrowing and `==`
