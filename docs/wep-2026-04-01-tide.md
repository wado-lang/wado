...
The CM interfaces keep the `web:` namespace (`web:dom/node`): that is what the
component imports and what a host implements, whichever package declares them.
...
The examples further down illustrate shape, not syntax: they use a shorthand path (`#[cm("web:dom#Element")]`), leave `linearity = "unrestricted"` implicit, and mark WebIDL attributes with `getter` / `setter`. Read the declaration above for the form the generator emits and the compiler accepts — `extends` requires the linearity on both sides.
...
- The released-jco transpile maps each `web:<package>/*` import to the
  package's glue. The glue an output imports is copied beside it as
  `web-<package>.js`.
...
└── src/
    ...
    └── ...                 # fetch.wado, canvas.wado, one module per spec package
```
...
- [x] Import synthesis for `web:*` interfaces a package declares, not the stdlib
...
No Wado host is a browser. `wado test`, `wado run` and `wado serve` define every `web:*` import as a trap, so a program instantiates wherever it runs, and a Wado effect handler answers each call it makes: `with &mut dom do { app(); }`. The program is the same one the browser runs; only its entry installs the handler. A call no handler answers traps when made, not at instantiation.

### M6 — Handler-served DOM under `wado test`

- [x] The host fills every `web:*` import with a trap
