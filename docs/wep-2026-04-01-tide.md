...
- [ ] Keyword, AST, parser; single parent, parent must be a `resource`, cycles rejected
- [ ] Subtype relation and implicit upcast at call args / return / annotated `let` / parent-typed field assignment / branch unify
- [ ] Method resolution over the chain, plus the resolved corner cases (override forbidden, trait-vs-inherited collision, statics do not inherit, `Self` fixed at the declaring resource, visibility judged at the declaring module)
- [ ] Backing validation: `extends` requires `type="extern-ref"` on both sides
