...
come from one generation run. The web platform also moves at the pace of its specs, not
the compiler's.
...
- In a browser, the glue does nothing. The exception unwinds the Wado frames,
  which nothing in Wado catches, and reaches the export's JS caller unchanged.
  The Component Model makes an exception that reaches a component boundary a
  trap (`call_and_trap_on_throw`), so it cannot be caught in between.
