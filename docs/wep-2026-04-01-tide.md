...
Release is the open end: v1 keeps the registry entry for the life of the instance — the honest equivalent of `.forget()`. The way out is a guest-exported `callback` resource whose drop releases the entry, and eventually CM first-class function values (Concurrency.md future extensions), which retires the trampoline entirely.
