...
The member is a bare name, unlike the `[constructor]` / `[method]` / `[static]` prefixes a WASI binding carries: those name a CM `resource`, and an extern-ref interface declares none — the receiver is the universal handle, passed as an ordinary first parameter. A component whose interface exports a `[method]x.y` name it declares no resource for fails validation.

`type = "extern-ref"` selects the handle backing (see Resource Inheritance). No `getter` / `setter` sub-attributes are needed: Tide owns the JS glue, so a WebIDL attribute is emitted as an ordinary getter/setter method pair, and a WebIDL constructor as a bare `new`.

The examples further down illustrate shape, not syntax: they use a shorthand path (`#[cm("web:dom#Element")]`), leave `type = "extern-ref"` implicit, and mark WebIDL attributes with `getter` / `setter`. Read the declaration above for the form the generator emits and the compiler accepts — `extends` requires the backing on both sides.
...
### Premise: the MVP does not wait for CM-GC

Every extends-participating type collapses to one CM type at the boundary, so v1 lowers that type to an opaque, copyable `u32` — an index into a host-side table owned by the JS glue. The same declarations flip to a real `externref` when CM-GC lands ([GC in Components](./wep-2026-03-28-gc-in-components.md)), with no user source change; the interim cost is that a handle is never released (see Deferred).
...
- [x] Validate the value (`"extern-ref"` | `"i32"`) and the placement (a `resource`, since the backing is a property of the handle type)
- [x] `type = "extern-ref"` makes the resource copyable and exempt from the affine analysis of [Resource Ownership](./wep-2026-05-21-resource-ownership.md), and from the cleanup pass that would otherwise drop it
...
- [x] Backing validation: `extends` requires `type = "extern-ref"` on both sides
...
Where M2 left off: an extern-ref resource still lowers as an `i32` CM resource, so a program using `extends` type-checks but its CM surface is not yet this shape. The lowering path is `elaborator/cm_binding` and `component_model.rs`; the backing fact is `TypeTable::is_extern_ref_resource`, the relation `TypeTable::resource_parent`.

- [ ] `extern-ref`-backed resource → the universal handle at the CM boundary; upcast and the inherited-method receiver are no-ops
- [ ] One WIT interface per Wado type, receiver typed `extern-ref`
- [ ] Import synthesis for a non-WASI package: verify the world and import plumbing accepts `web:*`
- [ ] E2E in `tests/integration/`: a two-level `extends` program against a wasmtime host stub

### M4 — Downcast and `Eq`

- [ ] `downcast::<T>()` on extern-ref-backed resources: strict-subtype target check, generic `T` rejected
- [ ] Per-type `is-T` import synthesis; lowering branches into `Some(self)` / `None`
- [ ] `Eq` auto-derived through `is-same`

Out of MVP: `Inspect`, `InspectAlt`, `Display` host imports; the `serde`-is-an-error rule.

### M5 — WebIDL frontend, minimum slice

- [ ] `@webref/idl` JSON ingestion alongside the WIT frontend, sharing the IR (`wado-from-idl`)
- [ ] IR and codegen: `extends`, `#[cm(..., type = "extern-ref")]`, `#[cm_params]`
...
| Handle release                         | long-lived pages                 | v1 leaks one table slot per handle; CM-GC removes the table                   |
...
| CM-GC switch to a real `externref`     | performance, release             | Same Wado source; lowering-only change                                        |
