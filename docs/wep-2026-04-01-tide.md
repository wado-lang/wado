...
`mise run test-web-glue` runs `example/web-browser` on Node against a DOM stub,
which checks the glue and the transpile alone. The same program is checked in a
browser by hand, with `mise run web-browser-demo`.

Known gap: CI runs no browser, so a difference between the stub and a browser's
DOM shows only in the hand check.
...
- [x] CI gate: Node run against a minimal DOM stub, which checks the glue alone; manual browser check for the same program
