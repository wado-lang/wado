...
Tide is written in Wado itself, reusing the `wadopoet` code generation library from package-gale. It runs as a CLI command:

```sh
wado tide generate --input ./webidl-json/ --output ./web/
```

Pipeline:

```
@webref/idl (JSON) → Parse → IR (interfaces, types, inheritance, mixins) → WadoPoet codegen → web/*.wado
```
...
- `wadopoet` from package-gale for code generation.
