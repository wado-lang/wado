...
server-side program runs against (`SurfaceDom`), and the browser glue. The glue tags each
handle with the class numbers the bindings declare, so the two must come from
one generation run. The web platform also moves at the pace of its specs, not
the compiler's.
