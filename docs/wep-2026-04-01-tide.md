...
Web bindings use the grammar WASI bindings already use — `namespace:package/interface[@version]#member`, with a `[constructor]` / `[method]` / `[static]` prefix on the member. Web packages carry no version, and each Wado resource gets its own interface (the universal-receiver lowering in [Resource Inheritance](./wep-2026-04-28-resource-inheritance.md)):

```wado
#[cm("web:dom/element", type = "extern-ref")]
pub resource Element extends Node {
    #[cm("web:dom/element#[method]element.get-attribute")]
    #[cm_params("self", "qualified-name")]
    fn get_attribute(&self, qualified_name: String) -> Option<String>;

    #[cm("web:dom/element#[method]element.id")]
    #[cm_params("self")]
    fn id(&self) -> String;

    #[cm("web:dom/element#[method]element.set-id")]
    #[cm_params("self", "value")]
    fn set_id(&self, value: String);
}
```

`type = "extern-ref"` selects the handle backing (see Resource Inheritance). No `getter` / `setter` sub-attributes are needed: Tide owns the JS glue, so a WebIDL attribute is emitted as an ordinary getter/setter method pair, and a WebIDL constructor as `[constructor]`.

The examples further down this WEP predate this decision and use a shorthand (`#[cm("web:dom#Element")]`); the generator emits the form above.
...
Two limits fall out of the engine rules, not the design ([Research: Callbacks across the CM Boundary](./research-cm-boundary-callbacks.md)). Host re-entry is forbidden only on the current task's chain, so an event-loop callback is legal while one invoked _during_ an import call (a sort comparator) traps — v1 binds deferred callbacks only. And an async-lifted export returns a promise to JS, so callback returns are `()`; `preventDefault` is an ordinary import call from inside the callback.

Release is the open end: v1 keeps the registry entry for the life of the instance — the honest equivalent of `.forget()`. The way out is to give the callback a resource type and let CM's ownership discipline drive the release, the shape [component-model#278](https://github.com/WebAssembly/component-model/issues/278) proposes and the shape a Wado closure already has (an environment plus one `__call`). The resource has to sit on the **host** side: an elaborated world requires every type an import refers to to be imported (`wit-parser`, `resolve/mod.rs`), so a guest-exported resource cannot appear in `add-event-listener`'s signature. So the callback is an imported resource — or just another `extern-ref` — the guest mints it from the key, `own` transfers it to the listener, and the glue's destructor calls a synthesized `release-K(key)` export. The guest-side registry survives either way: a resource `rep` is an `i32`, not a GC reference (`Explainer.md`, `resource.new`). CM first-class function values retire the trampoline; nothing before that does.
...
The MVP is one vertical slice that runs in a browser: get an element by id, read and write its text, downcast it to `HtmlInputElement`, read its value. Everything off that path is deferred (last section). Each milestone lands compiling and running; none leaves the tree in a "compiles but does not run" state.

### Premise: the MVP does not wait for CM-GC

`extern-ref` backing is a Wado-side and WIT-side decision, not a wasm-side one. Every extends-participating type collapses to one CM type at the boundary, so v1 lowers that type to an opaque, copyable `u32` — an index into a host-side table owned by the JS glue — and the same declarations flip to a real `externref` when CM-GC lands ([GC in Components](./wep-2026-03-28-gc-in-components.md)). No user source changes at the flip. The cost of the interim: a handle is never released (see Deferred).
...
- [x] Method resolution over the chain, plus the corner cases that have a surface: override forbidden, trait-vs-inherited collision, statics do not inherit, `Self` fixed at the declaring resource. Visibility judged at the declaring module is left unimplemented — a resource method has no visibility of its own to judge (see [Resource Inheritance](./wep-2026-04-28-resource-inheritance.md))
