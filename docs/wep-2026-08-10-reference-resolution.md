...
which read the spelling off the site so a message says what the programmer
wrote. Neither is a comparable value.
...
### 5. The walk reaches every reference site

The table answers only for sites the walk records, so the walk's coverage is
part of the design rather than an implementation detail.

- **Every bound position is a reference site.** The walk once reached a generic
  parameter's bounds and nothing else: `walk_item`'s `Item::Trait` arm visited
  neither `trait Sub: Super`'s supertraits nor `type A: Bound`'s bounds, so
  those sites had no entry and every consumer of an inherited bound fell back to
  resolving a spelling. All three positions route through
  `AstVisitor::visit_trait_bounds` — one rule rather than three copies of one.
- **A qualified path in expression position is a reference site too.**
  `Trait::method(recv, …)` reaches dispatch as a substring of an `Ident`'s name,
  which no vantage owns. The path's leading segment carries its own `AstId` for
  LSP navigation; the walk records it, and the UFCS dispatcher names the
  required trait from it.
- **A site absent from the table is a bug in the walk; a site present but
  undeclared is the frame derivation's to answer.** Measured on the e2e suite by
  making the frame fallback panic: 4141 trait references reached it before the
  fixes above, 4 after — and those 4 are sites the table _does_ hold, with
  `Unresolved`, for a name that reaches no import, no local declaration and no
  prelude entry. A bodiless derive (`impl Deserialize for Point;`) may
  legitimately name a stdlib trait the module never `use`d, and only the
  declaration indexes can answer for it.

### 6. The mangled name carries identity

A method mangle already named its receiver by the declaring module; the trait
half does too, through `name::FqTraitName` — the same
unforgeable-by-construction discipline `FqTypeName` carries. It replaces
`LocalMethodName`'s three separate fields (`trait_name`, `base_trait_name`,
`base_trait_module`), which could disagree, with one that cannot. Its
constructors take a declaration: the impl header's site (through
`Resolutions`), a `CompilerItems` entry (`trait_fq`), or a resolved `DeclKey`.
Flipping the field's type enumerated every producer — ~180 of them — which is
the property this design was chosen for.

That closes #1785's remaining direction (an aliased bound reaches the impl that
defines the method) and the collision where two same-named traits implemented
for one receiver mangled to one name and one impl overwrote the other.
`AssocTypeProjection` carries its bounds the same way, answered where the trait
declaration wrote them; it was the last bound store that kept a spelling, and
it kept one because the sites it needed were the ones the walk never reached.

The other bound stores keep their sites too — `infer_holes`' recorded bounds,
`type_param_bounds` on the struct and trait digests, `BlanketImpl::bounds` —
and `find_method_in_trait_bounds` takes the bounds themselves, answering from
the winning one's site.
...
The two namespaces get separate storage, and both layers write both maps from
one receiver identity — `Receiver::head_key` and `Receiver::decl_key` off the
same `ImplTargetKey` — so they cannot drift apart. The AST layer is derived from
the impl headers' resolved targets rather than the heads they wrote. AST-first
...
spellings, so there is no namespace to get wrong), `Instantiated(&MangledName)`
(a receiver with its type arguments, which only the mangled namespace can spell)
and `Declared(&DeclName)` (a caller that genuinely holds nothing else). A bare
`&str` can no longer claim to be mangled, and candidate lists are built by one
function, so each entry's form is decided by what produced it.

Three rules fell out, each of which the single-namespace index had hidden:

- **A type parameter has no spelling in the declaration namespace.** The
  synthesis layer recorded core's generic `impl<T: …>` under the declaration key
  `("T", trait)`, where it answered for a user `struct T`.
- **A bodiless derive asks for an impl, it does not host one.** Type-param
  dispatch runs through a blanket only when the receiver has no per-type impl,
  and `impl Serialize for Config;` sat in the index as though it held a body —
  so dispatch stopped at a module with none and nothing downstream was
  instantiated.
- **A frame's own type parameter wins over a same-named declaration.** The
  declaration indexes cannot see binders, so a name-only lookup inside a generic
  frame must stop before them.
...
compile, which is this class's signature. The arity check already compared
identities; the identity was wrong, because the header derived it by resolving
its own spelling through a layer that cannot see a trait. The header carries
`trait_ref` — the table's answer for the site it wrote — and that answers now.

The receiver half answers the same way, and one rule keeps the two ends of a
lookup together: **a head that names no declaration gets no module.**
`ImplTargetKey::of_decl` is where that is decided, mirroring
`FqTypeName::of_head`, so a definition reached through a written head and a
lookup reached through a resolved type land on the same key. The tuple family is
...
missing with no diagnostic — and made coherence treat every one of them as
foreign.

### 9. Keying the trait-driven registries

The associated-type registries key on `tir::TraitKey`, the trait's declaring
module and declared name, filled from the impl header's site and from each
blanket bound's. The synthesis gate — `bound_driven_synth_requests` and
`SynthesisCtx`'s `pending` / `requested` — keys the same way; the traits that
drive synthesis are all compiler items, so `OnBoundTrait::compiler_item` reads
the declaration off the registry instead of the bound's spelling.

What still asks by spelling is `resolve_assoc_type_qualified`, because an
`AssocTypeProjection` records the trait as a `String`. It declines when two
declarations sharing the name disagree rather than letting registration order
decide.

`blanket_impls` and `trait_impl_modules` remain keyed by trait name.
`impl_target_key_at` computes the receiver's `ImplTargetKey` in the same loop
that builds the index, so nothing blocks re-keying them on identity.

### 10. Name-only callers go through the table
...
The table exposes the scope order it runs at every site
(`Resolutions::declaration_named`), and every name-only caller goes through it —
`decl_key_or_local` is the one chain, and an impl target keys off its answer
rather than asking the table directly. One implementation, so a name-only answer
and the answer its site would have got cannot differ. "Is this name a trait / an
effect / a resource" is asked of the declaration index with a resolved key
rather than scanned for by bare name.
...
### 11. The CM registry's source interface
...
- **The question cannot be asked wrongly.** Resolving needs a reference site's
  `AstId`, and the site determines its module. There is no
  `(name, guessed frame)` entry point left to call.
...
- The flip reaches every consumer of trait and type identity — ~200 parameters,
  plus the maps and guards keyed alongside them.
- `Resolutions` is a whole-program table built before elaboration. It must be
  ...
  resolves nothing.
- The table subsumes the consumers' own derivations, so the two had to be shown
  to agree before anything depended on it. Compared across all 2127 fixtures,
  the impl-header answers disagree exactly once, on
  `inherent_impl_undeclared_type.wado`: the table says `Unresolved` where the
  consumer says `TypeParam` — the conflation this design removes.
...
The split also drew the design's own remaining limit. `ImplReceiver` stops a
caller passing a bare `&str` as mangled, but `MangledName::new` takes any
string, so a caller can still promote a declaration name by hand — and one did:
the candidate builder wrapped struct-instantiation keys, which are declaration
names, so the query asked the fq-keyed map for a bare `Config` that it never
holds. The type's doc calls its constructors "a bounded set of entry points";
they are not bounded yet. Closing that means `MangledName` is constructible only
from `Receiver::head_key` / `Receiver::mangle`.
