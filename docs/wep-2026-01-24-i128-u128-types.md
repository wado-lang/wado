...
// Conversion
let x = a.to_i64();  // truncates to lower 64 bits
```
...
| `low()`/`high()` accessors                    | ✅     |
| `mul_u64()` helper method                     | ✅     |
| i32 → u64 cast codegen fix                    | ✅     |
| Literal coercion (`let x: u128 = 42`)         | ❌     |
| `as i128` / `as u128` casts                   | ❌     |
| `Mul`/`Div`/`Rem` trait implementations       | ❌     |
| Bitwise traits (`BitAnd`, `Shl`, etc.)        | ❌     |
| `Display` trait                               | ❌     |

### Remaining Tasks

1. **Literal coercion**: Allow `let x: u128 = 42` to call `u128::from_u64(42)` implicitly
2. **Cast expressions**: Support `42 as u128` syntax
3. **Full arithmetic**: `Mul`, `Div`, `Rem` (require multi-word algorithms)
4. **Bitwise operations**: `BitAnd`, `BitOr`, `BitXor`, `BitNot`, `Shl`, `Shr`
5. **Display trait**: Convert to decimal string for `println`
...
This makes `i128` and `u128` available in all modules without explicit imports (same as `String`, `Array`, etc.).
