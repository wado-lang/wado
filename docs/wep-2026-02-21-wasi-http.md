...
- **`task return` outside the target world**: an `export async fn` the world does not export keeps its body, and Wado code can call it. `example/http_mock.wado` does. There is no CM task to deliver to, so `task return value` reduces to `value` evaluated for effect and the call observes nothing. What it should observe is undecided. Delivering the value to the Wado caller would let a test assert on an exported handler's own result, which it can reach today only through the mocks it installed. Closing this means choosing that destination and saying what a direct call returns.
