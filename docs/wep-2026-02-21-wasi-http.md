...
`--max-concurrency` independently bounds total resource use: it caps concurrently in-flight requests and sizes the pooling allocator's fiber-stack pool, so worst-case resource use is bounded by construction rather than growing without limit. A worker runs its share of that bound and leaves the surplus in its queue, back-pressuring the connections that carry it; the bound is therefore also a throughput setting, since every in-flight request pins a fiber stack the GC walks on each collection. It defaults to 32 requests per worker, measured as the plateau of the routing benchmark.
