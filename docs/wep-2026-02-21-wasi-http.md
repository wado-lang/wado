...
Liveness, however, is a separate property that `wado serve` must guarantee on its own — a worker stuck forever is a correctness bug, not a tolerated denial of service. Every wait a request makes has to answer to something: `--timeout` bounds the path to the response head, and the head's consumer bounds the handler that produces it.

Two waits would otherwise run forever. A guest parked in a host call keeps its in-flight slot after its client has been given a `504`. The epoch deadline cannot reclaim it, because a parked guest runs no wasm. So the handler is raced against the receiver of the head and ends when that receiver goes, whether the timeout fired or the client disconnected. The body pump stalls on a consumer that holds the connection open but stops reading it. The connection is never closed, so hyper never drops the body channel, so `frame_tx.send` blocks forever (issue #1138). The pump already exits when the connection _closes_, which drops the channel and fails the send, but nothing ends a connection that simply stays open and undrained. So every blocking `frame_tx.send` is bounded by `--timeout` as well.

Both are liveness guarantees, not DoS countermeasures. They are load-bearing when `wado serve` runs standalone: `wado serve file.wado` is a plain `0.0.0.0:8080` server with no platform front and no external timeout, so they are the only thing that ends the hang. On the managed platforms they are defense-in-depth. The platform timeout would eventually close the connection and free the worker, but `--timeout` reclaims it sooner and on a value the operator controls.

`--max-concurrency` independently bounds total resource use: it caps concurrently in-flight requests and sizes the pooling allocator's fiber-stack pool, so worst-case resource use is bounded by construction rather than growing without limit. A worker runs its share of that bound and leaves the surplus in its queue, so the connections carrying it back-pressure. Queueing is part of the client's time to first byte, so the wait for a worker slot answers to `--timeout` like the handler itself: a request that never reaches a worker gets the same clean `504`, never an unbounded hang. The bound is also a throughput setting: every in-flight request pins a fiber stack the GC walks on each collection. It defaults to 32 requests per worker, the plateau measured on the routing benchmark.
