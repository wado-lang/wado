...
The generator's entry file (the one named by `generator = "..."` in its `[package]` section) MUST export a type named `Options`. A missing or ill-typed `Options` export is a compile error at the point the generator is registered, not at the point it runs.
...
Until the Wado → WIT mapping ships, the compiler extracts `Options` directly from the generator module's Wado IR. When that mapping eventually lands, a more strongly-typed wire (per-generator WIT `record` instead of `any-value`) becomes possible and may replace this layer in a future `core:kiln` version bump; in that case author-visible code is unchanged.
...
Gale is Tier 1 and migrates: a `.g4` invocation in `wado.toml` or inline at a `use` site replaces the `mise run update-gale-golden` dance. Gale's module exports `core:kiln/generator` with an `Options` matching its current CLI flags.

### Versioning

The `kiln` world is versioned as an ordinary WIT interface, starting at `core:kiln@0.1`. A generator declares the exact version it targets in its own `wado.toml`; a mismatch against the compiler's supported set is a hard error naming both sides. `0.x` compatibility is not guaranteed; post-1.0 follows ordinary component-model versioning. The world version is part of the cache key, so bumping it invalidates every cached output automatically.
...
Until the Wado → WIT mapping ships, extract `Options` directly from the generator module's Wado IR (endorsed transitional path in "Options schema"). A package is treated as a Kiln generator iff its manifest sets the `[package].generator` well-known-world field to a file path; the referenced entry file must export `pub struct Options` and `pub fn generate(req: Request<Options>) -> Result<Response, Error>`. (The original M1–M5 landing used a transitional `build-world = "wado:kiln/generator"` string field; M6 replaces it with the `generator` field and simultaneously renames `wado:kiln` → `core:kiln`.)
...
M6 is the first milestone where a Wado-authored generator actually runs through the native `wado compile` path. It packages four concerns together because they share the same user-visible surface (the `core:kiln` namespace, the `generator` manifest field, the `build/kiln/` layout) and must ship atomically to avoid a half-renamed release:
...
- [ ] Document the layout in the WEP (above, under "`build/kiln/` directory layout") and in `package-gale/README.md` / consumer fixtures' `.gitignore`.
- [ ] Existing per-invocation `build/kiln/<invocation>/` output is unchanged; only the sibling `generators/` and `metadata/` directories are new.
...
The six subsections are expected to land across three PRs:
