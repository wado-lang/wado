...
- `build/kiln/generators/<generator-source-hash>.wasm` — the `wasm32-wasip3` component the compiler produced from a generator package's Wado source, named after the hash of the sources it was built from.

  A generator's identity is what it was built from, so the name describes the content. The file is therefore immutable: a changed generator writes a new name instead of overwriting one that no longer describes it, which is what lets a cache hit skip validation entirely and leaves no window where a reader sees one build's name over another's bytes. It is not part of the cache key for an invocation (see "Caching and the `<primary>.kiln.json` cache file") — it caches the generator compilation itself, keyed independently.

  The path handed to the compiler is absolutized and normalized first, because the compiler embeds it: left as written, one generator would compile to different bytes depending on which project addressed it or which directory the command ran from, and those bytes would land under a single name.

- `build/kiln/generators.json` — what this project last built: per generator, its identity, the `OptionsDescriptor` extracted from it, and the source closure that says whether that identity is still current. Purely a lookup aid — a later run needs the closure to know the component's name without recompiling to learn it. It makes no claim about the generated outputs; that is `<primary>.kiln.json`'s job, per invocation, and keeping the two apart is what keeps this file free of correctness weight. Deleting it costs a rebuild and nothing else.

  Concurrent builds of one generator are single-flighted: a miss takes a per-generator lock and re-checks behind it, so a parallel `wado test` — which resolves the generator once per fixture — costs one build rather than one per fixture.
