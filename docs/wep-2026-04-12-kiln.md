...
- [ ] Cache key: canonical JSON byte form (sorted keys, NFC-normalized strings, fixed numeric representation) replaces M4's provisional `encode_options_canonical` on the common subset. Wire bytes feed `hash_options_canonical` directly. Lands with M6.5 stage 2 once the adapter emits the encoded string.
...
- [ ] Deferred to the adapter follow-up: lower-side CM emission for generic worlds — today `emit_world_exports` declares non-HTTP, non-test exports as `(func) -> result`, which mismatches the kiln generator's actual `(func (raw: RawRequest)) -> Result<Response, Error>` shape. `lower_to_flat_inner` needs proper non-empty-array serialization into linear memory, the component builder needs to emit CM record/variant types for `core:kiln/types` (RawRequest, Response, Error, InputFile, OutputFile, HostError, Diagnostic, SourceSpan), and `CanonicalIntrinsic::TaskReturn` must reach the synthesized return type. `CliGeneratorProvider` therefore keeps `skip_validation: true` with a pointer at this follow-up.
- [ ] Deferred to a follow-up (tracked in "Planned follow-up branches"): compiler-synthesized CM adapter so authors write `fn generate(req: Request<Options>)` directly. The v1 `bind_request` path already delivers the ergonomic one-liner and preserves determinism, caching, sandboxing, and the canonical-JSON wire form; the adapter is purely a UX polish that removes the explicit `bind_request::<Options>(raw)?` call. Removal of `RawRequest` from the facade and of the `bind_request` helper can ship alongside that adapter.
- [ ] Deferred to the adapter follow-up: replace the provisional binary options encoder in `wado-compiler::kiln::cache` with a canonical-JSON byte encoder (M6.4 cache-key carryover). The current binary encoding keeps cache-key determinism; migrating it invalidates in-flight cache entries but does not change any user-visible behaviour.
...
- [ ] `descriptor()` populates `OptionsDescriptor` from the generator's Wado IR and caches to `build/kiln/metadata/<stable-id>.descriptor`. Descriptor-cache follow-up — the driver still falls through to the provisional TOML encoder when `descriptor()` returns `Unsupported`.
...
- [x] `package-gale/src/generator.wado` Kiln-shape implementation exports `fn generate(raw: RawRequest) -> Result<Response, Error>`, starts with `let req = bind_request::<Options>(raw)?;`, and delegates to `codegen.wado`. `Options` lives inline in `generator.wado` and covers the current `gale gen` flags (`emit_highlight`). Additional flags (`emit_visitor`, ...) are tracked with the existing `package-gale/TODO.md`.
...
1. CM adapter lower-side — emit CM record/variant types for the full `core:kiln/types` surface in the component builder, route `CanonicalIntrinsic::TaskReturn` at the synthesized return type (mirroring the `http-handler-result` special case), and complete `lower_to_flat_inner` for `Array<T>` with non-empty element serialization. This closes the validation gap that currently keeps `skip_validation: true` on the `CliGeneratorProvider` compile path.
2. Adapter-driven `Request<Options>` — once the lower side validates, lift the explicit `bind_request::<Options>(raw)?` call into a compiler-synthesized wrapper so authors write `fn generate(req: Request<Options>)` directly. Ships with the canonical-JSON byte encoder migration so the wire `options: string` and the cache key agree byte-for-byte.
3. User-level `pub effect` / `pub resource` follow-up — codegen fix.
4. Gale consumer project + `kiln_path.rs` — full Gale-through-Kiln path with a real grammar-generated consumer crate, once the adapter follow-up lands.
