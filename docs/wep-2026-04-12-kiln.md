...
- [ ] `package-gale/tests/kiln-consumer/` minimal consumer project (grammar + `wado.toml` + `src/main.wado` that imports the generated parser) plus `package-gale/tests/kiln_path.rs` running `wado compile` and diffing against an existing golden. Deferred with the adapter follow-up; `wado-cli/tests/kiln_compile.rs` exercises the end-to-end path against a synthetic single-file generator, which is enough to unblock M6 without reshaping the Gale test harness.
