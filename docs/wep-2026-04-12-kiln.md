...
6. The compiler writes every file in the response to its configured output directory, stamps each file with a `#![generated(...)]` attribute naming the source IDL and the generator package, records the cache entry in `wado.lock`, and continues with normal compilation.
...
- A `with { generator: { ... } }` block must include at least `module` and `source`. Partial forms that only override `options` are not supported in v1; manifest and inline are full equivalents, not a cascade.
