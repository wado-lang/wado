...
1. CM adapter synthesis — lift the `bind_request::<Options>(raw)?` one-liner into a compiler-synthesized wrapper so authors write `fn generate(req: Request<Options>)` directly. Includes the canonical-JSON byte encoder migration (replacing the provisional binary encoder in `wado-compiler::kiln::cache`) so the wire `options: string` and the cache key agree byte-for-byte.
2. User-level `pub effect` / `pub resource` follow-up — codegen fix.
3. Gale consumer project + `kiln_path.rs` — full Gale-through-Kiln path with a real grammar-generated consumer crate, once the adapter follow-up lands.
