...
- [x] Lower-side CM emission, partial: `emit_kiln_world_types` defines the full `core:kiln/types` surface (`input-file`, `list-input-file`, `output-file`, `list-output-file`, `response`, `error`, `raw-request`, `handler-result`) inline in the component type section when `FlatPackage::is_kiln_generator_world()` fires; `emit_world_exports` uses `(param req: raw-request) -> result<response, error>` with `CanonicalOption::Realloc` threaded through the lift; `CanonicalIntrinsic::TaskReturn` picks `kiln-handler-result` so the canon adapter's declared result shape matches the core module's 4-flat `task-return` import. `worlds.wado` + `generator.wit` mark `generate` as `async func`; `kiln_runtime` drives the call through `store.run_concurrent(...)`. `CliGeneratorProvider::skip_validation: true` still holds pending the final CM name-set wrap-up: wasm-tools's validator requires each top-level record/variant export to satisfy `created == referenced` (the types need to come from an imported instance). That remaining step is tracked in "Planned follow-up branches" #1.
...
14. Stream 1 (M6.5 stage 2 lower-side, partial) — `emit_kiln_world_types` + `FlatPackage::is_kiln_generator_world` gate the full `core:kiln/types` type surface at the component level, `emit_world_exports` uses the `(param req: raw-request) -> result<response, error>` signature with realloc, `CanonicalIntrinsic::TaskReturn` picks `kiln-handler-result`, and `worlds.wado` + `generator.wit` mark `generate` as `async func`. `kiln_runtime` drives the call through `store.run_concurrent(async |accessor| ...)`. `skip_validation: true` still holds pending the CM name-set wrap-up (follow-up #1).
