...
- All Kiln artefacts live under `build/kiln/`: generated `.wado` under `build/kiln/<synthesized-id>/`, compiled generator components under `build/kiln/generators/`, and extracted metadata (e.g. `OptionsDescriptor`) under `build/kiln/metadata/`. The `<synthesized-id>/` root may be redirected per-invocation via `output_dir`. Committing `build/kiln/` and `wado.lock` is recommended for projects that want the LSP to work in web editors (VS Code Wasm, browser playground); native-only projects may keep `build/kiln/` in `.gitignore`. See "Transitional consume-only mode" for why.
...
- **`wado.toml` + `wado.lock`** ([`wep-2026-02-14-package-manifest.md`](./wep-2026-02-14-package-manifest.md)) — package manifest and lock file. We add a new `[build-dependencies]` section for build-only generator packages and record generator cache entries in the lock file.
...
All Kiln artefacts live under a single `build/kiln/` directory rooted at the consuming project. The layout is fixed by the compiler and is not configurable except for per-invocation output redirection (see `output_dir` in "Use site syntax" above).
...
└── <synthesized-id>/           # generated .wado files (one directory per invocation)
    ├── <entry>.wado
    └── <supplementary>.wado
```
...
- `<synthesized-id>/` — the generated `.wado` files that one invocation emitted. One directory per invocation, named after a synthesized id derived from the declaring file path and the `from` schema path. Files here carry the `#![generated(...)]` header (see "Generated file header") and are managed by the compiler.
...
- The compiler synthesizes an internal `Invocation { module, from, inputs, output_dir, options }` from each clause. The synthesized id used in the default `output_dir` and as the `wado.lock` entry name is the `kiln-<16-hex>` SHA-256 prefix of `(module, from, inputs, options)`. `decl_file` is intentionally not folded in, so two files that emit the exact same invocation tuple share a single cache entry.
...
4. For each invocation in topological order:
   ...
   e. Persist each `output-file` to `<output_dir>/<path>`, stamping the `#![generated(...)]` header. Delete any existing `#![generated]` file in `output_dir` that was not produced by this run. Record the cache entry in `wado.lock`.
...
[[generator-cache]]
invocation = "src_main_wado__schemas_user_proto"   # synthesized id: <decl-file>__<from>
...
options-hash = "sha256:..."
outputs = [
    { path = "build/kiln/src_main_wado__schemas_user_proto/user.wado",   hash = "sha256:...", entry = true },
    { path = "build/kiln/src_main_wado__schemas_user_proto/types.wado",  hash = "sha256:..." },
]
```
...
### M1 — Manifest + lockfile schema (done; later revised in M8)

`wado-manifest` carries `[build-dependencies]` and the lockfile grows `[[build-dependency]]` + `[[generator-cache]]`, with round-trip fixtures and deterministic re-serialize. `wasm32-unknown-unknown` stays green.
...
- `wado.toml` grows a `[build-dependencies]` section and a new `[package].generator` well-known-world field (joining `command`, `service`, `lib`); `wado.lock` grows `[[build-dependency]]` and `[[generator-cache]]`. All changes land in `wado-manifest`, which is already `wasm32-unknown-unknown`-ready.
