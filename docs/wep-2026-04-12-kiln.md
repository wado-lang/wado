...
Stage 2 (CM adapter synthesis):

- [ ] Register the full `core:kiln` type surface in `WasiRegistry` (`InputFile`, `OutputFile`, `Response`, `Error`, `RawRequest`, `HostError`, `Diagnostic`, `SourceSpan`).
...
1. M6.5 stage 2 — synthesize the CM adapter, add import-refusal diagnostic + fixture.
2. M6.6 — real `CliGeneratorProvider::get_component` + descriptor caching.
3. M6.7 — Gale Kiln-shape generator + consumer/E2E tests.
4. User-level `pub effect` / `pub resource` follow-up — codegen fix.
