...
- [ ] Decide the `Request<Options>` surface: whether `options` is typed (deserialized from `list<u8>`) on the Wado side or remains opaque bytes that the author decodes with `core:serde`. This choice is the last open design question in M6 and is flagged as a decision point in its own right; see follow-up.
...
- [ ] `Request<Options>` typing on the Wado side. Open: whether `core:kiln` exposes `Request<T>` with compiler-synthesized deserialization from the WIT-level `options: list<u8>`, or whether authors decode `options` explicitly via `core:serde`. Resolved in M6.4.
