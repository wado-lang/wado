...
- [x] CM adapter lift-side synthesis: `synthesize_lift_from_flat_params` gained a struct arm that walks each `TirStruct` field, reconstructs the field's AST surface through a local `type_id_to_ast_type` converter, recursively lifts flat CM values, and emits a `StructLiteral` keyed on the registered `TypeId` (via `find_struct_by_name`). A new `FlatLiftContext { wasi_registry, type_table_cell, wasi_package }` threads the full CM resolution stack (WASI + kiln namespace) through the three export-binding synthesizers (`synthesize_result/general/async_export_binding`). `WasiRegistry::resolve_cm_source_for` consults WASI first, then falls back to `core:kiln/*`, so kiln records resolve without leaking into bare-name WASI lookups. `module_source_for_cm_interface` mirrors `wasi_interface_suffix` for `core:*` sources so `try_lift_wasi_struct`'s synthesized `TypeId`s match the WIR `struct_type_map` for kiln records.
...
- [ ] `GeneratorModule::LocalPath(path)`: load the package, compile to a `wasm32-wasip3` component via the ordinary codegen path, cache at `build/kiln/generators/<stable-id>.wasm`.
- [ ] `GeneratorModule::Spec(_)`: keep returning `Unsupported` with the consume-only message.
- [ ] `descriptor()` populates `OptionsDescriptor` from the generator's Wado IR and caches to `build/kiln/metadata/<stable-id>.descriptor`.
- [ ] Import-refusal regression test: a generator source that tries to import `wasi:clocks` fails compilation with a clear diagnostic.
...
9. CM adapter lift path — struct-aware flat-param lifting in `synthesize_lift_from_flat_params`, `FlatLiftContext` threading, `WasiRegistry::resolve_cm_source_for` bridging `wasi:*` and `core:kiln/*`, and kiln-namespace hits in `TypeTable::find_named_type_by_wasi_package`. Generators now receive properly constructed `RawRequest` values at the user-function boundary.
