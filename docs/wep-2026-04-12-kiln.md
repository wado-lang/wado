...
- [x] `wado-compiler/lib/wado/kiln/generator.wit` (single source of truth; `wado-compiler` embeds via `include_str!`; `wado-cli` consumes via `wasmtime::component::bindgen!`).
- [x] `CompilerHost::run_generator(&self, component_wasm, GeneratorRequest) -> GeneratorResponse` with a default `Err(Unsupported)` body. Plain Rust mirrors of `wado:kiln/types`: `GeneratorRequest / InputFile / Response / OutputFile / ReadRecord / Diagnostic / Error / RunnerError`.
- [x] `wado-cli::kiln_runtime`: bespoke `Linker` exposing only `wado:kiln/host` (no WASI, no http, no clocks). `read-file` forwards to `load_source` and records each successful read into `reads`; `emit-diagnostic` forwards to `emit_diagnostic`.
