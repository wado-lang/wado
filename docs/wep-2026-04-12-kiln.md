...
`core:kiln/generator` is registered in `WorldRegistry`; `comp_feature` flags + `kiln_synth::prepare_kiln` auto-synthesize `Options::deserialize`. The full `core:kiln` type surface is registered in `CmInterfaceRegistry` via the two-pass stdlib bootstrap; CM lift / lower paths handle kiln records via `LiftContext::cm_package` and `CmInterfaceRegistry::resolve_cm_source_for`. Import-refusal (`Code::KilnGeneratorForbiddenImport`) blocks any non-kiln-allowlisted import in a generator-world build, with the M2 runtime check kept as defense in depth.
