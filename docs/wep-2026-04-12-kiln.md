...
M6 is the milestone where a Wado-authored generator is intended to run through the native `wado compile` path. The seven subsections share the same user-visible surface (the `core:kiln` namespace, the `[package].generator` manifest field, the `build/kiln/` layout). Stages 1–4 (M6.1–M6.4 plus the foundational half of M6.5) shipped on branch `claude/plan-m6-implementation-*`; the remaining work is gated on the `WasiRegistry` bare-name collision blocker below.
...
- [x] Hand-written facade `wado-compiler/lib/core/kiln.wado` re-exports `KilnHost` / `HostError` / `Diagnostic` / `DiagnosticLevel` / `SourceSpan` / `InputFile` / `OutputFile` / `Response` / `Error` and defines the Wado-only `pub struct Request<T>` (marked with `#[comp_feature("kiln_request")]`). `RawRequest` is intentionally not re-exported.
...
- [x] `core:kiln/generator` is registered in `WorldRegistry` via `WasiRegistry::build_from_stdlib_inner`. Only the world is registered — not the types — until the `WasiRegistry` bare-name collision blocker below is resolved (see "Known blockers"). `world_registry::tests::test_kiln_generator_world_registered` asserts presence + the single `generate` export.
...
Stage 2 (CM adapter synthesis, **blocked** — see "Known blockers"):

- [ ] Register the full `core:kiln` type surface in `WasiRegistry` (`InputFile`, `OutputFile`, `Response`, `Error`, `RawRequest`, `HostError`, `Diagnostic`, `SourceSpan`). Currently skipped because `Response` / `Error` would shadow `wasi:http/types::Response` / `ErrorCode`.
- [ ] Codegen synthesizes `_kiln_generate_cm_wrapper(raw: RawRequest) -> Result<Response, Error>` that decodes `raw.options` (canonical JSON) into `Options`, builds `Request<Options>`, and calls the user's `generate`. The wrapper is the actual CM export; the user's body stays written against `Request<Options>`.
- [ ] Canonical-JSON encoder on the caller side (M6.4 cache-key carryover).
- [ ] Import-refusal: any generator that imports an interface other than `core:kiln/kiln-host` produces a compile-time diagnostic. The M2 runtime-level check stays as defense in depth.
...
- [ ] `package-gale/src/generator.wado` Kiln-shape implementation exports `fn generate(req: Request<Options>) -> Result<Response, Error>` delegating to the renamed `codegen.wado` logic.
...
These two compiler bugs surfaced during M6 implementation. Both are documented with regression fixtures under `wado-compiler/tests/fixtures/cross_module_same_name_*.wado` and `user_{effect,resource,world}_smoke.wado` (currently marked `#![TODO]` where they ICE).

Blocker A — `WasiRegistry` bare-name collision:

- The per-kind maps (`structs`, `resources`, `flags`, `newtypes`, `enums`, `variants`) are keyed by the bare type name. Two stdlib modules declaring the same name silently overwrite each other. Verified latent in `wasi:*` today: `ErrorCode` is defined independently by `wasi:filesystem/types`, `wasi:http/types`, `wasi:sockets/types`, and `wasi:sockets/ip-name-lookup` — only the last registration wins in the bare-name slot.
- `WasiRegistry::check_collision` now tracks the first registrant per `(kind, name)` and emits a warning + `Collision::Reject` on mismatched registrations, preserving the first. The disambiguated `format!("{interface}#{name}")` key (already populated for `enums` / `variants`) stays intact for source-aware lookups.
- Kiln stage 2 requires registering `core:kiln/types::{Response, Error, InputFile, OutputFile, RawRequest}` — which would collide with `wasi:http::Response` and `wasi:http::ErrorCode` under today's bare-name lookups.
- [ ] Add `module_source` (or a resolved-symbol handle) to `Type::Named` so codegen lookups can pass the owning interface explicitly.
- [ ] Migrate the ~22 remaining bare-name callers of `get_variant_*` / `get_struct_*` / `get_resource_*` / `get_flags_*` / `get_enum_*` / `is_*` (in `synthesis/cm_binding.rs`, `codegen/component.rs`, `component_model.rs`) to their source-aware `*_by_interface` counterparts. Two hardcoded HTTP `ErrorCode` lookups have been migrated; the rest operate on `Type::Named { name }` and need the source-threading in the bullet above.
- [ ] Flip `WasiRegistry::check_collision` from `eprintln!` warning to `panic!`; add a `#[test]` that asserts a clean stdlib registration leaves `name_owner` collision-free.
- [ ] Register the full `core:kiln` type surface once the above is green.

Blocker B — user-level `pub effect` / `pub resource` ICE:
...
- Independent of Blocker A; does not block Kiln M6.5–M6.7 directly (Kiln generators import `KilnHost` rather than declaring new effects) but does block the broader "user modules can declare worlds / effects / resources" story.
...
Remaining work is gated on Blocker A. Planned follow-up branches:

1. **`WasiRegistry` source-awareness** — thread `module_source` through `Type::Named`, migrate bare-name lookups, promote the collision warning to `panic!`.
2. **M6.5 stage 2** — register full `core:kiln` type surface, synthesize the CM adapter, add import-refusal diagnostic + fixture.
3. **M6.6** — real `CliGeneratorProvider::get_component` + descriptor caching.
4. **M6.7** — Gale Kiln-shape generator + consumer/E2E tests.
5. **Bug B follow-up** — user-level `pub effect` / `pub resource` codegen fix.
