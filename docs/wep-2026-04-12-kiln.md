...
`additionalProperties` is `false`. gale's `{ highlight: bool, trace: bool =
false }` describes as:
...
[dependency-management plan](./dependency-management-implementation-plan.md).
...
`options = { highlight: false, trace: false }` explicitly; a consumer of the same
generator by local path may omit `trace`.
