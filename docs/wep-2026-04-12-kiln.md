...
interface types {
    ...
    /// A generic typed tree used to carry generator options across the
    /// CM boundary. The compiler lowers the user-written `options = { ... }`
    /// (TOML table or inline attribute object) into an `any-value` and
    /// synthesizes a matching decoder on the generator side so that the
    /// author writes `fn generate(req: Request<Options>)` with a fully
    /// typed `Options`. See "Options schema" below.
    variant any-value {
        %null,
        %bool(bool),
        int(s64),
    ...
    /// supplementary schemas (may be empty). `options` is an `any-value`
    /// tree; the generator-side adapter decodes it into the typed
    /// `Options` record declared by the generator module.
    record request {
        primary: input-file,
...
- `options` travels as a generic `any-value` tree at the WIT boundary but is typed per-generator on the Wado side: the compiler validates the user-written options against the generator's `Options` struct and synthesizes an adapter that decodes `any-value` into that typed struct before user code runs. Bytes are never exchanged opaquely. See the next section.
...
Wire format. The WIT request carries `options` as an `any-value` tree. At the call site, the compiler lowers the validated user-supplied options into `any-value`; inside the generator, a compiler-synthesized adapter decodes the tree into the typed `Options` struct before user code runs. Authors write `fn generate(req: Request<Options>)` and never see the tree. The same `any-value` traversal is used to derive the canonical byte form that feeds the invocation cache key.

Future evolution. Once the Wado → WIT mapping matures, a more strongly-typed wire (per-generator WIT `record` instead of `any-value`) becomes possible and may supersede this layer in a future `core:kiln` version bump; author-visible code is unchanged when that happens.
...
pub trait CompilerHost {
    ...
    /// instantiating the component, linking `core:kiln/host` so that
...
- [x] `wado-cli::kiln_runtime`: bespoke `Linker` exposing only `core:kiln/host` (no WASI, no http, no clocks). `read-file` forwards to `load_source` and records each successful read into `reads`; `emit-diagnostic` forwards to `emit_diagnostic`.
...
- [ ] Add `wado-compiler/lib/core/kiln.wado` (the user-facing module imported via `use {...} from "core:kiln"`). Two-file shape: a WIT-generated types file plus a handwritten entry that re-exports and supplies any Wado-side helpers around the CM boundary.
- [ ] Registration in the loader so bare `use { Request, Response, Error, ... } from "core:kiln"` resolves, matching the convention already used for `wasi:*` modules.
- [ ] `wado-from-idl`: extend to handle the `core:` namespace (the current tool targets `wasi:*`); add `mise run update-stdlib-kiln` task pointing at `lib/core/kiln/generator.wit`.
- [ ] `Request<T>` wire form is `any-value` (see "The `kiln` world" WIT sketch). `core:kiln.wado` re-exports the CM-generated `AnyValue` / `AnyKv` types and declares a generic `Request<T>` wrapper whose `options: T` field is populated by a compiler-synthesized decoder. Authors never touch `any-value`.
- [ ] Compiler adapter synthesis: given `OptionsDescriptor` (from M4) emit a `decode_options_any_value(v: &AnyValue) -> Result<Options, Error>` stub that is linked into the generator module before `generate` is called. Mirrors the compile-time validation already performed against the user-supplied `options = { ... }`, so decode failures are a compiler bug rather than a user-facing error.
- [ ] Cache key: canonical byte form is a deterministic traversal of `any-value` (keys sorted, numeric canonicalization, UTF-8 NFC). Replaces the provisional `encode_options_canonical` byte equivalence clause from M4.
...
- [ ] Codegen: lower the entry's `export fn generate(req: Request<Options>) -> Result<Response, Error>` to the WIT `generate` export; link `core:kiln/host` as CM imports.
- [ ] Import-refusal behaviour: any generator importing a non-`core:kiln/host` interface (e.g. `wasi:clocks`) produces a compile-time diagnostic (not just a runtime trap at `Linker` time). The M2 runtime-level check remains as a defense in depth.
...
- [x] `Request<Options>` typing on the Wado side: resolved to the `any-value` wire form with a compiler-synthesized adapter (see "The `kiln` world" and M6.4). A future core:kiln version bump may replace `any-value` with per-generator WIT records once Wado → WIT mapping matures.
