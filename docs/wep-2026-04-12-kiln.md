...
- [x] Lower-side CM emission, partial: `emit_kiln_world_types` defines the `core:kiln/types` record/variant surface (`input-file`, `list-input-file`, `output-file`, `list-output-file`, `response`, `error`, `raw-request`, `handler-result`) inline in the component type section when `FlatPackage::is_kiln_generator_world()` fires; `emit_world_exports` uses `(param req: raw-request) -> result<response, error>` with `CanonicalOption::Realloc` threaded through the lift; `CanonicalIntrinsic::TaskReturn` picks `kiln-handler-result` so the canon's declared shape matches the core module's 4-flat `task-return` import. `worlds.wado` + `generator.wit` mark `generate` as `async func`; `kiln_runtime` drives the call through `store.run_concurrent(...)`. `CliGeneratorProvider::skip_validation: true` still holds pending the final name-set wrapping: wasm-tools's CM validator requires each top-level record/variant export to satisfy `created == referenced` or sit inside an imported `core:kiln/types` instance. That wrap-up is tracked in "Planned follow-up branches" #1 (now narrowed to just the name-set step).
- [x] Compiler-synthesized `Request<T>` adapter: `kiln::import_check::inject_kiln_request_adapter` runs between module loading and annotate. When the entry module's `export fn generate(req: Request<T>)` is detected, the phase rewrites the signature to `generate(req: RawRequest)` and prepends `let req = bind_request::<T>(req)?;` — same-scope shadowing (WEP 2026-03-25) keeps `req` typed as `Request<T>` inside the user's body. The phase also extends or synthesizes a `use { RawRequest, bind_request } from "core:kiln"` declaration so authors never mention the compiler-synthesized symbols. The v1 explicit `fn generate(raw: RawRequest)` form still compiles for packages that prefer it. `adapter_generator_compiles_like_raw_request_generator` in `wado-cli/tests/kiln_compile.rs` covers the v2 path end to end through `CliGeneratorProvider::get_component`.
- [x] Canonical-JSON byte encoder landed with M6.4 (see above). `encode_options_canonical` and the provisional TOML-tree encoder in `wado-cli::kiln_driver` both produce the same RFC-8785-style form; `MAGIC` bumps to `kiln-cache-key-v2\0` so in-flight v1 caches invalidate cleanly.
...
- [x] `descriptor()` populates `OptionsDescriptor` from the generator's Wado IR and caches to `build/kiln/metadata/<stable-id>.descriptor`. `OptionsDescriptor` / `OptionsField` / `OptionsType` / `CanonicalValue` gain `Serialize + Deserialize` (spans `#[serde(skip, default)]` so they don't drift across edits). `CompilerOptions` threading produces `kiln_options_descriptor: Option<OptionsDescriptor>` on every kiln compile; `CliGeneratorProvider` shares a single `compile_local` helper between `get_component` and `descriptor`, so whichever path runs the first compile warms both caches. `descriptor_populates_metadata_cache_and_hits_on_second_call` covers compute → persist → hit → source-edit-invalidation.
...
- [x] `package-gale/tests/kiln-consumer/` minimal consumer project (grammar + `wado.toml` + `src/main.wado`) plus `wado-cli/tests/kiln_gale_path.rs` (relocated from `package-gale/tests/` since `package-gale` is not a Cargo crate). The test asserts (a) the consumer manifest lowers through `plan()` to a `GeneratorModule::LocalPath` pointing at `../../src/generator.wado`, and (b) `CliGeneratorProvider::get_component` compiles Gale end to end against the `core:kiln/generator` world with a warm-cache rerun. Full runtime invocation (wasmtime instantiate + diff against `tests/golden/calculator.wado`) is deferred with the instance-wrapping follow-up; a docstring in `kiln_gale_path.rs` notes the extension plan. To make inline-path generators work without the registry path, `kiln_driver::lower()` now accepts `GeneratorModuleRef::Inline(Dependency { source: Path { .. } })`.
...
11. Stream 3 (M6.6 descriptor caching) — `OptionsDescriptor` and friends gain `Serialize + Deserialize`, `Span` gets `Default`. `CompileResult` carries `kiln_options_descriptor: Option<OptionsDescriptor>` populated by a new phase in `compile_after_load`. `CliGeneratorProvider` shares a single `compile_local` helper between `get_component` and `descriptor`; two new tests cover descriptor cache miss → hit + source-edit invalidation.
12. Stream 2a (M6.4 canonical-JSON cache key) — `encode_options_canonical` switches to RFC-8785-style JSON using `unicode-normalization` for NFC, `MAGIC` bumps to `v2`, the provisional raw-TOML-tree encoder in `wado-cli::kiln_driver` mirrors the JSON form so the byte-equivalence tests still hold, and ten new round-trip tests exercise the edge cases.
13. Stream 1 (M6.5 stage 2 lower-side, partial) — `emit_kiln_world_types` + `FlatPackage::is_kiln_generator_world` gate the full `core:kiln/types` type surface at the component level, `emit_world_exports` uses the `(param req: raw-request) -> result<response, error>` signature with realloc, `CanonicalIntrinsic::TaskReturn` picks `kiln-handler-result`, and `worlds.wado` + `generator.wit` mark `generate` as `async func`. `kiln_runtime` drives the call through `store.run_concurrent(async |accessor| ...)` to match the wasmtime bindgen's `exports: { default: async }` signature. `CliGeneratorProvider::skip_validation: true` still holds pending the CM name-set wrap-up (follow-up #1).
14. Stream 2b + 2c (Request<T> adapter + Gale migration) — `kiln::import_check::inject_kiln_request_adapter` rewrites `fn generate(req: Request<T>)` to the internal `RawRequest` + `bind_request` shape before analyze runs, auto-extending the `use { ... } from "core:kiln"` declaration to bring `bind_request` and `RawRequest` into scope. `package-gale/src/generator.wado` switches to the v2 form. `adapter_generator_compiles_like_raw_request_generator` covers the end-to-end path.
15. Stream 4 (M6.7 Gale kiln-consumer harness) — `package-gale/tests/kiln-consumer/` (manifest + grammar + src/main.wado) plus `wado-cli/tests/kiln_gale_path.rs` assert the plan + compile path. `kiln_driver::lower()` accepts `module = { path = "..." }` (inline `DependencySource::Path`) so the consumer doesn't need the not-yet-landed registry resolution.

Planned follow-up branches:

1. CM name-set wrap-up — wrap the `core:kiln/types` record/variant surface in an imported-instance export so wasm-tools's CM validator recognizes the exported types as "named" (`created == referenced`). The top-level type exports emitted by `emit_kiln_world_types` pass wasmparser's `all_valtypes_named_in_defined` check on primitives, but records/variants require `set.contains(ComponentAnyTypeId::from(id))` to hold for the original type-id — which only happens when the types are exported through an instance. Once this lands, `CliGeneratorProvider::skip_validation` flips to `false` and the kiln generator component round-trips through `wasmtime::Component::from_binary`. Extends `wado-cli/tests/kiln_gale_path.rs` with a `run_pipeline` assertion that invokes the component end to end and diffs the generated `calculator.wado` against `tests/golden/calculator.wado`.
2. User-level `pub effect` / `pub resource` follow-up — codegen fix for the WIR lowering that currently produces invalid core Wasm when user modules declare new effects or resources. Independent of Kiln.
3. M7 LSP integration — wire `run_pipeline` into `wado-lsp::Engine` on workspace open, thread `InvocationIndex` through `annotate_with_invocations`, add `wado-lsp/tests/kiln_consume_only.rs`.
