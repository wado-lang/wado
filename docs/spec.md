...
- `==` and `!=` compare two handles when one type extends the other. The host hands out one handle per object, so equal handles name one object. There is no ordering.
