...
- A Wado call of an `export async fn` has type `()`. The declared return type is the Component Model contract; `task return` hands the value to the runtime, not to a Wado caller.
