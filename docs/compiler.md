...
| Reflect metadata   | `synthesis/traits.rs`          | `Reflect`'s `type_name` for every kind, plus the `ReflectStruct` / `ReflectVariant` / `ReflectEnum` / `ReflectFlags` impls: `members`, `wire_name_policy`, the payload / member associated tuples, and the value bridges ([WEP 2026-06-13](./wep-2026-06-13-reflect-derivation.md))                                          |
