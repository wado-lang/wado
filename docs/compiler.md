...
| ------------------ | ---------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Pattern lowering   | translator | `lower/translate/pattern.rs` | `LetDestructure` / `IfLet` → explicit `Let` + `If`, over the scrutinee and destructure temps it mints. Runs at the top of `translate`, before the fold — it rewrites expression shape, so it may not live in the planner |
