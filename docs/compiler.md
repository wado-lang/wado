...
- `monomorphize/` walks call sites, instantiates generic structs and functions with concrete type arguments, and rewrites references. Generic structs and generic functions are both keyed by `(name, ModuleSource)` so same-named generics from different modules coexist. The `ModuleSource` is always the body's home module (the module that defines the template), and the dispatch path panics if a queued generic has no defined home module — every generic must be defined somewhere reachable (in `core:prelude` for built-in shapes like `Tuple`). Variadic `TupleSpread` nodes are expanded here. `name.rs` produces stable mangled names (`Box$i32`, `identity$1`, …).
