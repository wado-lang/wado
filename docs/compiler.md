...
The AST is parser-immutable from this point on. Every surface-level rewrite — compound assignment (`x += y` → `x = x + y`), `while` / C-style `for` → explicit `loop`, the `assert` statement, the `matches` operator, the comparison chain `a < b < c`, template-string interpolations, and `use … namespace` prefix stripping (`helper::foo`) — happens inside the resolver, either by building TIR directly (`resolve_while`, `resolve_for`, `resolve_compound_assign` in `resolver/{stmt,operators}.rs`) or by routing through `resolve_*` with a short-lived AST scaffold whose nodes carry [`AstId::SYNTHETIC`] (`desugar_assert`, `desugar_matches_expr`, `desugar_comparison_chain` in `resolver/{assert,matches,operators}.rs`; `Resolver::strip_ns_prefix` in `resolver.rs`). Keeping the AST parser-shaped is what lets LSP queries land on the user's text rather than on a synthesised replacement.
