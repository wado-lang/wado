...
| Template strings   | `synthesis/template.rs`        | Expands template strings into `Display::fmt` / `Inspect::inspect` calls. The emitted `__tmpl` block is a contract with `optimize::tmpl_hoist` (see that module's docs). Also mints each tagged template shape's `$hole_fmt` bridge, whose arms are the same interpolations ([WEP 2026-01-10](./wep-2026-01-10-tagged-template-literals.md))                                                                                                                                                                              |
