...
- `desugar.rs` rewrites `x += y` to `x = x + y`, for/while loops to explicit loop blocks, and other purely syntactic constructs. The `assert` statement, the `matches` operator, and the comparison chain `a < b < c` are deferred to the resolver (`desugar_assert`, `desugar_matches_expr`, `desugar_comparison_chain` in `resolver/{assert,matches,operators}.rs`) because their expansions need typed sub-expressions, and keeping the AST intact also lets LSP queries land on the user's text. AST→AST transforms in this phase are written as [`AstFolder`](../wado-compiler/src/ast_folder.rs) impls (see `NsStripper`); the `default_fold_*` defaults guarantee structural recursion is exhaustive across every AST variant.
