...
| Template strings   | `synthesis/template.rs`        | Expands template strings into `Display::fmt` / `Inspect::inspect` calls. The emitted `__tmpl` block is a contract with `optimize::tmpl_hoist`, carried past lowering as a `BlockRole` on the node (see that module's docs)                                                                                                                                              |
