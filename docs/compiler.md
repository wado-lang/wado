...
- `desugar.rs` rewrites `x += y` to `x = x + y`, for/while loops to explicit loop blocks, and other purely syntactic constructs. The `assert` statement, the `matches` operator, and the comparison chain `a < b < c` are intentionally preserved here — their expansions run at TIR-lowering time (`resolver/{assert,matches,operators}.rs`) so the AST keeps the user's text intact for LSP queries. AST→AST transforms within this phase are written as [`AstFolder`](../wado-compiler/src/ast_folder.rs) impls (see `NsStripper` in `desugar.rs`); the `default_fold_*` defaults guarantee structural recursion is exhaustive across every AST variant.
