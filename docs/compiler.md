...
Bound-driven requests ([WEP 2026-06-25-trait-derivation](./wep-2026-06-25-trait-derivation.md)) originate earlier, during Annotate: `Elaborator::type_implements_trait_inner` (`elaborator/trait_query.rs`) records a `(type, trait)` pair on `TypeTable::bound_driven_synth_requests` whenever a `T: Serialize` / `T: Deserialize` / `T: Eq` / `T: Ord` bound is satisfied structurally with no explicit impl. This set lives on the one `TypeTable` every module's `Rc<RefCell<…>>` handle shares, since elaboration runs one fresh `Elaborator` per module.

Two passes read the same set as a snapshot, not a drain (consuming it would starve whichever pass runs second), each filtering for the trait names it owns: `serde_synth::synthesize_serde` for `Serialize` / `Deserialize`, routing entries into the target type's own `synthesis_requests`; `traits::synthesize_traits` for `Eq` / `Ord`, gating generation on set membership (`SynthesisCtx::is_requested`) instead of its former unconditional sweep.

An explicit `impl Eq for T;` / `impl Ord for T;` marker is validated immediately at its own span (`Elaborator::record_eq_ord_explicit_request`), a compile error if `T` is not structurally eligible — stronger than serde's marker, which defers to the bound site. Since the marker is itself an `Item::Impl`, `synthesize_traits` gates `Eq` / `Ord` generation on `SynthesisCtx::has_real_impl` rather than the general `has_impl`, so a body-less marker isn't mistaken for "already implemented" and left unsynthesized.
