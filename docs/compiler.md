...
| ------------------ | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Trait auto-derives | `synthesis/traits.rs`          | `Eq` / `Ord` / `Default` for bound-driven requests (below); `Inspect` / `InspectAlt` unconditionally (total); `Display` only for plain enums (bare case name); `DisplayAlt` wherever `Display` exists. Newtypes are _not_ derived a `Display`/format body — they inherit the base type's format traits at the call site (`peel_transparent_newtype` in `synthesis/template.rs`), like any inherited method; `Inspect`/`InspectAlt` are the exception, overridden per newtype for the `as Name` tag |
