...
| Reflect metadata   | `synthesis/traits.rs`          | `Reflect`'s `type_name` for every kind; the four structure kinds (`ReflectStruct` / `ReflectVariant` / `ReflectEnum` / `ReflectFlags`), each with `members`, `wire_name_policy`, its payload / member associated tuples and its value bridges; and `ReflectNewtype`'s `Base` ([WEP 2026-06-13](./wep-2026-06-13-reflect-derivation.md))                                 |
