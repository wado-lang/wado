...
- [ ] Add a `FlatPackage → NirPackage` conversion at the exit of `lower/`.
- [ ] Change `lower/`'s public signature.
- [ ] Add a temporary `NirPackage → FlatPackage` adapter so downstream consumers continue to work unchanged.

### Phase 3: Migrate `optimize/`

- [ ] Rename `TirExpr`/`TirStmt`/`FlatPackage` references inside `optimize/` to their NIR counterparts, pass by pass.
- [ ] Each pass migration is mechanical — same logic, renamed types.

### Phase 4: Migrate `wir_build/`

- [ ] Change `wir_build/build_wir_package`'s input type to `NirPackage`.
