<!-- Auto-generated by: wado doc --format markdown core:serde -->
...
Word boundaries follow `heck`: `_`/`-`/space delimit and are dropped; a
lower/digit → upper transition starts a word; inside an uppercase run, an
uppercase directly before a lowercase starts one. Digits attach to the
current word.
...
deserializable struct, generating a `lookup` that walks a decision tree over
the key's bytes — length first, then the byte that best tells the remaining
field names apart — so a key costs a walk, not a scan of every field.
Self-describing formats invoke it from `DeserializeStruct::next_field` with
the key's bytes; non-self-describing formats ignore the schema and stay
positional.
