...
deserializable struct. Its `lookup` walks a decision tree over the key's
bytes: the length at the root, then the byte that best tells the remaining
field names apart. A key costs one walk down that tree, not one test per
declared field. Self-describing formats invoke it from
`DeserializeStruct::next_field` with the key's bytes; non-self-describing
