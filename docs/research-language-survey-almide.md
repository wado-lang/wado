...
| A6 Boundary mechanisms | No macros, no reflection                                                                          | No generation a user can invoke. The TOML and `build.rs` that `REJECTED_PATTERNS.md` names as the alternative to macros are the compiler's own build — `codegen/templates/rust.toml` is the Rust backend's syntax table, "pure formatting, no semantic logic". Serialization does exist: `Codec`, derived by writing `: Codec`, produces `Value`, with JSON the only format on the far side | —                                                                                                                                                | Rejected: macros, reflection, monkey patching, and executable build scripts — `almide.toml` is declarative TOML only, because `setup.py`-style executable configuration destroys reproducibility |
...
A6 limits C1 from the other side. A wire format does have a route in, through
`Codec`. A foreign grammar or IDL does not: there is no generation mechanism, so
nothing can turn one into Almide source at build time, and the only way in is by
hand. That means the programs you can write are its own parts plus whatever
speaks JSON. No amount of effort would have produced a Gale here, because a Gale
starts by consuming a grammar file.
