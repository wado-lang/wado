...
- `value_copy_elide` — strip a `$value_copy$T(arg)` wrapper whose target is observably read-only. Read-only-ness closes over alias chains: mutations through pattern-lowering temps, `match` payload bindings, and non-fresh call results propagate back to the root local.
