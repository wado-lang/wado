...
`WirInstr::ArrayClone` (codegen, `codegen/emit.rs`) — the `builtin::array_clone::<T>` an `Array<T>` field's `$value_copy$T` helper emits — lowers to `array.new_default` + a single native `array.copy` when `T` has no per-element copy helper (a primitive or otherwise non-value-typed element), the same default `-f no-array-copy` toggle as `WirInstr::ArrayCopy`. A value-typed element (`element_copy_func` set) still needs a per-element helper call between `array.get` and `array.set`, which `array.copy` cannot express, so that case keeps the JIT-compiled loop. Every deep copy of a struct with an `Array<T>` field for a primitive `T` (`String`'s `Array<u8>` `repr`, `List<T>` for a primitive `T`, ...) previously paid one interpreted loop iteration per element regardless of `T`.
