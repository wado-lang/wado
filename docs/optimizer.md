...
Write-backs are inserted before `return` and non-enclosing `break` statements that exit the HFS loop scope. The unlabeled `break` at `loop_depth 0` shortcut elides this pre-break write-back since the post-loop write-back placed by `scalarize_block` already covers the same scalars.

Sync placement is per call site: every reachable `Call`/`MethodCall`/`IndirectCall` that takes the scalarized local as a `&T` or `&mut T` argument is wrapped in a synthetic `Block` carrying the scalar→struct write-back before the call and the struct→scalar re-read after. Non-unit returns are bound to a fresh `_hfs_call_*` temp inside the wrapper so the block evaluates to the call's value. The re-read is elided when the callee receives the local as `&T` (immutable ref) since the callee cannot mutate through it — write-back-only sync. Branching constructs (`Match`/`If`/`Switch`/`LabeledBlock`) recurse per branch, so a call in one match arm can never trigger sync that clobbers a sibling scalar-update arm — see issue #1008.
