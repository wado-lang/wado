...
Loop-invariant arithmetic: once field-hoisting converges for a loop, each maximal loop-invariant pure-arithmetic tree (`Binary` / `Unary` whose leaves are loop-invariant scalar locals or numeric/bool/char literals) is lifted into a single pre-loop `_licm_arith_*` temp, structurally deduped so equal expressions share one temp. This runs after field-hoisting so the `_licm_*` field temps it created count as invariant operands — it is what pulls `end - start` out of the JSON deserializer's `pos < (end - start)` scan guards (CSE dedups that subtraction within an iteration but never lifts it out). Operations that can trap or carry references are excluded so a hoist never moves a trap ahead of a possibly-zero-iteration loop: `Div`/`Mod` (zero-divisor trap), `Cast` (a float→int cast lowers to the trapping `i32.trunc_f64_s` family, not `trunc_sat`), and `Ref`/`MutRef`/`Deref` / `RefEq`/`RefNotEq` (references, not arithmetic). Nested loops are left to their own recursive LICM level; loop-body `let` bindings are recorded as fully-modified and so are correctly rejected as non-invariant.

E2E: [opt_licm_immut_ref.wado](../wado-compiler/tests/fixtures/opt_licm_immut_ref.wado), [opt_licm_immut_ref_method.wado](../wado-compiler/tests/fixtures/opt_licm_immut_ref_method.wado), [opt_licm_mut_ref_no_hoist.wado](../wado-compiler/tests/fixtures/opt_licm_mut_ref_no_hoist.wado), [opt_licm_nested_ref_chain.wado](../wado-compiler/tests/fixtures/opt_licm_nested_ref_chain.wado), [opt_licm_aliased_mut_ref_params.wado](../wado-compiler/tests/fixtures/opt_licm_aliased_mut_ref_params.wado), [opt_licm_aliased_ref_fields.wado](../wado-compiler/tests/fixtures/opt_licm_aliased_ref_fields.wado), [opt_licm_invariant_arith.wado](../wado-compiler/tests/fixtures/opt_licm_invariant_arith.wado).
...
Branch joins (`If`/`Switch`/`Match`) walk each arm with cloned entry state and pick a per-candidate join target; convergence sync is inserted at each arm's exit. A call in one match arm can never trigger sync that clobbers a sibling scalar-update arm (issue #1008). Loops commit any `ScalarOnly` candidate before the body runs (so inner reads see an up-to-date field) and join entry-state with body-exit-state for the post-loop state — capturing both the zero-iterations and the `>= 1`-iteration paths. Escape paths (`return`, `break` to a non-enclosing label) commit `ScalarOnly` candidates so the field is canonical at exit.

Deferred back-edge write-back: a candidate whose field the loop body never accesses through the GC reference — no call takes the local by `&`/`&mut`, and the field is never `&`/`&mut`-referenced directly (`&mut self.f`) — is _call-clean_. Its scalar fully shadows the field for the whole loop, so it enters as `ScalarOnly` and stays `ScalarOnly` across the back-edge instead of being forced back to `Both` at body end. This eliminates the per-iteration `struct.set` the back-edge invariant would otherwise emit (one GC store per byte in `core:json`'s `skip_whitespace`, the dominant cost on pretty-printed JSON); the field is instead committed only at the loop's escape points. To keep that sound, every loop exit commits `ScalarOnly` candidates: `return` and unlabeled `break` directly, and a labeled `break` whose target block was not entered during the loop-body walk (so it escapes the HFS loop) — a no-op when the state is already `Both`, so it never re-introduces the commit-on-every-labeled-break regression, whose breaks target _registered_ in-loop labels. Candidates that are read through the reference inside the loop keep the classic `Both` entry / body-end force-`Both` discipline.
...
- Peephole — small Wasm-instruction-selection rewrites with no NIR analogue: constant comparison folding and dead `If` elimination; `eqz` and negated-comparison folding; branchless increment; byte-mask elision; sign-extension folding (`i32.extend8_s(i32.load8_u a)` → `i32.load8_s a`, idempotent re-extends, and mask drops under an extension); redundant `ref.cast` / `ref.test` elimination against the WIR static type (exact-`type_id` identity casts collapse to the operand — or to `ref.as_non_null` when only the non-null assertion remains — and always-true tests fold to `1`); GC-operand nullability relaxation; and `local.set` + first-`local.get` fusion into `local.tee`. Redundant `ref.as_non_null` elision (keyed on `WirInstr::is_nonnull_result`, which now also recognises non-nullable `ref.cast`) lives in `cleanup`. E2E: [wir_optimize_negate_eqz.wado](../wado-compiler/tests/fixtures/wir_optimize_negate_eqz.wado), [wir_optimize_branchless_increment.wado](../wado-compiler/tests/fixtures/wir_optimize_branchless_increment.wado), [wir_optimize_sign_extend.wado](../wado-compiler/tests/fixtures/wir_optimize_sign_extend.wado), [wir_optimize_local_tee.wado](../wado-compiler/tests/fixtures/wir_optimize_local_tee.wado).
