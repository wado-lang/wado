...
- `tmpl_hoist` — hoist a template string's backing buffer out of a loop and reuse it when the result does not escape the iteration. It recognises an expansion by the label `synthesis::template` stamps on it (`name::is_template_block`; the `__` prefix is reserved in source, so the label cannot come from user code), which is why `const_branch_prune` leaves that block un-flattened until the fixpoint ends. The stdlib symbols and struct fields it matches all resolve through `CompilerItem` / `SeqField` / `FormatterField` — never a spelled-out name.
