...
Deferred back-edge write-back: a candidate whose field the loop body never reads through the GC reference — no call passes the local by `&`/`&mut` — is _call-clean_. Its scalar fully shadows the field for the whole loop, so it enters as `ScalarOnly` and stays `ScalarOnly` across the back-edge instead of being forced back to `Both` at body end. This eliminates the per-iteration `struct.set` the back-edge invariant would otherwise emit (one GC store per byte in `core:json`'s `skip_whitespace`, the dominant cost on pretty-printed JSON); the field is instead committed only at the loop's escape points. To make that sound for the guard exit, a labeled `break` whose target block was not entered during the loop-body walk (so it escapes the HFS loop) commits `ScalarOnly` candidates — a no-op when the state is already `Both`, so it never re-introduces the commit-on-every-labeled-break regression, whose breaks target _registered_ in-loop labels. Candidates that are read through the reference inside the loop keep the classic `Both` entry / body-end force-`Both` discipline.
