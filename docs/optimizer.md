...
- `field_scalarize` — shadow hot GC fields in scalar locals across a loop, with dataflow-driven write-back and re-read. A nested loop is inside that scope, not an exit from it: only the candidates some call in its body can reach are committed before it and re-read after, and an unlabeled `break` out of it is joined with the loop's other exits rather than committing every scalar on the spot. A bit-buffer refill loop reaches nothing, so it syncs nothing — worth 7% of `core:zlib`'s inflate.
