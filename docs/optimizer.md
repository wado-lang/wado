...
- `inline` — replace calls to small, non-recursive functions with their body; reference parameters and receivers inline too. `#[inline]` raises the budget 5x, `#[inline(always)]` forces it, `#[inline(never)]` and cold call sites opt out. Size is the callee's hot path priced in emitted Wasm instructions, so control flow outweighs a straight-line operation and a cold or unreachable tail prices nothing; the weights are in the pass's `weight` module. A callee over budget as written gets a second reading **after the fold its callers' constants cause**: a branch a constant argument decides costs the one arm it keeps, and a call the compile-time engine runs on constants costs nothing. Which parameters arrive constant is taken across every call site, so admission stays a property of the callee. Folded size alone would admit every marginal fold — measured -9% on cbor-twitter — so the fold must also delete a loop, or halve the body. Such a callee then receives no inlining itself: a body worth more copied than called is destroyed by growing it past the budget, and that is one-way. Holding it is what an `#[inline(never)]` on each leaf otherwise applies by hand. A body carries two prices, and they part company exactly where it keeps a cold arm: `inline_cost` is what running one copy costs, which is what the threshold judges, and `inline_size` is what holding one costs, cold paths included and only what a divergence makes unreachable left out. `InlineBudget` spends the second — a callee's growth being its size across its call sites, less the body that goes with the last one — over a cap set as a percentage of the unit as the loop found it, cheapest body first. What the budget turns down is reported at debug level, naming both prices. `cold_outline` is what keeps the two in agreement, so no level sets a growth cap by default.
- `cold_outline` — move what a `cold_path()` marker opens into a function of its own, so `inline`'s cold discount describes the callee rather than promising a split nobody made. The caller keeps the branch; the marked arm becomes a call. A region moves when control cannot leave it — a `break` or `continue` targeting a label or loop the region carries itself is not a way out — and every local it touches is one the call can hand over: a parameter the region only reads, passed straight through as the enclosing function received it; one the region declares; or a slot nothing outside the region reads, which moves with it unless a loop encloses the region, where a second entry would find the slot fresh rather than holding what the first left. A write is what no call carries back, a parameter slot included: `mut n` assigned inside the region would leave the enclosing frame holding the value it was called with. A region that is already a lone call is left alone, which is also what makes a second scan a fixed point. Runs once, before the loop, so the inliner never sees the unsplit shape. Two things are open. It costs `sieve` 4.5% for no reason the IR shows — the hot loops are identical in WIR and in the emitted Wasm, and the threshold perturbs the row by only 2.3% — so the cause is below the Wasm and needs `perf` against a jitdump. And a region runs to the end of its block, so a marker mid-loop-body cannot be taken: the loop's own bookkeeping falls inside the region and writes locals the next iteration reads. Ending the region at the last statement that can travel would take that shape, which `core:json`'s escape tail splits by hand for want of it.
