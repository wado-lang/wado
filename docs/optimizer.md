...
2. Box-local elimination — substitute the field read for a `Box<T>` local lowering minted, then retype the ones adjacency cannot move to the field they wrap. A by-reference `for` bumps the index between a box's definition and its use, so nothing may move there — and nothing has to.
