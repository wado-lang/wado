...
- `inline` — replace calls to small, non-recursive functions with their body; reference parameters and receivers inline too. `#[inline]` raises the budget 5x, `#[inline(always)]` forces it, `#[inline(never)]` and cold call sites opt out. A body spliced in statement position is scanned again in the same round, with each callee priced as written. That re-scan may add at most the budget squared. Without the cap, a call tree that doubles per level would copy its leaves exponentially. A callee over budget as written is re-read under the constants its callers pass. A callee is _held_ when constant parameters would fold a loop out of its body. Nothing is spliced into a held callee, so it stays small enough to be admitted at the folded price once the constants arrive. The hold is a bet on a later round. Once the loop has converged with the hold still in place, the bet is settled: the holds are released, and the loop runs on so the held functions receive inlining like any other. `--optimize-inline-growth` additionally caps what the pass adds to the whole unit; no level sets it.
