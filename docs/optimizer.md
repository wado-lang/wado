...
- `const_object_globalization` — hoist constant read-only aggregate `let` bindings into shared immutable globals; see [WEP](./wep-2026-05-31-const-object-globalization.md). Also hoists a constant aggregate literal referenced via `&` directly at an expression position with no enclosing `let` (e.g. a synthesized `serde` field key, `st.field(&"id_str", …)`) — the same shape, minus the name, so it is rewritten in place instead of replacing a `let` statement. Skips a `let` whose value already qualifies for the whole-binding case, since hoisting both would nest one global's `GlobalVarSet` inside another's initializer. A hoisted string literal needs the `array.new_fixed` repr to promote eager regardless of the shared `string_inline_max_bytes` threshold (which stays conservative for ordinary code, e.g. `assert` diagnostic templates that must stay a byte-scannable `array.new_data` segment) — `wir_build::translate::FunctionTranslator::force_fixed_string_repr` scopes that override to just a hoisted global's own `GlobalVarSet` value.
