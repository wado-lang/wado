...
- `inline` — replace calls to small, non-recursive functions with their body; reference parameters and receivers inline too. `#[inline]` raises the budget 5x, `#[inline(always)]` forces it, `#[inline(never)]` and cold call sites opt out. Size is the callee's hot path priced in emitted Wasm instructions, so control flow outweighs a straight-line operation and a cold or unreachable tail prices nothing; the weights are in the pass's `weight` module. A callee over budget as written gets a second reading **after the fold its callers' constants cause**: a branch a constant argument decides costs the one arm it keeps, and a call the compile-time engine runs on constants costs nothing. Which parameters arrive constant is taken across every call site, so admission stays a property of the callee. Folded size alone would admit every marginal fold — measured -9% on cbor-twitter, more inlining and none of it paying — so the fold must also delete a loop, or halve the body, to count. A callee that is over budget as written, under it with its parameters assumed constant, and loses a loop when they are, then receives no inlining itself. Such a body is worth more as something callers copy than as something callers call, and growing it is one-way: it passes the budget and every caller loses the fold. Holding it is what an `#[inline(never)]` on each leaf otherwise applies by hand. The admission reads the call sites and the hold reads the body alone, because the hold protects an option whose evidence has not arrived yet — a `Serialize` derivation computes its wire key with a compile-time call, so the key is a literal only rounds after the leaves would have been spliced in. Keeping a frozen copy to splice from instead, so the original could grow freely, was built and measured slower than not growing it: a detached copy is code no other pass can reach, so `sroa_param` and `dae` rewrite a signature under it and it has to be discarded. `classify_callee` records the numbers.
