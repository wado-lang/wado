...
- `licm` — hoist loop-invariant field-access chains and non-trapping arithmetic out of loops. A field load blocked only by an opaque `&mut`-call clobber of its pointee type (a may-alias the compiler cannot rule out, e.g. `write_escaped_string(&mut buf, &s)` where a caller could pass `buf === s`) is still hoisted to the pre-header and reloaded right after each clobbering statement, so the common clobber-free path drops the per-byte struct load while a genuine alias keeps observing the fresh field. Gated so no clobbering statement also reads the hoisted field.
