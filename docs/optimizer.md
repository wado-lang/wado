...
2. Fixed-point loop (skipped at `-O0`), in order: `container_sroa`, peephole (pre-inline; hosts `match_to_switch` and `value_copy_elide` as rules), `value_copy_demote`, `sroa_param`, `inline`, peephole (post-inline; hosts `array_literal`, `labeled_block_fusion`, `match_thread`, `ref_elim`, and `elide_box_local` as rules), `sroa`, `copy_prop`, `dae`, `drve`, `const_folding`, `licm`, `tmpl_hoist`. (`match_to_switch` on global initializers runs once before the loop; `-O0` lowers everything via `match_to_switch_all`.)
...
- `match_thread` — jump-thread `match L:{… break L: Case(p) …} { arms }`, the value-producing shape `inline` leaves when a variant-returning callee is a match scrutinee (every inlined `x = f()?`). Each break site becomes its statically selected arm (payload bound, divergent arms keep their early exit), and the labeled block yields the match result directly — the intermediate variant is never allocated or dispatched on. Rewrites the `Match` node in place, so it applies in any position; exits nested in `Switch` / inner-`match` arms thread too. Bails on guards, non-`Variant`/`Wildcard` patterns, `null` (`Option::None`) breaks, and loop-exit capture.
