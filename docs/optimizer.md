...
- `match_to_switch` — lower a dense integer/enum `match` to a `br_table` switch, from twelve arms up. One indirect branch replaces a cascade the predictor gets right, so it pays only once the cascade is long.
