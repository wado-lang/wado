...
- `value_copy_elide` — strip a `$value_copy$T(arg)` wrapper whose target is observably read-only. Read-only-ness closes over alias chains: mutations through pattern-lowering temps, `match` payload bindings, and non-fresh call results propagate back to the root local. Source-side it is field-sensitive: a copy still strips when the source's read paths are provably disjoint from every mutation of the same storage — distinct struct fields or variant cases — so an iterator's `next` bumping `self.index` no longer forces a per-element deep copy of `self.repr`. Disjointness needs the mutated local to be the sole precise mutator of its alias component; anything coarser falls back to the whole-local read-only bit.
