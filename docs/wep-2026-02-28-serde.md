...
- What builds a constrained type from its checked `Base`. A newtype's `From` is
  ...
  none at all. The two ways to close it are a constructor on `Constrained` and a
  `Self: From<Self::Base>` bound — and the bound is unspellable while a trait
  bound takes associated-type bindings but no type arguments.
