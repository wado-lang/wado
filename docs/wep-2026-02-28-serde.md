...
- What the blanket converts a checked `Base` into for a type that is not a
  newtype. A newtype carries a bidirectional `From` ([Conversion
  Traits](./wep-2026-03-16-conversion-traits.md) §7); a struct wrapper — the
  vehicle when the invariant must hold against in-process construction — carries
  nothing, so the trait states no contract the blanket can call. Closing this
  means either a bound (`Self: From<Self::Base>`) or a constructor on
  `Constrained` itself.
