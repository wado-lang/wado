...
All core serde features are implemented and passing. The generic trait method blocker has been resolved.

### What's Implemented

- **Traits**: `Serialize`, `Deserialize`, `Serializer`, `Deserializer`, and all sub-traits
- **Builtin impls**: All primitives, `String`, `bool`, `char`, `Option<T>`, `Array<T>`
- **Compiler synthesis**: `impl Serialize for Type;` and `impl Deserialize for Type;` for structs
- **JSON format**: `core:json` module with `to_string<T>()` and `from_string<T>()`
- **Error handling**: `SerializeError` (NaN/Infinity rejection) and `DeserializeError` (malformed input, missing fields, type mismatch, trailing data, EOF)
- **Field naming**: Automatic `snake_case` → `camelCase` conversion for struct fields
- **Trait bound enforcement**: Compile error when types lack `Serialize`/`Deserialize` impls

### What's Not Yet Implemented

- `#[serde(rename)]` and `#[serde(rename_all)]` attributes
- `#[serde(default)]` attribute for optional fields
- Variant serialization/deserialization synthesis (only structs are synthesized)
- `core:json_nsd` (non-self-describing JSON)
- `core:cbor` and `core:cbor_nsd`
- Large integer JSON string encoding (i64/u64/i128/u128 > 2^53)
- `TreeMap` and tuple `Serialize`/`Deserialize` impls

## E2E Test Status

E2E test fixtures in `wado-compiler/tests/fixtures/serde_*.wado` validate the serde design against the compiler. All fixtures pass.

### Trait and Serializer Fundamentals

| Fixture                      | Description                                                |
| ---------------------------- | ---------------------------------------------------------- |
| `serde_error_types`          | `SerializeError`, `DeserializeError` enums and structs     |
| `serde_generic_trait_method` | Generic method on a trait (the former blocker)             |
| `serde_serialize_trait`      | `Serialize` trait with `Serializer` trait, manual impls    |
| `serde_sub_serializer`       | `SerializeSeq` and `SerializeStruct` sub-serializer traits |
| `serde_deserialize_trait`    | `Deserialize` trait (static method), `Deserializer` trait  |

### Manual Serialize/Deserialize Implementations

| Fixture                     | Description                                                         |
| --------------------------- | ------------------------------------------------------------------- |
| `serde_serialize_struct`    | Manual `Serialize` impl for a user-defined struct                   |
| `serde_serialize_enum`      | Manual `Serialize` impl for an enum via `serialize_unit_variant`    |
| `serde_serialize_variant`   | Manual `Serialize` impl for a variant with payloads                 |
| `serde_serialize_option`    | Bounded `impl<T: Serialize> Serialize for Option<T>`                |
| `serde_serialize_array`     | Bounded `impl<T: Serialize> Serialize for Array<T>`                 |
| `serde_full_json_serialize` | Complete JSON serializer: nested structs, Option, Array, primitives |

### Compiler-Synthesized Implementations

| Fixture                   | Description                                                     |
| ------------------------- | --------------------------------------------------------------- |
| `serde_synth_serialize`   | Auto-generated `Serialize` impl with `snake_case` → `camelCase` |
| `serde_synth_deserialize` | Auto-generated `Deserialize` with field lookup and bitmask      |
| `serde_synth_roundtrip`   | Full struct → JSON → struct roundtrip                           |

### JSON Error Handling

| Fixture                       | Description                                                     |
| ----------------------------- | --------------------------------------------------------------- |
| `serde_json_ser_error`        | NaN, Infinity, -Infinity rejection for f32/f64                  |
| `serde_json_deser_error`      | Invalid JSON, missing fields, type mismatch, trailing data, EOF |
| `serde_json_array_string`     | `Array<String>` serialization regression test                   |
| `serde_json_multi_type_deser` | Multiple type monomorphization handling                         |

### Compile Error Tests

| Fixture                        | Description                                      |
| ------------------------------ | ------------------------------------------------ |
| `serde_compile_error_no_impl`  | Compile error when `Serialize` not implemented   |
| `serde_compile_error_no_deser` | Compile error when `Deserialize` not implemented |
