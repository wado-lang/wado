...
`#[serde(positional)]` marks a field as ordinal — a general, format-agnostic
hint that name-only or sequence-only formats ignore (backward compatible). The
schema gains one method enumerating ordinal fields in declaration order;
`lookup` omits them (a positional field is never matched by name):
...
over positional ones. The generated `Deserialize` loop (`next_field` + `value`)
is unchanged — only the format's `DeserializeStruct` consults `positional_at`,
...
field becomes optional in one of two mutually exclusive ways — unifying serde
with [default field values](./wep-2026-04-11-default-arguments.md), added later.

#### Declared field default (preferred)

A field with a declared default is optional and falls back to it — no attribute.
The same rule as construction and calls, extended to deserialization (and to
[`core:args`](./wep-2026-06-22-core-args.md)):
...
For a **defaultless** field, `#[serde(default)]` falls back to the type's
zero-value (`Default::default()`):

```wado
struct Config {
    host: String,                          // required
    #[serde(default)] port: i32,           // → 0
    #[serde(default)] tags: List<String>,  // → []
}
```

A declared default and `#[serde(default)]` on the same field is a compile error:
they give conflicting fallbacks (`expr` vs zero-value) for the same condition —
always redundant or contradictory. Pick one.
...
An absent `Option<T>` field deserializes to `null` with no annotation, aligning
with Rust serde.
