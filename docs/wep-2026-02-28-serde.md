...
- What builds a constrained type from its checked `Base`. A newtype's `From` is
  ...
  none at all. The supertrait `Constrained: From<Self::Base>` does not reach one
  either: every impl of `Constrained` is rejected as not implementing `From`,
  the ones that spell it included.
