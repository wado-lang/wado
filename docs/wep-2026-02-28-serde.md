...
- What builds a constrained type from its checked `Base` inside a generic body.
  The supertrait `Constrained: From<Self::Base>` states the requirement and each
  impl is held to it, but the call does not follow: `T::from(b)` under
  `T: Constrained` asks for `From` at `Base` as the clause spells it and reaches
  no impl ([Trait Resolution](./wep-2026-09-01-trait-resolution.md), known gaps).
