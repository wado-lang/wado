...
E2E test fixtures in `wado-compiler/tests/fixtures/serde_*.wado` validate the serde design against the compiler.
...
### TODO (blocked on generic trait methods)

All remaining fixtures are blocked by a compiler limitation: **generic methods on trait definitions** (method-level type parameters in traits) are not resolved in the WIR phase. The pattern `fn serialize<S: Serializer>(&self, s: &mut S)` on a trait is fundamental to the serde design.

| Fixture                      | Description                                                                          |
| ---------------------------- | ------------------------------------------------------------------------------------ |
| `serde_generic_trait_method` | Isolated test for generic method on a trait (the blocker)                            |
| `serde_serialize_trait`      | `Serialize` trait with `Serializer` trait, manual impls for i32/String               |
| `serde_sub_serializer`       | `SerializeSeq` and `SerializeStruct` sub-serializer traits                           |
| `serde_serialize_struct`     | Manual `Serialize` impl for a user-defined struct (what the compiler would generate) |
| `serde_serialize_enum`       | Manual `Serialize` impl for an enum via `serialize_unit_variant`                     |
| `serde_serialize_variant`    | Manual `Serialize` impl for a variant with payloads via `begin_variant`              |
| `serde_serialize_option`     | Bounded `impl<T: Serialize> Serialize for Option<T>`                                 |
| `serde_serialize_array`      | Bounded `impl<T: Serialize> Serialize for Array<T>` with `begin_seq`                 |
| `serde_deserialize_trait`    | `Deserialize` trait (static method), `Deserializer` trait, mock deserializer         |
| `serde_full_json_serialize`  | Complete JSON serializer: nested structs, Option, Array, primitives                  |

### Unblocking

Once generic methods on trait definitions are supported in WIR, remove `"TODO": true` from each fixture and verify they pass.
