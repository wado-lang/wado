...
- What builds a constrained type from its checked `Base` inside a generic body.
  The supertrait `Constrained: From<Self::Base>` states the requirement, and
  every impl is held to it, but `T::from(b)` under `T: Constrained` asks for
  `From` at `Base` as the clause spells it and reaches no impl
