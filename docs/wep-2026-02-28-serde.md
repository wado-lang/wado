...
```wado
trait FieldSchema {
    fn lookup(key: Slice<u8>) -> Option<i32>;  // excludes positional fields
...
CLIs) and lets [`core:args`](./wep-2026-06-22-core-args.md) layer "ordinal =
...
Every body is a library derivation over the reflection API — a `Reflect*`-bound
blanket `impl` in `core:serde`, not compiler output (WEP
[Library-Defined Derivation](./wep-2026-06-13-reflect-derivation.md)). The
shapes below are what those derivations produce. The only serde code the
compiler still synthesizes is the per-struct `FieldSchema` the deserializer
reads.
...
[Serialization Names](#serialization-names) for `name` / `name_policy`
...
#### Struct Deserialization (Slot Tuple)

The `ReflectStruct` blanket holds the values the wire delivers in a slot tuple
`[..Option<F>]`, seeded with the fields' declared defaults, and feeds it to
`from_fields` in declaration order (see
[Building a struct from a streaming format](./wep-2026-06-13-reflect-derivation.md#building-a-struct-from-a-streaming-format)).
Fields are matched by index, and the same shape serves every format — what
`next_field` means is the format's to decide (below). An index the format does
not report leaves its slot as seeded: the declared default, or a `MissingField`
...
[field default value](./wep-2026-04-11-default-arguments.md) is the **single**
...
(and to [`core:args`](./wep-2026-06-22-core-args.md)):
...
A wire boundary checks a value's permitted range — a name at most 64 bytes, an
age in 0…150, a currency code matching `^[A-Z]{3}$`. The check belongs to the
_type_, not to the field holding it: declared once, it guards every boundary
that admits the type. A per-field attribute would restate the rule at each field
and leave the type itself unconstrained.

`Constrained` is how a type states its own permitted range:

```wado
pub struct Constraint {
    pub min_length: Option<i64> = null, pub max_length: Option<i64> = null,
    pub minimum: Option<f64> = null,    pub maximum: Option<f64> = null,
    pub exclusive_minimum: Option<f64> = null, pub exclusive_maximum: Option<f64> = null,
    pub multiple_of: Option<f64> = null,
    pub min_items: Option<i64> = null,  pub max_items: Option<i64> = null,
    pub unique_items: bool = false,
    pub pattern: Option<String> = null, pub format: Option<String> = null,
}

pub trait Constrained {
    type Base: Deserialize;      // `type UserName = String` → String
    fn constraint() -> Constraint;
}

// The boundary guard, once, for every constrained type: read the base, check
// it, wrap it. A newtype's wrap is the `From` the compiler already generates.
impl<T: Constrained> Deserialize for T { … }
```

`Base` is what makes the guard writable at all — the blanket reads the
underlying value, checks it, and converts. It is also what a schema library
delegates to for the shape the keywords qualify: `{"type":"string"}` comes from
`Base`, `minLength` from the `Constraint`.

A newtype is the vehicle: it is a distinct type that inherits its base's
behaviour, so declaring the constraint is the whole definition.
...
impl Constrained for Age {
    type Base = i32;

    fn constraint() -> Constraint {
        return Constraint { minimum: Option::Some(0.0), maximum: Option::Some(150.0) };
    }
}

struct CreateUser {
    user_name: UserName,   // guarded wherever it is deserialized
    age: Age = 0,
}
```

The constraint is a _value_, so a schema library reads the same `constraint()` a
boundary enforces and the two cannot drift. A per-field attribute bought that by
making the compiler the single source; the type is one instead.

Enforcement splits by who owns the semantics. `core:serde` checks what it can
decide alone — the lengths, the numeric bounds, `multiple_of`, the item counts,
uniqueness — raising `DeserializeError` (`InvalidValue`) naming the field.
`pattern` and `format` it carries without enforcing: a regex engine and a format
vocabulary are library-sized, and JSON Schema itself treats `format` as an
annotation until a consumer opts into asserting it. A validator owning an engine
([Jade](./wep-2026-06-13-jade.md)) asserts them; `core:serde` never claims to.

A type whose rule is not expressible as a `Constraint` skips `Constrained` and
writes `Deserialize` by hand — a hand-written impl wins over any blanket ([Trait Derivation Policy](./wep-2026-06-25-trait-derivation.md)) — so the closed key set
bounds what is _declared_, never what can be checked.

A newtype's own value is not itself guarded: `"" as UserName` and a literal
coercion both bypass the check, because a newtype's `From` is bidirectional and
free ([Conversion Traits](./wep-2026-03-16-conversion-traits.md) §7).
`Constrained` guards the wire boundary — the untrusted direction — and a type
that must hold its invariant against in-process construction needs a struct
wrapper with a private field and a `TryFrom` constructor instead.

Known gaps:

- The blanket depends on [#1932](https://github.com/wado-lang/wado/issues/1932):
  a newtype currently takes its base's blanket over one whose bound the newtype
  itself satisfies, so `impl<T: Constrained> Deserialize for T` would not fire
  for `type UserName = String`. Until that is fixed the guard has to be written
  directly on each constrained type.
- What builds a constrained newtype from its checked `Base`. A struct wrapper
  works: `Constrained: From<Self::Base>` reaches the `impl From<String>` it
  writes, so `T::from(b)` under `T: Constrained` builds one. A newtype's `From`
  is auto-generated
  ([Conversion Traits](./wep-2026-03-16-conversion-traits.md) §7), and the
  supertrait check does not see it, so every `Constrained` impl on a newtype is
  rejected as not implementing `From`. It is the same obligation check that
  admits a projecting clause only where the impl writes the binding itself
  ([Super Traits](./wep-2026-07-27-super-traits.md), known gaps).
- Whether a constraint reaches through `Option<T>` and `List<T>` to the element,
  or applies only to the type that declares it.
- Whether `Base` stays an authored associated type or is read from
  `ReflectNewtype::Base` once reflection covers the newtype kind
  ([#1933](https://github.com/wado-lang/wado/issues/1933)). Reading it would make
  a newtype's `Constrained` impl one line, but it would also bind `Constrained`
  to newtypes and shut out the struct wrapper.
- Where the offset in the raised error comes from: `DeserializeStruct` reports
  the field index, not the value's span.
...
- Self-describing (`core:json`) reads the next key and resolves it through
  `S::lookup`, yielding the field's index — or a sentinel the caller answers
  with `skip()` for a key the schema does not know.
- Non-self-describing (`core:json_nsd`) ignores `S` and yields `0, 1, 2, …` in
  declaration order, reading the object as an array. Serialization mirrors it:
  the field name is dropped and the value written as the next element.
...
4. **Bound-driven derivation**: `impl Serialize for Type;` requires no procedural macro system. The bodies are library blankets over the reflection API, monomorphized per type, so they fold to what a hand-written impl would emit.
...
- **Alternative variant representations**: Internally tagged (`#[wire(tag = "type")]`), adjacently tagged (`#[wire(tag = "type", content = "data")]`), and untagged (`#[wire(untagged)]`). Currently only the default externally tagged representation is supported.
