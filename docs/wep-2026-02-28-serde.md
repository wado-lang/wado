...
- What builds a constrained type from its checked `Base`. A newtype's `From` is
  auto-generated ([Conversion Traits](./wep-2026-03-16-conversion-traits.md) §7)
  but invisible to a generic body that cannot name it, and a struct wrapper has
  none at all. `Constrained: From<Self::Base>` parses and resolves, but the
  supertrait check reads the bound's written `Self::Base` without projecting it
  through the impl's own `type Base`, so it rejects a type carrying the
  matching `From`.
