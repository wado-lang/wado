...
trait Deserializer {
    ...
    fn begin_struct(&mut self, name: &String, num_fields: i32,
                    lookup: fn(&String, i32, i32) -> Option<i32>) -> Result<Self::StructAccess, DeserializeError>;
...
- **lookup**: A function `fn(&String, i32, i32) -> Option<i32>` that maps a byte slice of the input string to a field index. The three parameters are `(input, start, end)` — the function compares `input[start..end]` against known field names using byte-level comparison, avoiding substring allocation during parsing. Self-describing formats call this to convert input key strings to field indices. Non-self-describing formats ignore it.
...
**`DeserializeStruct::next_field()`** — Returns the next field as an `i32` index. Self-describing formats convert field names to indices using a lookup function provided by the generated code; non-self-describing formats return sequential indices (0, 1, 2, ...).
...
fn _user_field_lookup(input: &String, start: i32, end: i32) -> Option<i32> {
...
The field enum documents the mapping between field names and indices. The lookup function matches byte slices of the input against known field names using byte-level comparison (avoiding substring allocation), and is passed to `begin_struct` for use by self-describing formats.
...
- **SD format** (e.g., `core:json`): `begin_struct` stores the lookup function. `next_field()` locates the next key in the input buffer and calls `lookup(input, start, end)` with the byte range → `Some(0)`, returns `0`.
...
impl Deserializer for JsonDeserializer {
    ...
    fn begin_struct(&mut self, name: &String, num_fields: i32,
                    lookup: fn(&String, i32, i32) -> Option<i32>) -> Result<JsonStructAccess, DeserializeError> {
        self.expect_char('{')?;
...
impl DeserializeStruct for JsonStructAccess {
    fn next_field(&mut self) -> Result<Option<i32>, DeserializeError> {
        ...
        if let Some([start, end]) = key {
            let idx = (self.lookup)(&self.deserializer.input, start, end);
            if let Some(i) = idx {
...
1. `next_field()` → locates `"age"` at bytes [start, end) → `lookup(input, start, end)` → `Some(1)`, then `value()` reads `30`
2. `next_field()` → locates `"userName"` at bytes [start, end) → `lookup(input, start, end)` → `Some(0)`, then `value()` reads `"Alice"`
...
impl Deserializer for JsonNsdDeserializer {
    ...
    fn begin_struct(&mut self, name: &String, num_fields: i32,
                    lookup: fn(&String, i32, i32) -> Option<i32>) -> Result<JsonNsdStructAccess, DeserializeError> {
...
- **Static field-lookup dispatch (drop the `lookup` closure)**: `begin_struct` currently takes a runtime `lookup: fn(&String, i32, i32) -> Option<i32>`. The derive synthesizes this argument as a zero-capture closure wrapping the per-type `_T_field_lookup` function. Because Wado lowers every `fn`-typed value to a heap closure, this is allocated on every struct decode — two GC objects (the empty functor env plus the canonical-closure wrapper) per object — which is pure overhead in hot deserialization loops (e.g. one decode per array element). The lookup is type-specific but format-agnostic knowledge known at compile time, so it need not be a runtime value at all. A future redesign can replace the `lookup` parameter with a zero-size type parameter `S: FieldSchema` whose static `S::lookup(input, start, end) -> Option<i32>` is resolved at monomorphization, turning the indirect call into a direct, inlinable call with no allocation. This mirrors how Rust serde threads a zero-size `Visitor` / field-`Deserialize` type rather than a closure. Self-describing formats call `S::lookup`; non-self-describing formats (`core:json_nsd`) ignore `S` and stay positional, so the change is uniform across `core:json`, `core:json_nsd`, and `core:router`. It is the codegen-side counterpart to optimizer-driven closure hoisting: it removes the per-decode allocation by construction instead of relying on a later pass to recover it. Cost: it threads an extra type parameter through `Deserializer::begin_struct`, `DeserializeStruct`, and the derive.
