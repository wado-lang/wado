...
A wire boundary is where a value's permitted range is checked — a name is at
most 64 bytes, an age is 0…150, a currency code matches `^[A-Z]{3}$`. The check
belongs to the _type_, not to the field that happens to hold it: a constrained
value is a distinct type, its constraint is declared once, and every boundary
that admits the type gets the same guard. A per-field attribute would instead
restate the rule at each field and leave the type itself unconstrained.
...
// it, wrap it. The wrap is the `From` a newtype already carries.
...
The constraint is a _value_, not a keyword list, so a schema library reads the
same `constraint()` a boundary enforces and the two cannot drift — the property
a per-field attribute bought by making the compiler the single source, obtained
instead by making the type one.
...
uniqueness — and raises `DeserializeError` (`InvalidValue`) with the field's
offset. `pattern` and `format` are carried, not enforced: a regex engine and a
format vocabulary are library-sized, and JSON Schema itself treats `format` as
an annotation until a consumer opts into asserting it. A validator that owns an
engine (Jade, see [Jade](./wep-2026-06-13-jade.md)) asserts them; `core:serde`
never claims to.
