...
Field identity has two axes: a field is addressable by **name** (nominal) or by
**position** (ordinal). Whole-struct formats already pick one — SD formats
(`core:json`) resolve every field nominally via `FieldSchema::lookup`, NSD
formats (`core:json_nsd`) resolve every field ordinally by sequence. Some
formats need to **mix both within one struct**: a command-line parser
(`core:args`) reads most fields as `--name` options but a few as positional
arguments.

The `#[serde(positional)]` field attribute marks a field as ordinal. It is a
general, format-agnostic hint — formats that resolve purely by name or purely
by sequence ignore it, so it is backward compatible. The per-type schema gains
one method enumerating the ordinal fields in declaration order, and `lookup`
omits them (a positional field is never matched by name):

```wado
trait FieldSchema {
    fn lookup(input: &String, start: i32, end: i32) -> Option<i32>;  // excludes positional fields
    fn positional_at(rank: i32) -> Option<i32>;                      // index of the rank-th ordinal field, or null
}
```

The synthesizer emits both from the field list: `lookup` over the
non-positional fields, `positional_at` over the positional ones in source
order. The generated `Deserialize` driver loop (`next_field` + `value`) is
unchanged — only the format's `DeserializeStruct` consults `positional_at`,
assigning bare input tokens to the next unfilled ordinal field. SD/NSD JSON and
CBOR never call it.

This keeps the positional concept in serde (it is about the data model and the
type's schema, not about CLIs) while letting `core:args` layer "ordinal field =
positional argument, nominal field = `--option`" on top. See
[WEP: Command-Line Argument Parsing](./wep-2026-06-22-core-args.md).
...
Note: each field name is serialized verbatim by default (identity); see [Serialization Names](#serialization-names) for `rename` / `rename_all` overrides.
...
Struct field names are used verbatim by default (identity): the field name as written is the single source of truth for the wire key. A struct serialized for a camelCase-expecting consumer opts in with `#[serde(rename_all = "camelCase")]`. Enum, flags, and variant case names likewise remain as-is (PascalCase identity) because they represent type-level or constant-level identifiers.
...
A missing required field during deserialization produces a `DeserializeError`
with `MissingField` kind. A field is **not** required — it is filled when absent
— in two ways. This WEP predates [default field values](./wep-2026-04-11-default-arguments.md);
the two are now unified under the language's "has default → optional" rule.

#### Declared field default (preferred)

A field with a declared default is optional on deserialize and falls back to
that default when absent — no attribute needed. This is the same rule that makes
the field omittable at construction and in calls, now extended to deserialization
(and to argument parsing in [`core:args`](./wep-2026-06-22-core-args.md)):

```wado
struct Config {
    host: String,            // required — error if missing
    port: i32 = 8080,        // optional — 8080 if missing
    verbose: bool = false,   // optional — false if missing
}
```

#### `#[serde(default)]` (zero-value fallback)

`#[serde(default)]` makes a **defaultless** field optional, falling back to the
type's zero-value (`Default::default()`). Use it when no declared default is
appropriate but absence should still be tolerated:

```wado
struct Config {
    host: String,                     // required — error if missing
    #[serde(default)]
    port: i32,                        // 0 if missing
    #[serde(default)]
    tags: List<String>,              // [] if missing
}
```

A declared field default and `#[serde(default)]` on the same field is a compile
error. The two specify different fallbacks (`expr` vs the type's zero-value) for
the same absent-field condition, so the combination is always either redundant
or contradictory — never separately meaningful. Pick one: a declared default for
a specific value, `#[serde(default)]` for the zero-value.
...
#### `Option<T>` is implicitly optional

An `Option<T>` field is optional with no annotation: an absent field
deserializes to `null`. This aligns Wado with Rust serde and removes the
redundant `#[serde(default)]` the earlier examples carried on `Option` fields.
...
- **Generic struct Deserialize synthesis (and `FieldSchema` naming alignment)**: Synthesized struct Deserialize currently targets only non-generic named structs. `generate_struct_deserialize` collects `req.type_params` but never consumes them: the per-field machinery is built on the raw, unsubstituted type-param field types and `deser_func` is emitted with `impl_type_params: []`, so there is no per-instantiation monomorphization. This predates the `FieldSchema` change and affects `deserialize` itself. When generic struct synthesis is implemented, also align the two field-schema name sources: `next_field::<S>`'s type argument is spelled via `tt.type_name(struct_type)` (e.g. `Pair<T>` for a generic instance), while the synthesized `impl FieldSchema for <Type>` is keyed off `req.target_type_name` (the base name `Pair`, args dropped by `get_type_name`). For non-generic structs both are the bare struct name and coincide exactly, so there is no live bug; for generics they would diverge. Derive both from the same source (or rely solely on the `TypeId`-driven resolution that already backs `value::<FieldType>()`) so the `next_field` type argument and the `FieldSchema::lookup` impl key agree.
