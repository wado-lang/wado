...
`task return expr;` delivers the function's result to the CM runtime without terminating the function. Valid only inside `export async fn`. Regular `return` is forbidden in `async fn` bodies. A Wado call of such a function gets the delivered value back as an ordinary return value, so a test can assert on it.
