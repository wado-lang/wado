...
3. **B — derivation** is the only layer gated on the companion language WEP
   (`ReflectStruct` synthesis, per-field metadata, `#[validate]`). It lands as those
...
- **Capability B (type → schema)** is blocked on `ReflectStruct` (unimplemented).
  Its originally-planned first consumer, a schema for
  [Kiln](./wep-2026-04-12-kiln.md) generator options, no longer needs it:
  ...
  Library-level `#[derive(Jade)]` lands with `ReflectStruct`.
