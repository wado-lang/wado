...
`type` is a contextual keyword, so the field is spelled `type` directly. Fields
whose JSON key collides with a hard keyword (`ref_`, `not_`, …) keep a `_` suffix
and serialize under the exact JSON Schema key.
