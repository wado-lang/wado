...
- **The base's schema, in the constrained blanket.** `impl<T: Constrained +
  ...
  (`T::Base: JsonSchema`), which the grammar has no form for — a bound reaches
  the parameter, not a projection of it — or Jade producing the base shape
  itself from a closed set of base types.
