...
- **Which blanket owns a constrained composite.** A struct wrapper that
  ...
  one owner: either a constrained type must be a newtype, whose reflected kind
  no structural blanket matches, or the two blankets merge into one that asks
  for the constraint and falls back to the structure.
