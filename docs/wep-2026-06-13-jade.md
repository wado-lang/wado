...
{
    fn schema_name() -> String { return Reflect::<T>::type_name(); }
...
- Assertion keywords (`minLength`, `minimum`, `pattern`, `format`, …) from the
  field's _type_. A constrained value is a distinct type declaring its own
  permitted range through `core:serde`'s `Constrained` (see
  [Serde](./wep-2026-02-28-serde.md)), and Jade's `impl<T: Constrained>
  JsonSchema for T` takes `T::Base`'s schema for the shape and adds the
  `Constraint`'s keywords to it — `{"type":"string"}` from the base, `minLength`
  from the constraint. The same value guards the `Deserialize` boundary, so a
  schema and the parser that enforces it cannot disagree, and the constraint is
  stated once no matter how many structs carry the type. The blanket is
  `impl<T: Constrained + Reflect> JsonSchema for T`: the `$defs` key comes from
  the identity root, which a constrained type satisfies whether it is a newtype
  or a struct wrapper — a kind bound would admit only one of the two.
...
A named constrained type is also a better schema: it earns a `$defs` entry and a
`$ref` per use, which is what a reader of the emitted document expects, where a
per-field attribute could only inline the same keywords at every site.
...
- **`ReflectStruct` synthesis + `where`-clause pack binding** — powers the value-free
  `impl<T, ..F> … where T: ReflectStruct<FieldTypes = [..F]>` above.
...
- **Capability B (type → schema)** has its linchpin: `ReflectStruct` and its
  three sibling traits are synthesized, the `where`-clause pack binding that
  carries `impl<T: ReflectStruct<FieldTypes = [..F]>, ..F: JsonSchema>` resolves,
