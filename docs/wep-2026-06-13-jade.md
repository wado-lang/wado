...
- Assertion keywords (`minLength`, `minimum`, `pattern`, `format`, …) from the
  ...
  JsonSchema for T` maps that `Constraint` value to keywords. The same value
  guards the `Deserialize` boundary, so a schema and the parser that enforces it
  cannot disagree — and the constraint is stated once no matter how many structs
  carry the type.
