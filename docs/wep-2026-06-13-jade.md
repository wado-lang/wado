...
- **Capability B (type → schema)** is blocked on `ReflectStruct` (unimplemented). The
  first consumer, Kiln's `describe-options` (see
  [the Kiln WEP](./wep-2026-04-12-kiln.md)), sidesteps it: the compiler derives
  the schema from a generator's source-extracted `OptionsDescriptor` in Rust and
  emits a `package-jade` `Schema`. Library-level `#[derive(Jade)]` lands with
  `ReflectStruct`.
