...
- **Kiln** ([`wep-2026-04-12-kiln.md`](./wep-2026-04-12-kiln.md)) — capability D
  ...
  { generator: … }`), the same mechanism as Gale (`.g4`).
...
four letters alongside Gale and Kiln. Imported as `wado:jade@0.1`.
...
- **The base's schema, in the constrained blanket.** `impl<T: Constrained +
  ...
  every constrained type. A bound reaches a parameter, not a projection of one,
  so `T::Base: JsonSchema` has no form in the grammar.
- **Which blanket owns a constrained composite.** A struct wrapper that
  ...
  blanket, so the shape emitted is whichever registered first. Nothing names an
  owner, and registration order is not part of the design.
