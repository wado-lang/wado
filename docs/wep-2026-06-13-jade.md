...
- **Rust** keeps B (schemars) and C (the `jsonschema` crate) in separate
  crates. schemars is tightly coupled to serde — `#[wire(name_policy, default,
...
- **serde derive + attributes** (`#[wire(name_policy)]`, field defaults
  ...
  consistent.
- **`core:serde`'s `Constrained`** — a type states its own permitted range, so
  the assertion keywords B emits and the check the `Deserialize` boundary runs
  read one value. Neither schemars (its `#[validate]` equivalent lives in a
  separate crate that serde never consults) nor Go's tags close that gap.
- **`ReflectStruct` — compile-time structural introspection**
  ...
  tuple (`type FieldTypes = [..F]`, `field_names()`). This is the static,
...
- **Format-bearing stdlib** — `core:temporal` (`date-time`, `date`, `time`,
  `duration`), `core:url` (`uri`), `core:uuid` (`uuid`) supply the `format`
  assertions other ecosystems reimplement.
...
B  derivation   JsonSchema: T → Schema             (library code over `ReflectStruct`)
...
pub struct Schema {
    ...
    // Applicators
    type: Option<TypeSet>,       // "string" | ["string","null"]
    ...
    default: Option<Value>, examples: List<Value>,
...
A struct-field position accepts any keyword, so every key can be spelled after
its JSON Schema name: `type` (a contextual keyword) and the plain identifiers
`ref` / `not` / `default` need no adornment. `enum_` / `const_` keep a `_` suffix
only by convention — the hard keywords `enum` / `const` are fine as field names
but not as plain bindings — and still serialize under the exact JSON Schema key.
...
type's structure at compile time_. It does — or will: `ReflectStruct`
([`wep-2026-03-14-variadic-type-parameters.md`](./wep-2026-03-14-variadic-type-parameters.md),
§10). `JsonSchema` is therefore an **ordinary generic library trait that Jade
writes on top of `ReflectStruct`**, not a compiler-synthesized one. The compiler
...
by walking `ReflectStruct`'s field pack — the same compile-time `[..F::method()]`
expansion `[..T::default()]` already uses, with no runtime cost and no
per-struct boilerplate. The pack binding is the inline-bound form `core:serde`'s
own blankets use (WEP 2026-03-14 §11):

```wado
// Jade library code (illustrative): structure from Reflect, value-free.
impl<T: ReflectStruct<FieldTypes = [..F]>, ..F: JsonSchema> JsonSchema for T {
    // `type_name()` names the declaration; a `$defs` key is per instantiation.
    fn schema_name() -> String { return key_of(Reflect::<T>::type_info()); }

    fn json_schema(gen: &mut SchemaGenerator) -> Schema {
        let meta = T::field_meta();                  // wire_name, has_default, doc
...
`field_meta()` is the per-field metadata `ReflectStruct` exposes (see "Language
dependency"): the wire name, whether the field has a default, and its doc
string. A `variant` / `enum` / `flags` type derives through the analogous
`ReflectVariant` / `ReflectEnum` / `ReflectFlags` blanket impls — a `variant` to
`oneOf`, an `enum` to a string `enum`.

It _shares the serde metadata_: `#[wire(name_policy = "camelCase")]` renames
...
Constraints and annotations come from two sources Jade does not own:

- `description` / `title` from `///` doc comments — the idiomatic source
  (schemars does the same); no attribute needed.
- Assertion keywords (`minLength`, `minimum`, `pattern`, `format`, …) from the
  field's _type_, which declares its own permitted range through `core:serde`'s
  `Constrained` (see [Serde](./wep-2026-02-28-serde.md)). Jade's
  `impl<T: Constrained + Reflect> JsonSchema for T` takes `T::Base`'s schema for
  the shape and adds the `Constraint`'s keywords — `{"type":"string"}` from the
  base, `minLength` from the constraint — while the `$defs` key comes from the
  identity root, which a constrained type satisfies whether it is a newtype or a
  struct wrapper (a kind bound would admit only one). The same `Constraint`
  guards the `Deserialize` boundary, so the schema and the parser enforcing it
  cannot disagree, and it is stated once however many structs carry the type.

```wado
type UserName = String;   // impl Constrained: min_length 1, max_length 64
type Email = String;      // impl Constrained: format "email"
type Age = i32;           // impl Constrained: minimum 0, maximum 150

#[wire(name_policy = "camelCase")]
struct CreateUser {
    /// The user's login name.
    user_name: UserName,   // → {"$ref": "#/$defs/UserName"}, and in $defs:
                           //   {"type":"string","minLength":1,"maxLength":64,
                           //    "description":"The user's login name."}
    email: Email,
    age: Age = 0,          // default ⇒ not required
    /// ISO-4217 code.
    currency: String,      // unconstrained: just {"type":"string"}
}
```

A named constrained type also earns a `$defs` entry and a `$ref` per use, where
a per-field attribute could only inline the same keywords at every site.
...
- **`format` is asserting and pluggable.** Built-in assertions delegate:
  `date-time` / `date` / `time` / `duration` → `core:temporal`, `uri` →
  `core:url`, `uuid` → `core:uuid`, `regex` → the pattern engine below. Per the spec, `format` is _annotation by
...
`object` → `struct` (with `#[wire]` attributes reconstructed from the naming),
a schema carrying assertion keywords → a newtype plus its `Constrained` impl,
`oneOf` of tagged objects → `variant`, `enum` of strings →
...
### Language dependency — `ReflectStruct`

Jade itself ships zero compiler changes; everything in Layers A, C, D is plain
library code. But Layer B needs the compiler to expose type structure — the same
dependency that moved serde derivation out of `synthesis/serde_synth.rs`. That work is _not_ Jade's to define inline — it is general language
...
- **`ReflectStruct` synthesis + pack binding** — powers the value-free
  `impl<T: ReflectStruct<FieldTypes = [..F]>, ..F> …` above.
- **Per-field `ReflectStruct` metadata** — wire name (serde rename), has-default
  (→ `required`), doc string (→ `description`).
- **Variant / enum / flags reflection** — for `oneOf` and string `enum`.
- **`Reflect` / `TypeInfo`** — the identity root every nameable type carries, and
  a `$defs` key that separates `Pair<String>` from `Pair<i32>`.

The companion WEP owns the full design.

Assertion keywords are not on that list: they come from `core:serde`'s
`Constrained`, which is library code Jade reads rather than a language feature
it waits for.
...
3. **B — derivation** is the only layer gated on the companion language WEP —
   now on its remaining two pieces (`Member::doc()` and `TypeInfo`), the
   reflection layer itself having landed. It lands as those
...
- **`Constraint` key surface.** Which JSON Schema assertions `core:serde`'s
  `Constraint` carries versus what is left to hand-authoring on the `Schema`
  value. The mechanism is settled (a type states its own range through
  `Constrained`, enforced at the `Deserialize` boundary and read by Jade for
  keywords); only the recognized key set is open, and it is the JSON Schema
  assertion vocabulary that bounds it.
- **The base's schema, in the constrained blanket.** `impl<T: Constrained +
  Reflect> JsonSchema for T` reads `T::Base`'s shape, but `Constrained`
  guarantees only `Base: Deserialize`, so the blanket does not type-check for
  every constrained type. Closing it takes either a bound on the projection
  (`T::Base: JsonSchema`), which the grammar has no form for — no `where`
  clauses, and an inline bound reaches only the parameter — or Jade producing
  the base shape itself from a closed set of base types.
- **Which blanket owns a constrained composite.** A struct wrapper that
  implements `Constrained` satisfies both that blanket and the structural
  `ReflectStruct` one; Wado has no specialization and picks the first satisfied
  blanket, so the shape emitted is whichever registered first. Closing it takes
  one owner: either the constrained vehicle narrows to newtypes, whose reflected
  kind no structural blanket matches, or the two blankets merge into one that
  asks for the constraint and falls back to the structure.
...
- **No `#[jade(…)]` attribute — and no validation attribute at all.** A package
  that merely lives in this repo is an ordinary library; it cannot own a
  compiler-recognized attribute, and Wado has no reflection or macros for it to
  read one itself. A built-in attribute was the alternative, and it is not
  needed: a constraint is a property of a type, so `core:serde`'s `Constrained`
  carries it as an ordinary trait impl — enforced at the `Deserialize` boundary
  and read by Jade for keywords, both from the same value. That leaves the
  compiler out of the vocabulary entirely, where an attribute would have made it
  own every recognized key. `description` comes from `///` doc comments.
- **Capability B is library code, not compiler magic.** It is built on the
  compile-time `ReflectStruct` introspection, in the same no-dynamic-reflection,
  monomorphized lineage as tuple `for-of`. The compiler gains general type
  introspection; it never learns about Jade.

## Implementation status

- **Capability A (Schema document model) — minimal, in `package-jade`.** A
  `Schema` struct for the Draft 2020-12 subset needed to describe Kiln generator
  options (objects, primitives, string `enum`, `required`, `default`,
  `additionalProperties`, type unions with `null`) plus a hand-written
  `Serialize` that omits absent fields, so the emitted schema stays idiomatic.
  Serves both JSON (`core:json`) and CBOR (`core:cbor`). Deserialize and the
  full keyword surface are follow-ups.
- **Capability B (type → schema)** has its linchpin: `ReflectStruct` and its
  three sibling traits are synthesized, the pack binding that carries
  `impl<T: ReflectStruct<FieldTypes = [..F]>, ..F: JsonSchema>` resolves,
  and `core:serde` derives through both. What B still waits on is the two facts
  a schema needs that serde does not (see the companion WEP's known gaps):
  `Member::doc()` for `description` / `title`, and `TypeInfo` for a `$defs` key
  that separates `Pair<String>` from `Pair<i32>`. Assertion keywords wait on
  `core:serde`'s `Constrained` instead, which is library work.
  Its originally-planned first consumer, a schema for
  [Kiln](./wep-2026-04-12-kiln.md) generator options, no longer needs any of it:
  options cross the generator boundary as a typed WIT value, so their shape is
  read from the component's own WIT rather than from a derived schema document.
- **Capability C (validation)** and **D (codegen)** are future work.
