...
{
    fn schema_name() -> String { return TypeInfo::of::<T>().name(); }
...
- Assertion keywords (`minLength`, `minimum`, `pattern`, `format`, …) from the
  ...
  stated once no matter how many structs carry the type. The `$defs` key is
  `TypeInfo::of::<T>().name()`, which is why that call carries no reflection
  bound: a constrained type may be a newtype or a struct wrapper, and the
  blanket must name either.
...
- **`TypeInfo`** — a `$defs` key that separates `Pair<String>` from `Pair<i32>`.
