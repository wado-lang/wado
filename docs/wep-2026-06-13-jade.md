...
- **Validation keywords (`min_length`, `minimum`, `pattern`, `format`, …) from
  ...
  namespace. Jade reads its entries via `ReflectStruct`; the same attribute is
...
### Language dependency — `ReflectStruct` and `#[validate]`
...
- **The generic, built-in `#[validate(…)]` attribute** — Jade reads its entries
  via `ReflectStruct` for schema keywords; it is also enforced at the `Deserialize`
...
3. **B — derivation** is the only layer gated on the companion language WEP —
   now on its remaining three pieces (`#[validate]`, `Member::doc()`,
   `TypeInfo`), the reflection layer itself having landed. It lands as those
...
- **`#[validate]` keyword surface.** Which JSON Schema assertions are worth a
  ...
  `Deserialize` boundary and read via `ReflectStruct`); only the recognized key set is
  open.
...
- **No `#[jade(…)]` attribute.** A package that merely lives in this repo is an
  ...
  boundary so it is never silently inert, and read via `ReflectStruct` for schema
...
- **Capability B (type → schema)** has its linchpin: `ReflectStruct` and its
  ...
  and `core:serde` derives through both. What B still waits on is the three facts
  a schema needs that serde does not (see the companion WEP's known gaps):
  `#[validate(…)]` for the assertion keywords, `Member::doc()` for `description`
  / `title`, and `TypeInfo` for a `$defs` key that separates `Pair<String>` from
  `Pair<i32>`. Its originally-planned first consumer, a schema for
