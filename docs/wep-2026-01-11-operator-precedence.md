...
5. **No short-circuit**: every operand is evaluated once, left to right, and the comparisons join with `&`. A chain is a range test, and a test that skips an operand on one path is what `&&` is for.
