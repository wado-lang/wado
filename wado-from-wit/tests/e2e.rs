...
"#;
    ...
    // `self` is a reserved keyword, so it's escaped as `self_`
    assert!(output.contains("fn method_file_read(self_: &File, len: u32) -> Array<u8>;"));
    assert!(output.contains("fn method_file_write(self_: &File, data: Array<u8>);"));
}
